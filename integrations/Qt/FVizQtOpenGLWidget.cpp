...
FVizQtOpenGLWidget::FVizQtOpenGLWidget(
    QWidget* parent,
...
bool FVizQtOpenGLWidget::isValid() const noexcept
{
    return window_ != nullptr && initialization_result_ == FVIZ_OK;
}

FVizResult FVizQtOpenGLWidget::initializationResult() const noexcept
{
    return initialization_result_;
}

FVizRenderWindow* FVizQtOpenGLWidget::renderWindow() noexcept
{
    return window_;
}

FVizRenderer* FVizQtOpenGLWidget::renderer() noexcept
{
    return window_ != nullptr ? fviz_render_window_renderer(window_) : nullptr;
}

FVizRenderWindowInteractor* FVizQtOpenGLWidget::interactor() noexcept
{
    return window_ != nullptr ? fviz_render_window_interactor(window_) : nullptr;
}

FVizResult FVizQtOpenGLWidget::requestRender()
{
...
bool FVizQtOpenGLWidget::renderPending() const noexcept
{
    return window_ != nullptr && fviz_render_window_render_requested(window_) != FVIZ_FALSE;
}

FVizResult FVizQtOpenGLWidget::renderIfPending()
{
    if (window_ == nullptr) return FVIZ_ERROR_INVALID_STATE;
...
FVizResult FVizQtOpenGLWidget::addObserver(
    FVizObject* object,
    FVizEventId eventId,
    float priority,
...
FVizResult FVizQtOpenGLWidget::addCommandObserver(
    FVizObject* object,
    FVizEventId eventId,
    float priority,
    FVizCommand* command,
...
FVizResult FVizQtOpenGLWidget::removeObserver(FVizObject* object, FVizObserverTag tag)
{
    if (object == nullptr) return FVIZ_ERROR_INVALID_ARGUMENT;
    return fviz_object_remove_observer(object, tag);
}

void FVizQtOpenGLWidget::setInteractorTimerPumpEnabled(bool enabled)
{
...
bool FVizQtOpenGLWidget::interactorTimerPumpEnabled() const noexcept
{
    return timer_pump_requested_;
}

void FVizQtOpenGLWidget::setInteractorTimerPumpInterval(int milliseconds)
{
    timer_pump_.setInterval(std::max(1, milliseconds));
}

int FVizQtOpenGLWidget::interactorTimerPumpInterval() const noexcept
{
    return timer_pump_.interval();
}

void FVizQtOpenGLWidget::initializeGL()
{
    if (context() != nullptr)
    {
    ...
    surface.make_current = &FVizQtOpenGLWidget::makeCurrentCallback;
    surface.get_framebuffer_size = &FVizQtOpenGLWidget::framebufferSizeCallback;
    surface.get_default_framebuffer = &FVizQtOpenGLWidget::defaultFramebufferCallback;
    surface.request_render = &FVizQtOpenGLWidget::requestRenderCallback;
...
void FVizQtOpenGLWidget::resizeGL(int, int)
{
    if (window_ == nullptr) return;
    (void)fviz_render_window_sync_external_surface_size(window_);
...
void FVizQtOpenGLWidget::paintGL()
{
    if (window_ == nullptr) return;
    (void)fviz_render_window_sync_external_surface_size(window_);
...
bool FVizQtOpenGLWidget::event(QEvent* event)
{
    ...
    if (event != nullptr)
    {
        if (event->type() == QEvent::Hide)
            timer_pump_.stop();
        else if (event->type() == QEvent::Show && timer_pump_requested_ && isVisible() &&
...
void FVizQtOpenGLWidget::mousePressEvent(QMouseEvent* event)
{
    double x = 0.0;
    double y = 0.0;
    ...
    {
        if (interaction.button != FVIZ_MOUSE_BUTTON_NONE) grabMouse();
        event->accept();
    }
    else
    {
...
void FVizQtOpenGLWidget::mouseReleaseEvent(QMouseEvent* event)
{
    double x = 0.0;
    double y = 0.0;
    eventPosition(event, &x, &y);
    ...
    if (event->buttons() == Qt::NoButton) releaseMouse();
    if (handled) event->accept();
    else QOpenGLWidget::mouseReleaseEvent(event);
}

void FVizQtOpenGLWidget::mouseDoubleClickEvent(QMouseEvent* event)
{
    double x = 0.0;
    double y = 0.0;
    eventPosition(event, &x, &y);
    ...
    else QOpenGLWidget::mouseDoubleClickEvent(event);
}

void FVizQtOpenGLWidget::mouseMoveEvent(QMouseEvent* event)
{
    double x = 0.0;
    double y = 0.0;
    eventPosition(event, &x, &y);
    ...
    if (dispatch(&interaction)) event->accept();
    else QOpenGLWidget::mouseMoveEvent(event);
}

void FVizQtOpenGLWidget::wheelEvent(QWheelEvent* event)
{
    double x = 0.0;
    double y = 0.0;
    ...
    if (dispatch(&interaction)) event->accept();
    else QOpenGLWidget::wheelEvent(event);
}

void FVizQtOpenGLWidget::keyPressEvent(QKeyEvent* event)
{
...
void FVizQtOpenGLWidget::keyReleaseEvent(QKeyEvent* event)
{
    FVizInteractionEvent interaction = makeBasicEvent(FVIZ_INTERACTION_KEY_UP);
    interaction.key = event->key() == Qt::Key_Escape ? FVIZ_KEY_ESCAPE : event->key();
    if (dispatch(&interaction)) event->accept();
    else QOpenGLWidget::keyReleaseEvent(event);
}

void FVizQtOpenGLWidget::focusInEvent(QFocusEvent* event)
{
...
void FVizQtOpenGLWidget::focusOutEvent(QFocusEvent* event)
{
...
FVizResult FVizQtOpenGLWidget::makeCurrentCallback(void* userData)
{
    auto* self = static_cast<FVizQtOpenGLWidget*>(userData);
...
FVizResult FVizQtOpenGLWidget::framebufferSizeCallback(void* userData, int* width, int* height)
{
    auto* self = static_cast<FVizQtOpenGLWidget*>(userData);
...
uint32_t FVizQtOpenGLWidget::defaultFramebufferCallback(void* userData)
{
    auto* self = static_cast<FVizQtOpenGLWidget*>(userData);
    return self != nullptr ? static_cast<uint32_t>(self->defaultFramebufferObject()) : 0u;
}

void FVizQtOpenGLWidget::requestRenderCallback(void* userData)
{
    auto* self = static_cast<FVizQtOpenGLWidget*>(userData);
    if (self != nullptr) self->update();
}

FVizInteractionEvent FVizQtOpenGLWidget::makePointerEvent(
...
FVizInteractionEvent FVizQtOpenGLWidget::makeBasicEvent(FVizInteractionEventType type) const
{
...
bool FVizQtOpenGLWidget::dispatch(FVizInteractionEvent* event)
{
    FVizRenderWindowInteractor* current = interactor();
...
void FVizQtOpenGLWidget::pumpInteractorTimers()
{
    FVizRenderWindowInteractor* current = interactor();
...
double FVizQtOpenGLWidget::timestampSeconds() const noexcept
{
