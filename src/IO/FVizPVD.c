...
static const FVizObjectClass g_fviz_pvd_collection_class = {FVIZ_TYPE_PVD_COLLECTION, "FVizPVDCollection",
                                                            &g_fviz_object_class, fviz_pvd_collection_destroy,
                                                            fviz_pvd_collection_mtime};
...
FVizResult fviz_pvd_collection_create(FVizPVDCollection** out_collection)
{
    ...
    *out_collection = NULL;
    collection =
        (FVizPVDCollection*)fviz_internal_object_allocate(sizeof(*collection), &g_fviz_pvd_collection_class, NULL);
...
FVizResult fviz_pvd_collection_add(FVizPVDCollection* collection, double time, uint32_t part, const char* group,
                                   const char* file, FVizSize* out_index)
{
    FVizPVDEntry entry;
...
FVizResult fviz_pvd_collection_find_nearest(const FVizPVDCollection* collection, double time, FVizSize* out_index)
{
    ...
    while (*out_index > 0u &&
           fviz_pvd_collection_time(collection, *out_index - 1u) == fviz_pvd_collection_time(collection, *out_index))
        --(*out_index);
    return FVIZ_OK;
}
...
static FVizBool fviz_pvd_attr(const char* tag, const char* name, char* out, FVizSize capacity)
{
    ...
    while (start < end && written + 1u < capacity)
    {
        ...
        if (*start == '&')
        {
            if ((FVizSize)(end - start) >= 5u && strncmp(start, "&amp;", 5u) == 0)
            {
                decoded = '&';
                consumed = 5u;
            }
            else if ((FVizSize)(end - start) >= 4u && strncmp(start, "&lt;", 4u) == 0)
            {
                decoded = '<';
                consumed = 4u;
            }
            ...
            else if ((FVizSize)(end - start) >= 6u && strncmp(start, "&quot;", 6u) == 0)
            {
                decoded = '"';
                consumed = 6u;
            }
...
FVizResult fviz_pvd_read(const char* file_path, FVizPVDCollection** out_collection)
{
    ...
    if (file == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_IO, "failed to open PVD file");
    ...
    if (size > 0u && fread(text, 1u, size, file) != size)
    {
        result = FVIZ_ERROR_IO;
        goto cleanup;
    }
    ...
    while ((cursor = strstr(cursor, "<DataSet")) != NULL)
    {
        ...
        if (parse_end == time_text || !isfinite(time))
        {
            result = FVIZ_ERROR_PARSE;
            goto cleanup;
        }
        ...
        if (part > UINT32_MAX ||
            fviz_pvd_collection_add(collection, time, (uint32_t)part, group, path, NULL) != FVIZ_OK)
        {
            result = fviz_last_error_code() == FVIZ_OK ? FVIZ_ERROR_PARSE : fviz_last_error_code();
            goto cleanup;
        }
    ...
    {
        result = FVIZ_ERROR_PARSE;
        fviz_internal_set_error(FVIZ_ERROR_PARSE, "PVD collection contains no datasets");
        goto cleanup;
    }
    *out_collection = collection;
    collection = NULL;
...
FVizResult fviz_pvd_write(const char* file_path, const FVizPVDCollection* collection)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "PVD writer requires a non-empty collection");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    file = fopen(file_path, "wb");
    if (file == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_IO, "failed to open PVD output");
        return FVIZ_ERROR_IO;
    }
