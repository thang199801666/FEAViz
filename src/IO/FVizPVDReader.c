...
static const FVizObjectClass g_fviz_pvd_reader_class={
    FVIZ_TYPE_PVD_READER,"FVizPVDReader",&g_fviz_object_class,
...
static FVizResult fviz_pvd_reader_ensure_collection(FVizPVDReader* reader)
{
    ...
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE,"PVD reader has no file name"); return FVIZ_ERROR_INVALID_STATE; }
    if (fviz_pvd_read(fviz_string_c_str(reader->file_name),&replacement)!=FVIZ_OK) return fviz_last_error_code();
    fviz_release(reader->collection); reader->collection=replacement;
...
static FVizResult fviz_pvd_resolve_path(const char* pvd_path,const char* child,FVizString** out_path)
{
    ...
    *out_path=NULL;
    ...
    slash1=strrchr(pvd_path,'/'); slash2=strrchr(pvd_path,'\\');
    ...
    if (slash!=NULL)
    {
        ...
        if (buffer==NULL) { fviz_release(result); return fviz_last_error_code(); }
        (void)memcpy(buffer,pvd_path,prefix); buffer[prefix]='\0';
        if (fviz_string_set(result,buffer)!=FVIZ_OK) { fviz_free(buffer); fviz_release(result); return fviz_last_error_code(); }
        fviz_free(buffer);
    }
    if (fviz_string_append(result,child)!=FVIZ_OK) { fviz_release(result); return fviz_last_error_code(); }
    *out_path=result; return FVIZ_OK;
}

static FVizResult fviz_pvd_load_entry(
    FVizPVDReader* reader, FVizSize index, FVizDataObject** out_data);
...
{
    const FVizSize count = reader != NULL && reader->collection != NULL
        ? fviz_pvd_collection_count(reader->collection) : 0u;
    const double time = first_index < count ? fviz_pvd_collection_time(reader->collection, first_index) : 0.0;
    FVizSize end = first_index;
    while (end < count && fviz_pvd_collection_time(reader->collection, end) == time) ++end;
    return end;
}

static FVizResult fviz_pvd_ensure_piece_reader(
    FVizPVDReader* reader,
    FVizSize entry_index,
    FVizPVTUReader** out_piece_reader)
{
    ...
    if (result == FVIZ_OK)
        result = fviz_pvtu_reader_set_file_name(replacement, fviz_string_c_str(resolved));
...
static FVizResult fviz_pvd_piece_count_for_group(
    FVizPVDReader* reader, FVizSize first_index, uint32_t* out_piece_count)
{
    ...
    if (reader == NULL || out_piece_count == NULL || group_count == 0u)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    if (group_count > 1u)
    {
    ...
    if (strcmp(ext, ".pvtu") == 0 || strcmp(ext, ".PVTU") == 0)
    {
        ...
        const FVizSize piece_count =
            fviz_pvd_ensure_piece_reader(reader, first_index, &piece_reader) == FVIZ_OK
            ? fviz_pvtu_reader_piece_count(piece_reader) : 0u;
...
static FVizResult fviz_pvd_load_time_piece(
    FVizPVDReader* reader,
    FVizSize first_index,
    uint32_t piece,
    ...
    FVizDataObject** out_data)
{
    ...
    if (reader == NULL || out_data == NULL || number_of_pieces == 0u ||
        piece >= number_of_pieces || group_count == 0u)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    if (group_count > 1u)
    {
        ...
        {
            fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "PVD piece request does not match timestep part count");
            return FVIZ_ERROR_INVALID_ARGUMENT;
        }
        if (ghost_levels != 0u)
        {
            fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED, "PVD multi-entry piece requests cannot synthesize additional ghost layers");
            return FVIZ_ERROR_NOT_SUPPORTED;
        }
    ...
    }
    {
        ...
        {
            fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "PVD/PVTU piece request does not match manifest piece count");
            return FVIZ_ERROR_INVALID_ARGUMENT;
        }
        if (ghost_levels > fviz_pvtu_reader_ghost_level(piece_reader))
        {
            fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED, "PVTU manifest does not contain the requested ghost depth");
            return FVIZ_ERROR_NOT_SUPPORTED;
        }
...
static FVizResult fviz_pvd_load_entry(FVizPVDReader* reader,FVizSize index,FVizDataObject** out_data)
{
    ...
    if (fviz_pvd_resolve_path(fviz_string_c_str(reader->file_name),file,&resolved)!=FVIZ_OK) return fviz_last_error_code();
    ext=fviz_extension(fviz_string_c_str(resolved));
    if (strcmp(ext,".vtp")==0 || strcmp(ext,".VTP")==0)
    {
        FVizPolyData* poly=NULL;
        if (fviz_vtp_read(fviz_string_c_str(resolved),&poly)!=FVIZ_OK) { fviz_release(resolved); return fviz_last_error_code(); }
        data=(FVizDataObject*)poly;
    }
    else if (strcmp(ext,".vtu")==0 || strcmp(ext,".VTU")==0)
    {
        FVizUnstructuredGrid* grid=NULL;
        if (fviz_vtu_read(fviz_string_c_str(resolved),&grid)!=FVIZ_OK) { fviz_release(resolved); return fviz_last_error_code(); }
        data=(FVizDataObject*)grid;
    }
    else if (strcmp(ext,".pvtu")==0 || strcmp(ext,".PVTU")==0)
    {
        FVizPartitionedDataSet* partitions=NULL;
        if (fviz_pvtu_read(fviz_string_c_str(resolved),&partitions)!=FVIZ_OK) { fviz_release(resolved); return fviz_last_error_code(); }
        data=(FVizDataObject*)partitions;
    }
    else
    {
        fviz_release(resolved);
        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,"PVD frame extension is not supported; expected .vtp, .vtu, or .pvtu");
        return FVIZ_ERROR_NOT_SUPPORTED;
    }
    fviz_release(resolved);
    *out_data=data;
...
static FVizResult fviz_pvd_load_time_group(FVizPVDReader* reader,FVizSize first_index,FVizDataObject** out_data)
{
    ...
    while (last_index<count && fviz_pvd_collection_time(reader->collection,last_index)==selected) ++last_index;
    ...
    else
    {
        ...
        { fviz_release(partitions); return fviz_last_error_code(); }
    ...
    if (fviz_pvd_cache_store(reader,first_index,result)!=FVIZ_OK) { fviz_release(result); return fviz_last_error_code(); }
    *out_data=result;
    return FVIZ_OK;
}
...
static FVizResult fviz_pvd_reader_process_request(FVizAlgorithm* algorithm,const FVizPipelineRequestInfo* request,void* state)
{
    ...
    if (request->type==FVIZ_PIPELINE_REQUEST_INFORMATION)
    {
        ...
        if (fviz_algorithm_set_output_time_steps(algorithm,request->requested_output_port,unique_times,unique_count)!=FVIZ_OK)
        { fviz_free(unique_times); return fviz_last_error_code(); }
    ...
    if (request->type==FVIZ_PIPELINE_REQUEST_DATA)
    {
        ...
        const double requested=request->has_time!=FVIZ_FALSE?request->time:fviz_pvd_collection_time(reader->collection,0u);
        if (fviz_pvd_collection_find_nearest(reader->collection,requested,&index)!=FVIZ_OK)
            return fviz_last_error_code();
        if (request->number_of_pieces > 1u)
            load_result = fviz_pvd_load_time_piece(
                reader, index, request->piece, request->number_of_pieces,
...
FVizResult fviz_pvd_reader_create(FVizPVDReader** out_reader)
{
    ...
    if (out_reader==NULL) { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"out_reader must not be NULL"); return FVIZ_ERROR_INVALID_ARGUMENT; }
    *out_reader=NULL;
    ...
    reader->cache_capacity=3u; reader->collection_dirty=FVIZ_TRUE;
    if (fviz_string_create(&reader->file_name)!=FVIZ_OK) { fviz_release(reader); return fviz_last_error_code(); }
    ...
    { fviz_release(reader); return fviz_last_error_code(); }
    *out_reader=reader; return FVIZ_OK;
}
...
FVizResult fviz_pvd_reader_set_file_name(FVizPVDReader* reader,const char* file_path)
{
    ...
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"PVD file name must not be empty"); return FVIZ_ERROR_INVALID_ARGUMENT; }
    ...
    fviz_release(reader->collection); reader->collection=NULL;
    fviz_pvd_reader_clear_cache(reader);
    fviz_release(reader->piece_reader); reader->piece_reader=NULL;
    reader->piece_reader_valid=FVIZ_FALSE;
    fviz_object_modified((FVizObject*)reader);
    return FVIZ_OK;
}
const char* fviz_pvd_reader_file_name(const FVizPVDReader* reader){return reader!=NULL&&reader->file_name!=NULL?fviz_string_c_str(reader->file_name):NULL;}
const FVizPVDCollection* fviz_pvd_reader_collection(const FVizPVDReader* reader){return reader!=NULL?reader->collection:NULL;}
FVizAlgorithm* fviz_pvd_reader_algorithm(FVizPVDReader* reader){return reader!=NULL?reader->algorithm:NULL;}
FVizAlgorithmOutput* fviz_pvd_reader_output_port(FVizPVDReader* reader){return reader!=NULL?fviz_algorithm_output_port(reader->algorithm,0u):NULL;}
FVizDataObject* fviz_pvd_reader_output(FVizPVDReader* reader){return reader!=NULL?fviz_algorithm_output_data(reader->algorithm,0u):NULL;}
FVizResult fviz_pvd_reader_update(FVizPVDReader* reader){return reader!=NULL?fviz_algorithm_update(reader->algorithm):FVIZ_ERROR_INVALID_ARGUMENT;}
...
FVizResult fviz_pvd_reader_update_piece_time(
    FVizPVDReader* reader,
    double time,
    uint32_t piece,
    uint32_t number_of_pieces,
...
FVizResult fviz_pvd_reader_piece_count_at_time(
    FVizPVDReader* reader, double time, uint32_t* out_piece_count)
{
    FVizSize index = 0u;
    if (out_piece_count != NULL) *out_piece_count = 0u;
    if (reader == NULL || out_piece_count == NULL || !isfinite(time))
        return FVIZ_ERROR_INVALID_ARGUMENT;
...
double fviz_pvd_reader_selected_time(const FVizPVDReader* reader){return reader!=NULL?reader->selected_time:0.0;}
...
FVizResult fviz_pvd_reader_prefetch_time(FVizPVDReader* reader,double time)
{
    ...
    if (fviz_pvd_collection_find_nearest(reader->collection,time,&index)!=FVIZ_OK ||
        fviz_pvd_load_time_group(reader,index,&data)!=FVIZ_OK) return fviz_last_error_code();
    fviz_release(data);
    return FVIZ_OK;
}

FVizResult fviz_pvd_reader_prefetch_piece_time(
    FVizPVDReader* reader,
    double time,
    uint32_t piece,
    uint32_t number_of_pieces,
    uint32_t ghost_levels)
{
    ...
    if (fviz_pvd_reader_ensure_collection(reader) != FVIZ_OK ||
        fviz_pvd_collection_find_nearest(reader->collection, time, &index) != FVIZ_OK)
        return fviz_last_error_code();
    result = fviz_pvd_load_time_piece(
        reader, index, piece, number_of_pieces, ghost_levels, &data);
    fviz_release(data);
    return result;
}
...
FVizResult fviz_pvd_reader_set_cache_byte_capacity(
    FVizPVDReader* reader, FVizSize byte_capacity)
{
    if (reader == NULL) return FVIZ_ERROR_INVALID_ARGUMENT;
...
void fviz_pvd_reader_clear_cache(FVizPVDReader* reader)
{
    ...
    for (i=0u;i<reader->cache_size;++i) fviz_release(reader->cache_entries[i].data);
