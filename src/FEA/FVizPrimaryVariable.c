...
static const FVizObjectClass g_fviz_fea_primary_result_class = {
    FVIZ_TYPE_FEA_PRIMARY_VARIABLE_RESULT,
    "FVizFEAPrimaryVariableResult",
    NULL,
    fviz_fea_primary_result_destroy,
    fviz_fea_primary_local_mtime
};

static const FVizObjectClass g_fviz_fea_primary_evaluator_class = {
    FVIZ_TYPE_FEA_PRIMARY_VARIABLE_EVALUATOR,
    "FVizFEAPrimaryVariableEvaluator",
    NULL,
    fviz_fea_primary_evaluator_destroy,
    fviz_fea_primary_local_mtime
};
...
FVizFEAPrimaryVariableCacheStatistics fviz_fea_primary_variable_evaluator_cache_statistics(
...
static FVizBool fviz_fea_is_integer_array(const FVizDataArray* array)
{
    ...
    return array!=NULL && fviz_data_array_components(array)==1u && type>=FVIZ_DATA_INT8 && type<=FVIZ_DATA_UINT64;
}
...
static FVizResult fviz_fea_read_id(const FVizDataArray* array,FVizSize tuple,FVizId* out_id)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"result label array must be a one-component integer array");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    data=fviz_data_array_const_data(array);
    switch (fviz_data_array_type(array))
    {
        case FVIZ_DATA_INT8: { const int8_t v=((const int8_t*)data)[tuple]; if (v<0) goto negative; *out_id=(FVizId)v; break; }
        case FVIZ_DATA_UINT8: *out_id=(FVizId)((const uint8_t*)data)[tuple]; break;
        case FVIZ_DATA_INT16: { const int16_t v=((const int16_t*)data)[tuple]; if (v<0) goto negative; *out_id=(FVizId)v; break; }
        case FVIZ_DATA_UINT16: *out_id=(FVizId)((const uint16_t*)data)[tuple]; break;
        case FVIZ_DATA_INT32: { const int32_t v=((const int32_t*)data)[tuple]; if (v<0) goto negative; *out_id=(FVizId)v; break; }
        case FVIZ_DATA_UINT32: *out_id=(FVizId)((const uint32_t*)data)[tuple]; break;
        case FVIZ_DATA_INT64: { const int64_t v=((const int64_t*)data)[tuple]; if (v<0) goto negative; *out_id=(FVizId)v; break; }
        case FVIZ_DATA_UINT64: *out_id=(FVizId)((const uint64_t*)data)[tuple]; break;
...
static FVizResult fviz_fea_read_local_id(const FVizDataArray* array,FVizSize tuple,int64_t* out_id)
{
    ...
    if (out_id==NULL || !fviz_fea_is_integer_array(array) || tuple>=fviz_data_array_tuple_count(array))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"result local-id array must be a one-component integer array");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    data=fviz_data_array_const_data(array);
    switch (fviz_data_array_type(array))
    {
        case FVIZ_DATA_INT8: *out_id=(int64_t)((const int8_t*)data)[tuple]; break;
        case FVIZ_DATA_UINT8: *out_id=(int64_t)((const uint8_t*)data)[tuple]; break;
        case FVIZ_DATA_INT16: *out_id=(int64_t)((const int16_t*)data)[tuple]; break;
        case FVIZ_DATA_UINT16: *out_id=(int64_t)((const uint16_t*)data)[tuple]; break;
        case FVIZ_DATA_INT32: *out_id=(int64_t)((const int32_t*)data)[tuple]; break;
        case FVIZ_DATA_UINT32: *out_id=(int64_t)((const uint32_t*)data)[tuple]; break;
        case FVIZ_DATA_INT64: *out_id=((const int64_t*)data)[tuple]; break;
...
static FVizResult fviz_fea_build_label_map(const FVizDataArray* labels,FVizSize count,FVizHashMap** out_map)
{
    ...
    *out_map=NULL;
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"active GlobalIds must be a one-component integer array matching mesh size");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
...
static FVizBool fviz_fea_resolve_label(
    FVizId label,const FVizHashMap* map,FVizSize count,FVizSize* out_index)
{
    void* value=NULL;
...
static FVizBool fviz_fea_primary_cache_matches(
    const FVizFEAPrimaryVariableEvaluator* evaluator,const FVizFEAField* field,
    const FVizUnstructuredGrid* grid,const FVizFEAPrimaryVariable* variable)
{
    const FVizMTime filter_mtime=variable->entity_filter_ids!=NULL?
        fviz_object_mtime((const FVizObject*)variable->entity_filter_ids):0u;
    ...
    if (evaluator->cached_result==NULL || evaluator->cached_field!=field || evaluator->cached_grid!=grid ||
        ...
        return FVIZ_FALSE;
    if (!fviz_fea_strings_equal(evaluator->cached_instance_name!=NULL?fviz_string_c_str(evaluator->cached_instance_name):"",variable->instance_name) ||
        !fviz_fea_strings_equal(evaluator->cached_component_label!=NULL?fviz_string_c_str(evaluator->cached_component_label):"",variable->component_label))
        return FVIZ_FALSE;
    return cached->source_position==variable->source_position &&
        cached->target_position==variable->target_position &&
        cached->section_point_number==variable->section_point_number &&
        cached->operation==variable->operation && cached->component==variable->component &&
        cached->invariant==variable->invariant && cached->averaging_enabled==variable->averaging_enabled &&
...
static FVizResult fviz_fea_primary_cache_store(
    FVizFEAPrimaryVariableEvaluator* evaluator,const FVizFEAField* field,const FVizUnstructuredGrid* grid,
    const FVizFEAPrimaryVariable* variable,FVizFEAPrimaryVariableResult* result)
{
    ...
    if (variable->instance_name!=NULL && variable->instance_name[0]!='\0' &&
        fviz_string_create_from(variable->instance_name,&instance)!=FVIZ_OK) return fviz_last_error_code();
    ...
    {
        fviz_release(instance); return fviz_last_error_code();
    }
    filter=variable->entity_filter_ids!=NULL?(const FVizDataArray*)fviz_retain((void*)variable->entity_filter_ids):NULL;
    if (variable->entity_filter_ids!=NULL && filter==NULL)
    {
        fviz_release(instance); fviz_release(component); return fviz_last_error_code();
    }
    ...
    return evaluator->cached_field!=NULL && evaluator->cached_grid!=NULL && evaluator->cached_result!=NULL?
        FVIZ_OK:fviz_last_error_code();
}

static FVizBool fviz_fea_block_basic_match(
    const FVizFEAField* field,FVizSize block_index,const FVizFEAPrimaryVariable* variable)
{
    ...
    if (variable->instance_name!=NULL && variable->instance_name[0]!='\0' &&
        strcmp(block_instance,variable->instance_name)!=0) return FVIZ_FALSE;
    if (variable->section_point_number!=FVIZ_FEA_SECTION_POINT_ANY && section!=variable->section_point_number)
        return FVIZ_FALSE;
    return FVIZ_TRUE;
}

static FVizBool fviz_fea_field_has_position(
    const FVizFEAField* field,const FVizFEAPrimaryVariable* variable,FVizFEAResultPosition position)
{
    FVizSize i;
...
static FVizFEAResultPosition fviz_fea_choose_source_position(
    const FVizFEAField* field,const FVizFEAPrimaryVariable* variable)
{
    static const FVizFEAResultPosition nodal_priority[]={
        FVIZ_FEA_POSITION_NODAL,FVIZ_FEA_POSITION_ELEMENT_NODAL,FVIZ_FEA_POSITION_INTEGRATION_POINT,
        FVIZ_FEA_POSITION_CENTROID,FVIZ_FEA_POSITION_WHOLE_ELEMENT};
    static const FVizFEAResultPosition element_nodal_priority[]={
        FVIZ_FEA_POSITION_ELEMENT_NODAL,FVIZ_FEA_POSITION_INTEGRATION_POINT,
        FVIZ_FEA_POSITION_CENTROID,FVIZ_FEA_POSITION_WHOLE_ELEMENT};
    static const FVizFEAResultPosition cell_priority[]={
        FVIZ_FEA_POSITION_CENTROID,FVIZ_FEA_POSITION_WHOLE_ELEMENT,
        FVIZ_FEA_POSITION_INTEGRATION_POINT,FVIZ_FEA_POSITION_ELEMENT_NODAL};
    ...
    if (variable->source_position!=FVIZ_FEA_POSITION_UNKNOWN)
        return fviz_fea_field_has_position(field,variable,variable->source_position)?variable->source_position:FVIZ_FEA_POSITION_UNKNOWN;
    if (variable->target_position!=FVIZ_FEA_POSITION_UNKNOWN &&
        fviz_fea_field_has_position(field,variable,variable->target_position)) return variable->target_position;
    if (variable->target_position==FVIZ_FEA_POSITION_ELEMENT_NODAL)
    {
        list=element_nodal_priority; count=sizeof(element_nodal_priority)/sizeof(element_nodal_priority[0]);
    }
    else if (variable->target_position==FVIZ_FEA_POSITION_CENTROID || variable->target_position==FVIZ_FEA_POSITION_WHOLE_ELEMENT)
    {
        list=cell_priority; count=sizeof(cell_priority)/sizeof(cell_priority[0]);
    }
    for (i=0u;i<count;++i) if (fviz_fea_field_has_position(field,variable,list[i])) return list[i];
    for (i=0u;i<fviz_fea_field_block_count(field);++i)
...
static FVizFEAResultPosition fviz_fea_choose_target_position(FVizFEAResultPosition source,FVizFEAResultPosition requested)
{
    if (requested!=FVIZ_FEA_POSITION_UNKNOWN) return requested;
    if (source==FVIZ_FEA_POSITION_NODAL || source==FVIZ_FEA_POSITION_ELEMENT_NODAL || source==FVIZ_FEA_POSITION_INTEGRATION_POINT)
        return FVIZ_FEA_POSITION_NODAL;
    return source;
}

static FVizResult fviz_fea_evaluate_block_scalar(
    const FVizFEAField* field,FVizSize block_index,const FVizFEAPrimaryVariable* variable,FVizDataArray** out_scalar)
{
    ...
    *out_scalar=NULL;
    ...
    if (variable->component_label!=NULL && variable->component_label[0]!='\0')
    {
        if (fviz_fea_field_find_component(field,variable->component_label,&component)!=FVIZ_OK) return fviz_last_error_code();
    }
...
static FVizResult fviz_fea_create_raw_arrays(
    FVizDataArray** values,FVizDataArray** entity_ids,FVizDataArray** local_ids)
{
    ...
    {
        fviz_release(*values); fviz_release(*entity_ids); fviz_release(*local_ids);
        *values=NULL; *entity_ids=NULL; *local_ids=NULL;
        return fviz_last_error_code();
    }
    return FVIZ_OK;
}

static FVizResult fviz_fea_append_raw(
    FVizDataArray* values,FVizDataArray* entity_ids,FVizDataArray* local_ids,
    double value,FVizId entity_label,int64_t local_id)
{
    ...
    if (fviz_data_array_append_tuple(values,&value)!=FVIZ_OK ||
        ...
        fviz_data_array_append_tuple(local_ids,&local_id)!=FVIZ_OK) return fviz_last_error_code();
    return FVIZ_OK;
}
...
static FVizResult fviz_fea_array_range(const FVizDataArray* values,FVizBool* valid,double* minimum,double* maximum)
{
    ...
    for (i=0u;i<count;++i)
    {
        ...
        if (!found) { lo=hi=v; found=FVIZ_TRUE; }
        else { if (v<lo) lo=v; if (v>hi) hi=v; }
    }
    if (valid!=NULL) *valid=found;
    if (found) { if (minimum!=NULL) *minimum=lo; if (maximum!=NULL) *maximum=hi; }
    return FVIZ_OK;
}
...
static FVizResult fviz_fea_append_contribution(
    FVizArray* contributions,FVizSize point_id,FVizSize cell_id,FVizSize block_index,
    FVizId entity_label,int64_t local_id,double value)
{
    FVizFEAContribution c;
    c.point_id=point_id; c.cell_id=cell_id; c.block_index=block_index;
    c.entity_label=entity_label; c.local_id=local_id; c.value=value;
    return fviz_array_push(contributions,&c);
}

static FVizResult fviz_fea_dense_labels(
    const FVizDataArray* labels,FVizSize count,FVizDataArray** out_ids)
{
    ...
    *out_ids=NULL;
    ...
    { fviz_release(ids); return fviz_last_error_code(); }
    for (i=0u;i<count;++i)
    {
        FVizId id=(FVizId)i;
        if (labels!=NULL && fviz_fea_read_id(labels,i,&id)!=FVIZ_OK) { fviz_release(ids); return fviz_last_error_code(); }
...
static FVizResult fviz_fea_build_nodal_display(
    const FVizArray* contributions,const FVizDataArray* point_labels,FVizSize point_count,
    const FVizFEAPrimaryVariable* variable,FVizDataArray** out_values,FVizDataArray** out_ids,FVizDataArray** out_mask)
{
    FVizDataArray* values=NULL; FVizDataArray* ids=NULL; FVizDataArray* mask=NULL;
    double* sums=NULL,*mins=NULL,*maxs=NULL; FVizSize* counts=NULL,*first_blocks=NULL; uint8_t* multi=NULL;
    FVizSize i,n=fviz_array_count(contributions),bytes;
    if (fviz_data_array_create(FVIZ_DATA_FLOAT64,1u,&values)!=FVIZ_OK || fviz_data_array_resize(values,point_count)!=FVIZ_OK ||
        fviz_fea_dense_labels(point_labels,point_count,&ids)!=FVIZ_OK ||
        fviz_data_array_create(FVIZ_DATA_UINT8,1u,&mask)!=FVIZ_OK || fviz_data_array_resize(mask,point_count)!=FVIZ_OK) goto fail;
    if (point_count!=0u)
    {
        if (fviz_size_multiply(point_count,sizeof(double),&bytes)!=FVIZ_OK) goto fail;
        sums=(double*)fviz_alloc(bytes); mins=(double*)fviz_alloc(bytes); maxs=(double*)fviz_alloc(bytes);
        if (fviz_size_multiply(point_count,sizeof(FVizSize),&bytes)!=FVIZ_OK) goto fail;
        counts=(FVizSize*)fviz_alloc(bytes); first_blocks=(FVizSize*)fviz_alloc(bytes);
        multi=(uint8_t*)fviz_alloc(point_count);
        if (sums==NULL || mins==NULL || maxs==NULL || counts==NULL || first_blocks==NULL || multi==NULL) goto fail;
    ...
    for (i=0u;i<n;++i)
    {
        ...
        if (counts[p]==0u) { mins[p]=maxs[p]=c->value; first_blocks[p]=c->block_index; }
        else
        {
    ...
    }
    {
        ...
        for (i=0u;i<point_count;++i)
        {
            ...
            if (discontinuous) { flags[i]=1u; continue; }
            dst[i]=sums[i]/(double)counts[i];
        }
    }
    fviz_free(sums); fviz_free(mins); fviz_free(maxs); fviz_free(counts); fviz_free(first_blocks); fviz_free(multi);
    *out_values=values; *out_ids=ids; *out_mask=mask; return FVIZ_OK;
fail:
    fviz_free(sums); fviz_free(mins); fviz_free(maxs); fviz_free(counts); fviz_free(first_blocks); fviz_free(multi);
    fviz_release(values); fviz_release(ids); fviz_release(mask); return fviz_last_error_code();
}

static FVizResult fviz_fea_build_element_nodal_display(
    const FVizArray* contributions,FVizDataArray** out_values,FVizDataArray** out_entity_ids,FVizDataArray** out_local_ids,FVizDataArray** out_mask)
{
    ...
    if (fviz_data_array_create(FVIZ_DATA_FLOAT64,1u,&values)!=FVIZ_OK ||
        ...
        fviz_data_array_resize(values,n)!=FVIZ_OK || fviz_data_array_resize(entity,n)!=FVIZ_OK ||
        fviz_data_array_resize(local,n)!=FVIZ_OK || fviz_data_array_resize(mask,n)!=FVIZ_OK) goto fail;
    for (i=0u;i<n;++i)
    {
        const FVizFEAContribution* c=(const FVizFEAContribution*)fviz_array_const_at(contributions,i);
    ...
    *out_values=values; *out_entity_ids=entity; *out_local_ids=local; *out_mask=mask; return FVIZ_OK;
fail:
    fviz_release(values); fviz_release(entity); fviz_release(local); fviz_release(mask); return fviz_last_error_code();
}

static FVizResult fviz_fea_build_cell_display(
    const FVizArray* cell_samples,const FVizDataArray* cell_labels,FVizSize cell_count,
    const FVizFEAPrimaryVariable* variable,FVizDataArray** out_values,FVizDataArray** out_ids,FVizDataArray** out_mask)
{
    FVizDataArray* values=NULL,*ids=NULL,*mask=NULL;
    double* sums=NULL,*mins=NULL,*maxs=NULL; FVizSize* counts=NULL,*first_blocks=NULL; uint8_t* multi=NULL;
    FVizSize i,n=fviz_array_count(cell_samples),bytes;
    if (fviz_data_array_create(FVIZ_DATA_FLOAT64,1u,&values)!=FVIZ_OK || fviz_data_array_resize(values,cell_count)!=FVIZ_OK ||
        fviz_fea_dense_labels(cell_labels,cell_count,&ids)!=FVIZ_OK || fviz_data_array_create(FVIZ_DATA_UINT8,1u,&mask)!=FVIZ_OK ||
        fviz_data_array_resize(mask,cell_count)!=FVIZ_OK) goto fail;
    if (cell_count!=0u)
    {
        ...
        counts=(FVizSize*)fviz_alloc(bytes); first_blocks=(FVizSize*)fviz_alloc(bytes); multi=(uint8_t*)fviz_alloc(cell_count);
        if (sums==NULL||mins==NULL||maxs==NULL||counts==NULL||first_blocks==NULL||multi==NULL) goto fail;
        (void)memset(sums,0,cell_count*sizeof(double)); (void)memset(counts,0,cell_count*sizeof(FVizSize));
        (void)memset(first_blocks,0,cell_count*sizeof(FVizSize)); (void)memset(multi,0,cell_count);
    }
    for (i=0u;i<n;++i)
    {
        ...
        if (counts[cell]==0u) { mins[cell]=maxs[cell]=c->value; first_blocks[cell]=c->block_index; }
        else { if(c->value<mins[cell])mins[cell]=c->value; if(c->value>maxs[cell])maxs[cell]=c->value; if(c->block_index!=first_blocks[cell])multi[cell]=1u; }
        sums[cell]+=c->value; ++counts[cell];
    }
    {
        double* dst=(double*)fviz_data_array_data(values); uint8_t* flags=(uint8_t*)fviz_data_array_data(mask);
        for (i=0u;i<cell_count;++i)
        {
            FVizBool discontinuous=FVIZ_FALSE; dst[i]=NAN; flags[i]=0u;
            if (counts[i]==0u) continue;
            if (!variable->averaging_enabled && counts[i]>1u) discontinuous=FVIZ_TRUE;
            if (variable->averaging_enabled && !variable->average_across_blocks && multi[i]) discontinuous=FVIZ_TRUE;
            if (variable->averaging_enabled && variable->averaging_threshold_percent>=0.0 && counts[i]>1u)
            {
                const double reference=fmax(fmax(fabs(mins[i]),fabs(maxs[i])),1.0e-30);
                if (100.0*fabs(maxs[i]-mins[i])/reference>variable->averaging_threshold_percent) discontinuous=FVIZ_TRUE;
            }
            if (discontinuous) { flags[i]=1u; continue; }
            dst[i]=sums[i]/(double)counts[i];
        }
...
static FVizResult fviz_fea_primary_result_create(FVizFEAPrimaryVariableResult** out_result)
{
    ...
    *out_result=NULL;
    result=(FVizFEAPrimaryVariableResult*)fviz_internal_object_allocate(sizeof(*result),&g_fviz_fea_primary_result_class,NULL);
    if (result==NULL) return fviz_last_error_code();
    *out_result=result; return FVIZ_OK;
}

static FVizResult fviz_fea_primary_process_block(
    const FVizFEAField* field,FVizSize block_index,const FVizUnstructuredGrid* grid,
    const FVizFEAPrimaryVariable* variable,FVizFEAResultPosition source,FVizFEAResultPosition target,
    const FVizHashMap* point_map,const FVizHashMap* cell_map,const FVizHashMap* filter_set,
    FVizDataArray* raw_values,FVizDataArray* raw_entity,FVizDataArray* raw_local,
    FVizArray* point_contributions,FVizArray* cell_samples,FVizArray* element_node_contributions)
{
    ...
    const FVizSize point_count=fviz_unstructured_grid_point_count(grid),cell_count=fviz_unstructured_grid_cell_count(grid);
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"field block ids do not match evaluated tuple count"); goto fail;
    }
    if (source==FVIZ_FEA_POSITION_NODAL)
    {
        for (i=0u;i<tuple_count;++i)
        {
            FVizId label=(FVizId)i; FVizSize point_id; double value;
            ...
            { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"nodal result label does not exist in grid GlobalIds/internal ids"); goto fail; }
            if (fviz_data_array_get_component(scalar,i,0u,&value)!=FVIZ_OK ||
                fviz_fea_append_raw(raw_values,raw_entity,raw_local,value,label,FVIZ_FEA_INVALID_LOCAL)!=FVIZ_OK ||
                fviz_fea_append_contribution(point_contributions,point_id,(FVizSize)-1,block_index,label,FVIZ_FEA_INVALID_LOCAL,value)!=FVIZ_OK) goto fail;
        }
    }
    else if (source==FVIZ_FEA_POSITION_ELEMENT_NODAL)
    {
        FVizSize* occurrence=NULL; FVizSize bytes=0u;
        const FVizFEALocalIdBase base=fviz_fea_detect_local_base(local_ids,variable->local_id_base);
        if (cell_count!=0u)
        {
            if (fviz_size_multiply(cell_count,sizeof(FVizSize),&bytes)!=FVIZ_OK) goto fail;
            occurrence=(FVizSize*)fviz_alloc(bytes); if (occurrence==NULL) goto fail;
            (void)memset(occurrence,0,bytes);
        }
        for (i=0u;i<tuple_count;++i)
        {
            FVizId label; FVizSize cell_id,local_index; int64_t local_raw; FVizCellView view; double value;
            if (entity_ids==NULL)
            { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"element-nodal field blocks require entity_ids for mesh mapping"); fviz_free(occurrence); goto fail; }
            if (fviz_fea_read_id(entity_ids,i,&label)!=FVIZ_OK) { fviz_free(occurrence); goto fail; }
            if (!fviz_fea_filter_accepts(filter_set,label)) continue;
            if (!fviz_fea_resolve_label(label,cell_map,cell_count,&cell_id))
            { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"element-nodal element label does not exist in grid GlobalIds/internal ids"); fviz_free(occurrence); goto fail; }
            if (local_ids!=NULL)
            {
                if (fviz_fea_read_local_id(local_ids,i,&local_raw)!=FVIZ_OK) { fviz_free(occurrence); goto fail; }
                if (base==FVIZ_FEA_LOCAL_ID_ONE_BASED) --local_raw;
                if (local_raw<0) { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"element-nodal local id is negative after base conversion"); fviz_free(occurrence); goto fail; }
                local_index=(FVizSize)local_raw;
            }
            else local_index=occurrence[cell_id]++;
            if (fviz_cell_array_cell_view(fviz_unstructured_grid_cells((FVizUnstructuredGrid*)grid),cell_id,&view)!=FVIZ_OK || local_index>=view.point_count)
            { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"element-nodal local id exceeds cell node count"); fviz_free(occurrence); goto fail; }
            if (fviz_data_array_get_component(scalar,i,0u,&value)!=FVIZ_OK ||
                fviz_fea_append_raw(raw_values,raw_entity,raw_local,value,label,(int64_t)local_index)!=FVIZ_OK ||
                fviz_fea_append_contribution(point_contributions,(FVizSize)fviz_cell_view_point_id(&view,local_index),cell_id,block_index,label,(int64_t)local_index,value)!=FVIZ_OK ||
                fviz_fea_append_contribution(element_node_contributions,(FVizSize)fviz_cell_view_point_id(&view,local_index),cell_id,block_index,label,(int64_t)local_index,value)!=FVIZ_OK ||
                fviz_fea_append_contribution(cell_samples,(FVizSize)-1,cell_id,block_index,label,(int64_t)local_index,value)!=FVIZ_OK)
            { fviz_free(occurrence); goto fail; }
        }
    ...
    else if (source==FVIZ_FEA_POSITION_CENTROID || source==FVIZ_FEA_POSITION_WHOLE_ELEMENT)
    {
        for (i=0u;i<tuple_count;++i)
        {
            FVizId label=(FVizId)i; FVizSize cell_id,k; FVizCellView view; double value;
            if (entity_ids!=NULL && fviz_fea_read_id(entity_ids,i,&label)!=FVIZ_OK) goto fail;
            if (!fviz_fea_filter_accepts(filter_set,label)) continue;
            if (!fviz_fea_resolve_label(label,cell_map,cell_count,&cell_id))
            { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"element result label does not exist in grid GlobalIds/internal ids"); goto fail; }
            if (fviz_data_array_get_component(scalar,i,0u,&value)!=FVIZ_OK ||
                fviz_fea_append_raw(raw_values,raw_entity,raw_local,value,label,FVIZ_FEA_INVALID_LOCAL)!=FVIZ_OK ||
                fviz_fea_append_contribution(cell_samples,(FVizSize)-1,cell_id,block_index,label,FVIZ_FEA_INVALID_LOCAL,value)!=FVIZ_OK) goto fail;
            if (target==FVIZ_FEA_POSITION_NODAL || target==FVIZ_FEA_POSITION_ELEMENT_NODAL)
            {
                if (fviz_cell_array_cell_view(fviz_unstructured_grid_cells((FVizUnstructuredGrid*)grid),cell_id,&view)!=FVIZ_OK) goto fail;
                for (k=0u;k<view.point_count;++k)
                {
                    const FVizSize p=(FVizSize)fviz_cell_view_point_id(&view,k);
                    if (fviz_fea_append_contribution(point_contributions,p,cell_id,block_index,label,(int64_t)k,value)!=FVIZ_OK ||
                        fviz_fea_append_contribution(element_node_contributions,p,cell_id,block_index,label,(int64_t)k,value)!=FVIZ_OK) goto fail;
                }
            }
        }
    }
    else if (source==FVIZ_FEA_POSITION_INTEGRATION_POINT)
    {
        ...
        if (entity_ids==NULL)
        { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"integration-point field blocks require entity_ids for mesh mapping"); goto fail; }
        ...
        if (cell_count!=0u)
        {
            ...
            ip_counts=(FVizSize*)fviz_alloc(bytes); if (ip_counts==NULL) goto ip_fail; (void)memset(ip_counts,0,bytes);
        }
        for (i=0u;i<tuple_count;++i)
        {
            FVizId label; FVizSize cell_id; int64_t local=(int64_t)i; double value; FVizFEAIPTuple ip;
            if (fviz_fea_read_id(entity_ids,i,&label)!=FVIZ_OK) goto ip_fail;
            if (!fviz_fea_filter_accepts(filter_set,label)) continue;
            if (!fviz_fea_resolve_label(label,cell_map,cell_count,&cell_id))
            { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"integration-point element label does not exist in grid GlobalIds/internal ids"); goto ip_fail; }
            if (local_ids!=NULL && fviz_fea_read_local_id(local_ids,i,&local)!=FVIZ_OK) goto ip_fail;
            if (fviz_data_array_get_component(scalar,i,0u,&value)!=FVIZ_OK ||
                fviz_fea_append_raw(raw_values,raw_entity,raw_local,value,label,local)!=FVIZ_OK) goto ip_fail;
            ip.cell_id=cell_id; ip.order=local; ip.original_order=i; ip.entity_label=label; ip.value=value;
            if (fviz_array_push(ips,&ip)!=FVIZ_OK) goto ip_fail;
            ++ip_counts[cell_id]; ++total_ip;
            if (fviz_fea_append_contribution(cell_samples,(FVizSize)-1,cell_id,block_index,label,local,value)!=FVIZ_OK) goto ip_fail;
        }
        if (target==FVIZ_FEA_POSITION_CENTROID || target==FVIZ_FEA_POSITION_WHOLE_ELEMENT)
        { fviz_release(ips); ips=NULL; fviz_free(ip_counts); ip_counts=NULL; fviz_release(scalar); return FVIZ_OK; }
        if (fviz_size_multiply(cell_count+1u,sizeof(FVizSize),&bytes)!=FVIZ_OK) goto ip_fail;
        ip_offsets=(FVizSize*)fviz_alloc(bytes); if (ip_offsets==NULL) goto ip_fail;
        ip_offsets[0]=0u; for(i=0u;i<cell_count;++i) ip_offsets[i+1u]=ip_offsets[i]+ip_counts[i];
        if (fviz_array_count(ips)>1u) qsort(fviz_array_data(ips),(size_t)fviz_array_count(ips),sizeof(FVizFEAIPTuple),fviz_fea_ip_tuple_compare);
        if (fviz_data_array_create(FVIZ_DATA_FLOAT64,1u,&dense)!=FVIZ_OK || fviz_data_array_resize(dense,total_ip)!=FVIZ_OK) goto ip_fail;
            ...
            ((double*)fviz_data_array_data(dense))[i]=((const FVizFEAIPTuple*)fviz_array_const_at(ips,i))->value;
        {
            ...
            if (fviz_unstructured_grid_extrapolate_integration_point_data_element_nodal(
                    grid,dense,ip_offsets,NULL,&options,&element_nodal)!=FVIZ_OK) goto ip_fail;
        }
        {
            FVizSize cell,node,tuple=0u;
            for (cell=0u;cell<cell_count;++cell)
            {
                FVizCellView view; FVizId label=(FVizId)cell;
                if (fviz_cell_array_cell_view(fviz_unstructured_grid_cells((FVizUnstructuredGrid*)grid),cell,&view)!=FVIZ_OK) goto ip_fail;
                ...
                for (node=0u;node<view.point_count;++node,++tuple)
                {
                    ...
                    if (fviz_fea_append_contribution(point_contributions,(FVizSize)fviz_cell_view_point_id(&view,node),cell,block_index,label,(int64_t)node,value)!=FVIZ_OK ||
                        fviz_fea_append_contribution(element_node_contributions,(FVizSize)fviz_cell_view_point_id(&view,node),cell,block_index,label,(int64_t)node,value)!=FVIZ_OK) goto ip_fail;
                }
            }
        }
        fviz_release(ips); ips=NULL; fviz_free(ip_counts); ip_counts=NULL; fviz_free(ip_offsets); ip_offsets=NULL; fviz_release(dense); dense=NULL; fviz_release(element_nodal); element_nodal=NULL;
    }
    else
    {
        if (target!=source)
        { fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,"requested result-position conversion is not implemented for this source position"); goto fail; }
        for (i=0u;i<tuple_count;++i)
        {
            FVizId label=(FVizId)i; int64_t local=FVIZ_FEA_INVALID_LOCAL; double value;
            if (entity_ids!=NULL && fviz_fea_read_id(entity_ids,i,&label)!=FVIZ_OK) goto fail;
            if (!fviz_fea_filter_accepts(filter_set,label)) continue;
            if (local_ids!=NULL && fviz_fea_read_local_id(local_ids,i,&local)!=FVIZ_OK) goto fail;
            if (fviz_data_array_get_component(scalar,i,0u,&value)!=FVIZ_OK ||
                fviz_fea_append_raw(raw_values,raw_entity,raw_local,value,label,local)!=FVIZ_OK) goto fail;
        }
    }
    fviz_release(scalar);
    return FVIZ_OK;
ip_fail:
    fviz_release(ips); fviz_free(ip_counts); fviz_free(ip_offsets); fviz_release(dense); fviz_release(element_nodal);
fail:
    fviz_release(scalar);
    return fviz_last_error_code();
}

FVizResult fviz_fea_primary_variable_evaluate(
    FVizFEAPrimaryVariableEvaluator* evaluator,const FVizFEAField* field,const FVizUnstructuredGrid* grid,
    const FVizFEAPrimaryVariable* user_variable,FVizFEAPrimaryVariableResult** out_result)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"primary-variable evaluation requires evaluator, field, grid and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    *out_result=NULL;
    if (variable==NULL) { fviz_fea_primary_variable_initialize(&defaults); variable=&defaults; }
    else if (variable->struct_size<sizeof(FVizFEAPrimaryVariable))
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"primary-variable descriptor struct is too small"); return FVIZ_ERROR_INVALID_ARGUMENT; }
    if (variable->operation!=FVIZ_FEA_PRIMARY_COMPONENT && variable->operation!=FVIZ_FEA_PRIMARY_INVARIANT)
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"invalid primary-variable operation"); return FVIZ_ERROR_INVALID_ARGUMENT; }
    ...
    { fviz_internal_set_error(FVIZ_ERROR_NOT_FOUND,"no field block matches the requested instance/section/source position"); return FVIZ_ERROR_NOT_FOUND; }
    target=fviz_fea_choose_target_position(source,variable->target_position);
    point_labels=fviz_attribute_set_const_active(
        fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)grid),FVIZ_ATTRIBUTE_GLOBAL_IDS);
    cell_labels=fviz_attribute_set_const_active(
        fviz_unstructured_grid_cell_data((FVizUnstructuredGrid*)grid),FVIZ_ATTRIBUTE_GLOBAL_IDS);
    if (fviz_fea_build_label_map(point_labels,fviz_unstructured_grid_point_count(grid),&point_map)!=FVIZ_OK ||
        ...
        fviz_fea_create_raw_arrays(&result->raw_values,&result->raw_entity_ids,&result->raw_local_ids)!=FVIZ_OK) goto fail;
    result->source_position=source; result->target_position=target;
    for (i=0u;i<fviz_fea_field_block_count(field);++i)
    {
        if (!fviz_fea_block_basic_match(field,i,variable) || fviz_fea_field_block_position(field,i)!=source) continue;
        if (fviz_fea_primary_process_block(field,i,grid,variable,source,target,point_map,cell_map,filter_set,
                ...
                point_contributions,cell_samples,element_node_contributions)!=FVIZ_OK) goto fail;
        ++matched;
    }
    if (matched==0u)
    { fviz_internal_set_error(FVIZ_ERROR_NOT_FOUND,"no primary-variable blocks were selected"); goto fail; }
    if (target==FVIZ_FEA_POSITION_NODAL)
    {
        if (source==FVIZ_FEA_POSITION_NODAL || source==FVIZ_FEA_POSITION_ELEMENT_NODAL || source==FVIZ_FEA_POSITION_INTEGRATION_POINT ||
            source==FVIZ_FEA_POSITION_CENTROID || source==FVIZ_FEA_POSITION_WHOLE_ELEMENT)
        {
            if (fviz_fea_build_nodal_display(point_contributions,point_labels,fviz_unstructured_grid_point_count(grid),variable,
                    &result->display_values,&result->display_entity_ids,&result->discontinuity_mask)!=FVIZ_OK) goto fail;
            result->association=FVIZ_FEA_DISPLAY_ASSOCIATION_POINT;
        }
        else { fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,"source position cannot currently be converted to nodal display"); goto fail; }
    }
    else if (target==FVIZ_FEA_POSITION_ELEMENT_NODAL)
    {
        if (fviz_fea_build_element_nodal_display(element_node_contributions,&result->display_values,
                &result->display_entity_ids,&result->display_local_ids,&result->discontinuity_mask)!=FVIZ_OK) goto fail;
    ...
    {
        if (source==FVIZ_FEA_POSITION_NODAL)
        { fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,"nodal-to-centroid primary-variable reduction is not implemented yet"); goto fail; }
    ...
    else if (target==source)
    {
        if (fviz_data_array_deep_copy(result->raw_values,&result->display_values)!=FVIZ_OK ||
            ...
            fviz_data_array_resize(result->discontinuity_mask,fviz_data_array_tuple_count(result->display_values))!=FVIZ_OK) goto fail;
        (void)memset(fviz_data_array_data(result->discontinuity_mask),0,fviz_data_array_tuple_count(result->display_values));
        result->association=FVIZ_FEA_DISPLAY_ASSOCIATION_RAW;
    }
    else
    { fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,"requested target result position is not supported by the primary-variable engine"); goto fail; }
    if (fviz_fea_array_range(result->raw_values,&result->raw_range_valid,&result->raw_minimum,&result->raw_maximum)!=FVIZ_OK ||
        fviz_fea_array_range(result->display_values,&result->display_range_valid,&result->display_minimum,&result->display_maximum)!=FVIZ_OK) goto fail;
    if (fviz_fea_primary_cache_store(evaluator,field,grid,variable,result)!=FVIZ_OK) goto fail;
    *out_result=result;
    fviz_release(point_map);fviz_release(cell_map);fviz_release(filter_set);
    fviz_release(point_contributions);fviz_release(cell_samples);fviz_release(element_node_contributions);
    return FVIZ_OK;
fail:
    fviz_release(point_map);fviz_release(cell_map);fviz_release(filter_set);
    fviz_release(point_contributions);fviz_release(cell_samples);fviz_release(element_node_contributions);
...
FVizBool fviz_fea_primary_variable_result_raw_range(const FVizFEAPrimaryVariableResult* result,double* out_minimum,double* out_maximum)
{
...
FVizBool fviz_fea_primary_variable_result_display_range(const FVizFEAPrimaryVariableResult* result,double* out_minimum,double* out_maximum)
{
