...
static void fviz_fea_deformed_controller_clear_internal(
...
static const FVizObjectClass g_fviz_fea_deformed_result_class={
    FVIZ_TYPE_FEA_DEFORMED_SHAPE_RESULT,
    "FVizFEADeformedShapeResult",
    NULL,
    fviz_fea_deformed_result_destroy,
    fviz_fea_deformed_local_mtime
};

static const FVizObjectClass g_fviz_fea_deformed_controller_class={
    FVIZ_TYPE_FEA_DEFORMED_SHAPE_CONTROLLER,
    "FVizFEADeformedShapeController",
    NULL,
    fviz_fea_deformed_controller_destroy,
    fviz_fea_deformed_local_mtime
};
...
FVizFEADeformedShapeCacheStatistics fviz_fea_deformed_shape_controller_cache_statistics(
...
static FVizResult fviz_fea_deformed_build_point_label_map(
...
static FVizBool fviz_fea_deformed_options_equal(
    const FVizFEADeformedShapeController* controller,
...
static FVizResult fviz_fea_deformed_cache_store(
    FVizFEADeformedShapeController* controller,
    ...
    FVizFEADeformedShapeResult* result)
{
    ...
    {
        fviz_release(field_name); fviz_release(instance_name);
        return fviz_last_error_code();
    }
...
static FVizBool fviz_fea_deformed_cache_matches(
    const FVizFEADeformedShapeController* controller,
    const FVizFEAFrame* frame,
    const FVizUnstructuredGrid* grid,
    const FVizFEADeformedShapeOptions* options)
...
static FVizResult fviz_fea_deformed_map_displacements(
    const FVizFEAField* field,
    const FVizUnstructuredGrid* grid,
    const FVizFEADeformedShapeOptions* options,
    FVizDataArray** out_vectors,
    FVizDataArray** out_coverage,
    FVizSize* out_mapped)
{
    const FVizSize point_count=fviz_unstructured_grid_point_count(grid);
    double* vectors=NULL;
    ...
    *out_vectors=NULL; *out_coverage=NULL; *out_mapped=0u;
    if (fviz_fea_field_type(field)!=FVIZ_FEA_FIELD_VECTOR)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"deformed shape displacement field must be a vector field");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    if (point_count>0u)
    {
    ...
    for (block_index=0u;block_index<fviz_fea_field_block_count(field);++block_index)
    {
        ...
        if (options->instance_name!=NULL && options->instance_name[0]!='\0' &&
            strcmp(block_instance,options->instance_name)!=0) continue;
        ...
        {
            fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"nodal displacement block must contain at least three components");
            goto fail;
        }
        ...
        for (tuple=0u;tuple<tuple_count;++tuple)
        {
            ...
            if (fviz_data_array_get_component(values,tuple,0u,&x)!=FVIZ_OK ||
                ...
                fviz_data_array_get_component(values,tuple,2u,&z)!=FVIZ_OK) goto fail;
    ...
    if (fviz_data_array_create(FVIZ_DATA_FLOAT64,3u,&vector_array)!=FVIZ_OK ||
        ...
        goto fail;
    fviz_free(vectors); fviz_free(coverage); fviz_release(point_map);
...
fail:
    fviz_free(vectors); fviz_free(coverage); fviz_release(point_map);
    fviz_release(vector_array); fviz_release(coverage_array);
    return fviz_last_error_code()!=FVIZ_OK?fviz_last_error_code():FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_fea_deformed_shape_evaluate(
    FVizFEADeformedShapeController* controller,
    const FVizFEAFrame* frame,
    const FVizUnstructuredGrid* grid,
    const FVizFEADeformedShapeOptions* options,
    FVizFEADeformedShapeResult** out_result)
{
    ...
    if (controller==NULL || frame==NULL || grid==NULL || out_result==NULL)
    {
        ...
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"deformed-shape evaluation requires controller, frame, grid and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    *out_result=NULL;
    if (actual==NULL)
    ...
    if (actual->struct_size<sizeof(FVizFEADeformedShapeOptions) ||
        !isfinite(actual->uniform_scale) || !isfinite(actual->auto_target_fraction) ||
        !isfinite(actual->auto_minimum_scale) || !isfinite(actual->auto_maximum_scale))
    {
    ...
    ++controller->misses;
    result=(FVizFEADeformedShapeResult*)fviz_internal_object_allocate(
        sizeof(*result),&g_fviz_fea_deformed_result_class,NULL);
    ...
    if (actual->state!=FVIZ_FEA_DEFORMATION_UNDEFORMED &&
        actual->state!=FVIZ_FEA_DEFORMATION_DEFORMED &&
    ...
    else
    {
        const char* field_name=actual->displacement_field_name!=NULL && actual->displacement_field_name[0]!='\0'
            ? actual->displacement_field_name : "U";
        ...
        else if (actual->scale_mode==FVIZ_FEA_DEFORMATION_SCALE_UNIFORM) scale=actual->uniform_scale;
        else if (actual->scale_mode==FVIZ_FEA_DEFORMATION_SCALE_AUTO)
        {
            if (fviz_deformation_compute_auto_scale(
                    fviz_unstructured_grid_bounds(grid),vectors,
                    actual->auto_target_fraction,actual->auto_minimum_scale,actual->auto_maximum_scale,
                    &scale,&result->metrics)!=FVIZ_OK)
            {
                fviz_release(vectors); fviz_release(coverage); fviz_release(result);
                return fviz_last_error_code();
            }
        }
        else
