...
static const FVizObjectClass g_fviz_fea_field_class = {FVIZ_TYPE_FEA_FIELD, "FVizFEAField", NULL,
                                                       fviz_fea_field_destroy, fviz_fea_field_mtime};
...
static FVizBool fviz_fea_field_child_modified(FVizObject* caller, FVizEventId event_id, void* call_data,
                                              void* client_data)
{
...
static FVizBool fviz_fea_field_component_layout_valid(FVizFEAFieldType type, uint32_t components)
{
    switch (type)
    {
        case FVIZ_FEA_FIELD_SCALAR:
            return components == 1u ? FVIZ_TRUE : FVIZ_FALSE;
        case FVIZ_FEA_FIELD_VECTOR:
            return components >= 1u ? FVIZ_TRUE : FVIZ_FALSE;
        case FVIZ_FEA_FIELD_TENSOR_3D_SYMMETRIC:
            return components == 6u ? FVIZ_TRUE : FVIZ_FALSE;
        case FVIZ_FEA_FIELD_TENSOR_2D_SYMMETRIC:
            return components == 4u ? FVIZ_TRUE : FVIZ_FALSE;
        case FVIZ_FEA_FIELD_TENSOR_3D_FULL:
            return components == 9u ? FVIZ_TRUE : FVIZ_FALSE;
        default:
            return FVIZ_FALSE;
    }
}
...
static FVizFEAInvariantMask fviz_fea_field_default_invariants(FVizFEAFieldType type)
{
    const FVizFEAInvariantMask tensor =
        FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_MISES) | FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_TRESCA) |
        FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_PRESSURE) | FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_MAX_PRINCIPAL) |
        FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_MID_PRINCIPAL) |
        FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_MIN_PRINCIPAL);
    if (type == FVIZ_FEA_FIELD_VECTOR) return FVIZ_FEA_INVARIANT_BIT(FVIZ_FEA_INVARIANT_MAGNITUDE);
    if (type == FVIZ_FEA_FIELD_TENSOR_3D_SYMMETRIC || type == FVIZ_FEA_FIELD_TENSOR_2D_SYMMETRIC ||
...
static FVizResult fviz_fea_field_block_observe(FVizFEAField* field, FVizDataArray* array, FVizObserverTag* out_tag)
{
    if (out_tag != NULL) *out_tag = FVIZ_OBSERVER_TAG_INVALID;
    if (array == NULL) return FVIZ_OK;
    return fviz_object_add_observer((FVizObject*)array, FVIZ_EVENT_MODIFIED, 0.0f, fviz_fea_field_child_modified, field,
                                    out_tag);
}
...
FVizResult fviz_fea_field_create(const char* name, const char* description, FVizFEAFieldType field_type,
                                 FVizFEAField** out_field)
{
    FVizFEAField* field;
    if (out_field == NULL || name == NULL || name[0] == '\0' || field_type < FVIZ_FEA_FIELD_SCALAR ||
        field_type > FVIZ_FEA_FIELD_TENSOR_3D_FULL)
    {
...
FVizResult fviz_fea_field_set_component_labels(FVizFEAField* field, const char* const* labels, FVizSize label_count)
{
    ...
    if (label_count != 0u && !fviz_fea_field_component_layout_valid(field->field_type, (uint32_t)label_count))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "FEA component label count is incompatible with field type");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    ...
    for (i = 0u; i < label_count; ++i)
    {
        ...
        if (labels[i] == NULL || labels[i][0] == '\0' || fviz_string_create_from(labels[i], &text) != FVIZ_OK ||
            fviz_array_push(replacement, &text) != FVIZ_OK)
        {
            ...
            return labels[i] == NULL || labels[i][0] == '\0' ? FVIZ_ERROR_INVALID_ARGUMENT : fviz_last_error_code();
        }
    }
    for (i = 0u; i < fviz_array_count(field->component_labels); ++i)
...
FVizResult fviz_fea_field_find_component(const FVizFEAField* field, const char* label, FVizSize* out_component)
{
    FVizSize i;
...
FVizResult fviz_fea_field_add_block(FVizFEAField* field, const FVizFEAFieldBlockDescriptor* descriptor,
                                    FVizSize* out_block_index)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "FEA field block value component count is incompatible with field type");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "FEA field block component count differs from component labels");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    tuple_count = fviz_data_array_tuple_count(descriptor->values);
    if (descriptor->entity_ids != NULL && (!fviz_fea_data_array_is_integer_ids(descriptor->entity_ids) ||
                                           fviz_data_array_tuple_count(descriptor->entity_ids) != tuple_count))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "FEA entity ids must be one-component integer tuples matching values");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    if (descriptor->local_ids != NULL && (!fviz_fea_data_array_is_integer_ids(descriptor->local_ids) ||
                                          fviz_data_array_tuple_count(descriptor->local_ids) != tuple_count))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "FEA local ids must be one-component integer tuples matching values");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    (void)memset(&block, 0, sizeof(block));
...
static const FVizFEAFieldBlock* fviz_fea_field_const_block(const FVizFEAField* field, FVizSize block_index)
{
    return field != NULL && field->blocks != NULL && block_index < fviz_array_count(field->blocks)
               ? (const FVizFEAFieldBlock*)fviz_array_const_at(field->blocks, block_index)
               : NULL;
}
...
FVizDataArray* fviz_fea_field_block_values(FVizFEAField* field, FVizSize block_index)
{
    FVizFEAFieldBlock* block = field != NULL && field->blocks != NULL && block_index < fviz_array_count(field->blocks)
                                   ? (FVizFEAFieldBlock*)fviz_array_at(field->blocks, block_index)
                                   : NULL;
...
const char* fviz_fea_result_position_name(FVizFEAResultPosition position)
{
    switch (position)
    {
        case FVIZ_FEA_POSITION_NODAL:
            return "NODAL";
        case FVIZ_FEA_POSITION_ELEMENT_NODAL:
            return "ELEMENT_NODAL";
        case FVIZ_FEA_POSITION_INTEGRATION_POINT:
            return "INTEGRATION_POINT";
        case FVIZ_FEA_POSITION_CENTROID:
            return "CENTROID";
        case FVIZ_FEA_POSITION_ELEMENT_FACE:
            return "ELEMENT_FACE";
        case FVIZ_FEA_POSITION_WHOLE_ELEMENT:
            return "WHOLE_ELEMENT";
        case FVIZ_FEA_POSITION_WHOLE_REGION:
            return "WHOLE_REGION";
        default:
            return "UNKNOWN";
    }
...
const char* fviz_fea_invariant_name(FVizFEAInvariant invariant)
{
    switch (invariant)
    {
        case FVIZ_FEA_INVARIANT_MAGNITUDE:
            return "Magnitude";
        case FVIZ_FEA_INVARIANT_MISES:
            return "Mises";
        case FVIZ_FEA_INVARIANT_TRESCA:
            return "Tresca";
        case FVIZ_FEA_INVARIANT_PRESSURE:
            return "Pressure";
        case FVIZ_FEA_INVARIANT_MAX_PRINCIPAL:
            return "Max. Principal";
        case FVIZ_FEA_INVARIANT_MID_PRINCIPAL:
            return "Mid. Principal";
        case FVIZ_FEA_INVARIANT_MIN_PRINCIPAL:
...
static void fviz_fea_tensor_symmetric_components(const FVizFEAField* field, const FVizDataArray* values, FVizSize tuple,
                                                 double* s11, double* s22, double* s33, double* s12, double* s13,
                                                 double* s23)
{
...
FVizResult fviz_fea_field_evaluate_component(const FVizFEAField* field, FVizSize block_index, FVizSize component,
...
FVizResult fviz_fea_field_evaluate_invariant(const FVizFEAField* field, FVizSize block_index,
                                             FVizFEAInvariant invariant, FVizDataArray** out_values)
{
    ...
    *out_values = NULL;
    ...
    if (invariant != FVIZ_FEA_INVARIANT_MAGNITUDE && field->field_type != FVIZ_FEA_FIELD_TENSOR_3D_SYMMETRIC &&
        field->field_type != FVIZ_FEA_FIELD_TENSOR_2D_SYMMETRIC && field->field_type != FVIZ_FEA_FIELD_TENSOR_3D_FULL)
    {
    ...
    for (i = 0u; i < tuple_count; ++i)
    {
        ...
        else
        {
            ...
            {
                const double normal = (s11 - s22) * (s11 - s22) + (s22 - s33) * (s22 - s33) + (s33 - s11) * (s33 - s11);
            ...
            {
                case FVIZ_FEA_INVARIANT_TRESCA:
                    destination[i] = principal[0] - principal[2];
                    break;
                case FVIZ_FEA_INVARIANT_MAX_PRINCIPAL:
                    destination[i] = principal[0];
                    break;
                case FVIZ_FEA_INVARIANT_MID_PRINCIPAL:
                    destination[i] = principal[1];
                    break;
                case FVIZ_FEA_INVARIANT_MIN_PRINCIPAL:
                    destination[i] = principal[2];
                    break;
                default:
                    destination[i] = NAN;
                    break;
