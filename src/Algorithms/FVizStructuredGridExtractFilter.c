...
static const FVizObjectClass g_fviz_structured_extract_class = {
    FVIZ_TYPE_STRUCTURED_GRID_EXTRACT_FILTER, "FVizStructuredGridExtractFilter",
    &g_fviz_object_class, fviz_structured_extract_destroy, NULL
};
...
static FVizResult fviz_structured_extract_map_request(
    FVizAlgorithm* algorithm, uint32_t input_port, uint32_t connection,
    const FVizPipelineRequestInfo* downstream, FVizPipelineRequestInfo* upstream, void* state)
{
    uint32_t axis;
    (void)algorithm; (void)connection; (void)state;
    ...
    {
        upstream->extent[axis * 2u] = fviz_structured_extract_saturating_sub(
            downstream->extent[axis * 2u], downstream->ghost_levels);
        upstream->extent[axis * 2u + 1u] = fviz_structured_extract_saturating_add(
            downstream->extent[axis * 2u + 1u], downstream->ghost_levels);
    }
    upstream->has_extent = FVIZ_TRUE;
    return FVIZ_OK;
}

static FVizResult fviz_structured_extract_copy_field_attributes(
...
static FVizResult fviz_structured_extract_copy_indexed_attributes(
    const FVizAttributeSet* source, FVizAttributeSet* destination,
    const FVizId* source_ids, FVizSize output_count, FVizSize expected_source_count)
{
...
static FVizBool fviz_structured_extract_intersect_extent(
    const int64_t requested[6], const int64_t available[6], int64_t out_extent[6])
{
    uint32_t axis;
    for (axis = 0u; axis < 3u; ++axis)
    {
        const int64_t minimum = requested[axis * 2u] > available[axis * 2u]
            ? requested[axis * 2u] : available[axis * 2u];
        const int64_t maximum = requested[axis * 2u + 1u] < available[axis * 2u + 1u]
            ? requested[axis * 2u + 1u] : available[axis * 2u + 1u];
...
static FVizResult fviz_structured_extract_process_request(
    FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request, void* state)
{
    ...
    if (request->type == FVIZ_PIPELINE_REQUEST_INFORMATION)
    {
        if (input != NULL)
        {
            ...
            if (fviz_structured_grid_point_count(input) != 0u)
                (void)fviz_algorithm_set_output_whole_extent(
                    algorithm, request->requested_output_port, input_extent);
        }
        return FVIZ_OK;
    }
    ...
    if (request->has_extent != FVIZ_FALSE)
    {
        ...
        if ((request->flags & FVIZ_PIPELINE_REQUEST_FLAG_EXACT_EXTENT) != 0u)
            for (axis = 0u; axis < 3u; ++axis)
                ...
                {
                    fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "exact structured extent lies outside available input");
    ...
    else (void)memcpy(output_extent, input_extent, sizeof(output_extent));
    ...
    {
        FVizSize bytes;
        if (fviz_size_multiply(point_count, sizeof(FVizVec3), &bytes) != FVIZ_OK) { result = FVIZ_ERROR_OVERFLOW; goto done; }
        points = (FVizVec3*)fviz_alloc(bytes);
        if (fviz_size_multiply(point_count, sizeof(FVizId), &bytes) != FVIZ_OK) { result = FVIZ_ERROR_OVERFLOW; goto done; }
        point_source_ids = (FVizId*)fviz_alloc(bytes);
        if (points == NULL || point_source_ids == NULL) { result = fviz_last_error_code(); goto done; }
    ...
    {
        FVizSize bytes;
        if (fviz_size_multiply(cell_count, sizeof(FVizId), &bytes) != FVIZ_OK) { result = FVIZ_ERROR_OVERFLOW; goto done; }
        cell_source_ids = (FVizId*)fviz_alloc(bytes);
        if (cell_source_ids == NULL) { result = fviz_last_error_code(); goto done; }
        for (k = output_extent[4]; k < (output_extent[5] > output_extent[4] ? output_extent[5] : output_extent[4] + 1); ++k)
            for (j = output_extent[2]; j < (output_extent[3] > output_extent[2] ? output_extent[3] : output_extent[2] + 1); ++j)
                for (i = output_extent[0]; i < (output_extent[1] > output_extent[0] ? output_extent[1] : output_extent[0] + 1); ++i)
                {
                    FVizId source_id;
    ...
    if (fviz_structured_extract_copy_indexed_attributes(
            fviz_structured_grid_const_point_data(input), fviz_structured_grid_point_data(output),
            point_source_ids, point_count, fviz_structured_grid_point_count(input)) != FVIZ_OK ||
        fviz_structured_extract_copy_indexed_attributes(
            fviz_structured_grid_const_cell_data(input), fviz_structured_grid_cell_data(output),
            cell_source_ids, cell_count, fviz_structured_grid_cell_count(input)) != FVIZ_OK ||
        fviz_structured_extract_copy_field_attributes(
            fviz_structured_grid_const_field_data(input), fviz_structured_grid_field_data(output)) != FVIZ_OK ||
    ...
    fviz_free(cell_source_ids); fviz_free(point_source_ids); fviz_free(points); fviz_release(output);
    return result;
}

FVizResult fviz_structured_grid_extract_filter_create(FVizStructuredGridExtractFilter** out_filter)
{
    FVizStructuredGridExtractFilter* filter; FVizAlgorithmCallbacks callbacks;
    ...
    *out_filter = NULL;
    filter=(FVizStructuredGridExtractFilter*)fviz_internal_object_allocate(sizeof(*filter),&g_fviz_structured_extract_class,NULL);
    if(filter==NULL) return fviz_last_error_code();
    fviz_algorithm_callbacks_initialize(&callbacks); callbacks.process_request=fviz_structured_extract_process_request;
    callbacks.get_state_mtime=fviz_structured_extract_state_mtime; callbacks.state_object=(FVizObject*)filter;
    ...
    if(fviz_algorithm_create(1u,1u,&callbacks,filter,&filter->algorithm)!=FVIZ_OK ||
       fviz_algorithm_configure_input_port(filter->algorithm,0u,FVIZ_TYPE_STRUCTURED_GRID,FVIZ_FALSE,FVIZ_FALSE)!=FVIZ_OK ||
...
FVizResult fviz_structured_grid_extract_filter_set_input_data(FVizStructuredGridExtractFilter* f,FVizStructuredGrid* in)
{
    int64_t extent[6]; FVizResult r;
    if(f==NULL||in==NULL) return FVIZ_ERROR_INVALID_ARGUMENT;
    r=fviz_algorithm_set_input_data(f->algorithm,0u,(FVizDataObject*)in);
    if(r==FVIZ_OK && fviz_structured_grid_point_count(in)!=0u){fviz_structured_grid_extent(in,extent);(void)fviz_algorithm_set_output_whole_extent(f->algorithm,0u,extent);} return r;
}
FVizResult fviz_structured_grid_extract_filter_set_input_connection(FVizStructuredGridExtractFilter* f,FVizAlgorithmOutput* in)
{return f&&in?fviz_algorithm_set_input_connection(f->algorithm,0u,in):FVIZ_ERROR_INVALID_ARGUMENT;}
FVizAlgorithm* fviz_structured_grid_extract_filter_algorithm(FVizStructuredGridExtractFilter* f){return f?f->algorithm:NULL;}
FVizAlgorithmOutput* fviz_structured_grid_extract_filter_output_port(FVizStructuredGridExtractFilter* f){return f?fviz_algorithm_output_port(f->algorithm,0u):NULL;}
FVizStructuredGrid* fviz_structured_grid_extract_filter_output(FVizStructuredGridExtractFilter* f){return f?(FVizStructuredGrid*)fviz_algorithm_output_data(f->algorithm,0u):NULL;}
FVizResult fviz_structured_grid_extract_filter_update(FVizStructuredGridExtractFilter* f){return f?fviz_algorithm_update(f->algorithm):FVIZ_ERROR_INVALID_ARGUMENT;}
FVizResult fviz_structured_grid_extract_filter_update_extent(FVizStructuredGridExtractFilter* f,const int64_t e[6],uint32_t g)
{return f&&e?fviz_executive_update_extent(fviz_algorithm_executive(f->algorithm),0u,e,g):FVIZ_ERROR_INVALID_ARGUMENT;}
