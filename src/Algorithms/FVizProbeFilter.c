...
static const FVizObjectClass g_fviz_probe_filter_class = {FVIZ_TYPE_PROBE_FILTER, "FVizProbeFilter",
                                                          &g_fviz_object_class, fviz_probe_filter_destroy, NULL};
...
static FVizResult fviz_probe_location_weights(const FVizUnstructuredGrid* source, const FVizLocatedCell* location,
                                              FVizId out_ids[20], double weights[20])
{
    ...
    for (i = 0u; i < view.point_count; ++i)
        out_ids[i] = fviz_cell_view_point_id(&view, i);
    for (i = 0u; i < 20u; ++i)
        weights[i] = 0.0;
    {
...
static FVizResult fviz_probe_interpolate_tuple(const FVizDataArray* source, const FVizId* ids, const double* weights,
                                               FVizSize point_count, unsigned char* scratch)
{
...
static FVizResult fviz_probe_filter_process_request(FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request,
                                                    void* state)
{
    FVizPolyData* input;
    ...
    if (input == NULL || source == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE,
                                "probe filter requires both PolyData input and UnstructuredGrid source");
    ...
    if (fviz_poly_data_deep_copy(input, &output) != FVIZ_OK || fviz_point_locator_create(&locator) != FVIZ_OK ||
    ...
    for (i = 0u; i < point_count; ++i)
    {
        ...
        for (array_index = 0u; array_index < array_count; ++array_index)
        {
            ...
            if (valid != 0u)
            {
                if (fviz_probe_interpolate_tuple(arrays[array_index].source, ids, weights, location.point_count,
                                                 scratch) != FVIZ_OK ||
    ...
    *out_filter = NULL;
    ...
    if (fviz_algorithm_create(2u, 1u, &callbacks, filter, &filter->algorithm) != FVIZ_OK ||
        fviz_algorithm_configure_input_port(filter->algorithm, 0u, FVIZ_TYPE_POLY_DATA, FVIZ_FALSE, FVIZ_FALSE) !=
            FVIZ_OK ||
        fviz_algorithm_configure_input_port(filter->algorithm, 1u, FVIZ_TYPE_UNSTRUCTURED_GRID, FVIZ_FALSE,
                                            FVIZ_FALSE) != FVIZ_OK ||
...
{
    return filter != NULL ? fviz_algorithm_set_input_data(filter->algorithm, 1u, (FVizDataObject*)source)
                          : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_probe_filter_set_source_connection(FVizProbeFilter* filter, FVizAlgorithmOutput* source)
{
    return filter != NULL ? fviz_algorithm_set_input_connection(filter->algorithm, 1u, source)
                          : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizAlgorithm* fviz_probe_filter_algorithm(FVizProbeFilter* filter)
{
    return filter != NULL ? filter->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_probe_filter_output_port(FVizProbeFilter* filter)
{
    return filter != NULL ? fviz_algorithm_output_port(filter->algorithm, 0u) : NULL;
}

FVizPolyData* fviz_probe_filter_output(FVizProbeFilter* filter)
{
    return filter != NULL ? (FVizPolyData*)fviz_algorithm_output_data(filter->algorithm, 0u) : NULL;
}

FVizResult fviz_probe_filter_update(FVizProbeFilter* filter)
{
