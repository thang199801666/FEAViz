...
static const FVizObjectClass g_fviz_rectilinear_extract_class = {
    FVIZ_TYPE_RECTILINEAR_GRID_EXTRACT_FILTER,
    "FVizRectilinearGridExtractFilter",
    &g_fviz_object_class,
    fviz_rectilinear_extract_destroy,
    NULL
};
...
static FVizResult fviz_rectilinear_extract_map_request(
    FVizAlgorithm* algorithm,
    uint32_t input_port,
    uint32_t connection,
    ...
    void* state)
{
    ...
    {
        upstream->extent[axis * 2u] = fviz_rectilinear_extract_saturating_sub(
            downstream->extent[axis * 2u], downstream->ghost_levels);
        upstream->extent[axis * 2u + 1u] = fviz_rectilinear_extract_saturating_add(
...
static FVizBool fviz_rectilinear_extract_intersect_extent(
...
static FVizResult fviz_rectilinear_extract_copy_field_attributes(
...
static FVizResult fviz_rectilinear_extract_copy_indexed_attributes(
...
static FVizResult fviz_rectilinear_extract_copy_axis(
    const FVizRectilinearGrid* input,
    FVizRectilinearGrid* output,
    uint32_t axis,
    const int64_t input_extent[6],
    const int64_t output_extent[6])
{
    ...
    result = fviz_data_array_create(
        fviz_data_array_type(input_coordinates), 1u, &output_coordinates);
    if (result == FVIZ_OK) result = fviz_data_array_resize(output_coordinates, count);
    if (result == FVIZ_OK && count != 0u)
        result = fviz_data_array_set_tuples(output_coordinates, 0u, source, count);
    if (result == FVIZ_OK)
        result = fviz_rectilinear_grid_set_coordinates(output, axis, output_coordinates);
    fviz_release(output_coordinates);
    return result;
}

static FVizResult fviz_rectilinear_extract_process_request(
    FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request, void* state)
{
    ...
    {
        if (input == NULL)
            fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "rectilinear extract filter has no input");
    ...
    if (request->has_extent != FVIZ_FALSE)
    {
        ...
        if (fviz_rectilinear_extract_intersect_extent(
                request->extent, input_extent, output_extent) == FVIZ_FALSE)
        {
    ...
    if (fviz_rectilinear_extract_copy_indexed_attributes(
            fviz_rectilinear_grid_const_point_data(input),
            fviz_rectilinear_grid_point_data(output),
            point_source_ids, point_count, fviz_rectilinear_grid_point_count(input)) != FVIZ_OK ||
        fviz_rectilinear_extract_copy_indexed_attributes(
            fviz_rectilinear_grid_const_cell_data(input),
            fviz_rectilinear_grid_cell_data(output),
            cell_source_ids, cell_count, fviz_rectilinear_grid_cell_count(input)) != FVIZ_OK ||
        fviz_rectilinear_extract_copy_field_attributes(
            fviz_rectilinear_grid_const_field_data(input),
...
FVizResult fviz_rectilinear_grid_extract_filter_create(
    FVizRectilinearGridExtractFilter** out_filter)
{
    ...
    *out_filter = NULL;
    filter = (FVizRectilinearGridExtractFilter*)fviz_internal_object_allocate(
        sizeof(*filter), &g_fviz_rectilinear_extract_class, NULL);
    ...
    if (fviz_algorithm_create(1u, 1u, &callbacks, filter, &filter->algorithm) != FVIZ_OK ||
        ...
        fviz_algorithm_configure_output_port(
            filter->algorithm, 0u, FVIZ_TYPE_RECTILINEAR_GRID) != FVIZ_OK)
    {
        fviz_release(filter);
        return fviz_last_error_code();
    }
...
FVizResult fviz_rectilinear_grid_extract_filter_set_input_data(
...
FVizResult fviz_rectilinear_grid_extract_filter_set_input_connection(
...
{
    return filter != NULL
        ? (FVizRectilinearGrid*)fviz_algorithm_output_data(filter->algorithm, 0u)
        : NULL;
}
...
FVizResult fviz_rectilinear_grid_extract_filter_update_extent(
    FVizRectilinearGridExtractFilter* filter, const int64_t extent[6], uint32_t ghost_levels)
{
    return filter != NULL && extent != NULL
        ? fviz_executive_update_extent(
            fviz_algorithm_executive(filter->algorithm), 0u, extent, ghost_levels)
        : FVIZ_ERROR_INVALID_ARGUMENT;
}
