...
static FVizResult fviz_deformation_validate_vectors(const FVizDataArray* vectors, FVizSize expected_count)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "deformation vector tuple count must match point count");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
...
static void fviz_deformation_apply_range(FVizSize begin, FVizSize end, void* user_data)
{
    ...
    for (i = begin; i < end; ++i)
    {
        double x = 0.0, y = 0.0, z = 0.0;
        fviz_deformation_read3(&context->view, i, &x, &y, &z);
        context->destination[i] = fviz_vec3(context->source[i].x + (float)(context->scale * x),
...
FVizResult fviz_deformation_measure_vectors(const FVizDataArray* vectors, FVizDeformationMetrics* out_metrics)
{
    ...
    if (fviz_deformation_validate_vectors(vectors, (FVizSize)-1) != FVIZ_OK) return fviz_last_error_code();
...
FVizResult fviz_deformation_compute_auto_scale(FVizBounds model_bounds, const FVizDataArray* vectors,
                                               double target_fraction, double minimum_scale, double maximum_scale,
                                               double* out_scale, FVizDeformationMetrics* out_metrics)
{
    FVizDeformationMetrics metrics;
    double dx, dy, dz, diagonal, scale;
    if (out_scale == NULL || !isfinite(target_fraction) || target_fraction <= 0.0 || !isfinite(minimum_scale) ||
        !isfinite(maximum_scale) || minimum_scale < 0.0 || maximum_scale < minimum_scale)
    {
...
FVizResult fviz_deformation_apply_to_points(const FVizPoints* points, const FVizDataArray* vectors, double scale,
                                            FVizPoints** out_points)
{
    ...
    if (points == NULL || out_points == NULL || !isfinite(scale))
    {
        ...
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "point deformation requires points, finite scale and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    *out_points = NULL;
...
FVizResult fviz_deformation_apply_to_poly_data(const FVizPolyData* poly_data, const FVizDataArray* vectors,
                                               double scale, FVizPolyData** out_poly_data)
{
    ...
    if (poly_data == NULL || out_poly_data == NULL || !isfinite(scale))
    {
        ...
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "poly-data deformation requires input, finite scale and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    *out_poly_data = NULL;
    if (fviz_deformation_validate_vectors(vectors, count) != FVIZ_OK) return fviz_last_error_code();
...
FVizResult fviz_deformation_apply_to_unstructured_grid(const FVizUnstructuredGrid* grid, const FVizDataArray* vectors,
                                                       double scale, FVizUnstructuredGrid** out_grid)
{
    return fviz_unstructured_grid_warp_by_array(grid, vectors, scale, out_grid);
}

static FVizResult fviz_deformation_build_point_buffer(const FVizVec3* source, FVizSize count,
                                                      const FVizDataArray* vectors, double scale, FVizVec3** out_buffer)
{
    FVizVec3* buffer = NULL;
    FVizDeformPointsContext context;
    if (out_buffer == NULL || (count != 0u && source == NULL) || !isfinite(scale)) return FVIZ_ERROR_INVALID_ARGUMENT;
    *out_buffer = NULL;
    if (fviz_deformation_validate_vectors(vectors, count) != FVIZ_OK) return fviz_last_error_code();
    if (count == 0u) return FVIZ_OK;
...
FVizResult fviz_deformation_update_points(FVizPoints* destination, const FVizPoints* base_points,
                                          const FVizDataArray* vectors, double scale)
{
    FVizVec3* buffer = NULL;
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "deformation point update requires matching source/destination point counts");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
...
FVizResult fviz_deformation_update_poly_data_points(FVizPolyData* destination, const FVizPolyData* base_poly_data,
                                                    const FVizDataArray* vectors, double scale)
{
    FVizVec3* buffer = NULL;
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "deformation PolyData update requires matching source/destination point counts");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    if (fviz_deformation_build_point_buffer(fviz_poly_data_points(base_poly_data), count, vectors, scale, &buffer) !=
        FVIZ_OK)
        return fviz_last_error_code();
...
FVizResult fviz_deformation_update_unstructured_grid_points(FVizUnstructuredGrid* destination,
                                                            ...
                                                            const FVizDataArray* vectors, double scale)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "deformation grid update requires matching source/destination point counts");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
