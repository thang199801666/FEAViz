...
static const FVizObjectClass g_fviz_resample_class = {FVIZ_TYPE_RESAMPLE_WITH_DATA_SET, "FVizResampleWithDataSet",
                                                      &g_fviz_object_class, fviz_resample_destroy, NULL};
...
static FVizBool fviz_image_stencil(const FVizImageData* image, const FVizVec3 point, FVizImageStencil* out)
{
    ...
    for (axis = 0u; axis < 3u; ++axis)
    {
        ...
        if (dims[axis] <= 1u)
        {
            lower[axis] = extent[axis * 2u];
            t[axis] = 0.0;
        }
        else if (index[axis] >= maximum)
        {
            lower[axis] = extent[axis * 2u + 1u] - 1;
            t[axis] = 1.0;
            active[active_count++] = axis;
        }
        else
        {
            const double floor_value = floor(index[axis]);
            lower[axis] = (int64_t)floor_value;
            t[axis] = index[axis] - floor_value;
            active[active_count++] = axis;
        }
    }
    out->count = 0u;
...
static FVizResult fviz_resample_image(FVizAlgorithm* algorithm, FVizPolyData* input, FVizImageData* source,
                                      FVizPolyData** out_output)
{
    ...
    if (fviz_poly_data_deep_copy(input, &output) != FVIZ_OK ||
        ...
        fviz_data_array_resize(valid_mask, point_count) != FVIZ_OK)
        goto fail;
    ...
    for (i = 0u; i < candidate_count; ++i)
    {
        ...
        if (source_array == NULL || name == NULL || fviz_data_array_tuple_count(source_array) != source_points)
            continue;
        if (fviz_data_array_create(fviz_data_array_type(source_array), fviz_data_array_components(source_array),
                                   &destination) != FVIZ_OK ||
            fviz_data_array_resize(destination, point_count) != FVIZ_OK)
        {
    ...
    }
    {
        ...
        for (i = 0u; i < point_count; ++i)
        {
            ...
            FVizSize a;
            if (fviz_image_stencil(source, fviz_poly_data_points(input)[i], &stencil) == FVIZ_FALSE)
            {
                mask[i] = 0u;
                continue;
            }
            ...
            for (a = 0u; a < array_count; ++a)
            {
                ...
                unsigned char* tuple =
                    (unsigned char*)fviz_data_array_data(dst) + i * fviz_data_array_tuple_stride(dst);
                uint32_t component;
                for (component = 0u; component < sample_array->components; ++component)
                {
                    ...
                    fviz_resample_write_value(tuple + (FVizSize)component * sample_array->source_type_size,
                                              sample_array->source_type, value);
                }
            }
            if ((i & 4095u) == 4095u)
            {
                ...
                {
                    fviz_internal_set_error(FVIZ_ERROR_CANCELLED, "resample filter was aborted");
                    goto fail;
                }
                if (point_count != 0u)
    ...
    for (i = 0u; i < array_count; ++i)
        fviz_release(arrays[i].destination);
    fviz_free(arrays);
...
static FVizResult fviz_resample_process(FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request, void* state)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "resample filter requires sampling geometry and source data");
        return FVIZ_ERROR_INVALID_STATE;
    }
    if (fviz_object_is_type((FVizObject*)source, FVIZ_TYPE_IMAGE_DATA) != FVIZ_FALSE)
    {
        if (fviz_resample_image(algorithm, input, (FVizImageData*)source, &output) != FVIZ_OK)
            return fviz_last_error_code();
    }
    else if (fviz_object_is_type((FVizObject*)source, FVIZ_TYPE_UNSTRUCTURED_GRID) != FVIZ_FALSE)
    {
        FVizProbeFilter* probe = NULL;
        if (fviz_probe_filter_create(&probe) != FVIZ_OK || fviz_probe_filter_set_input_data(probe, input) != FVIZ_OK ||
    ...
    *out_filter = NULL;
    ...
    if (fviz_algorithm_create(2u, 1u, &callbacks, filter, &filter->algorithm) != FVIZ_OK ||
        fviz_algorithm_configure_input_port(filter->algorithm, 0u, FVIZ_TYPE_POLY_DATA, FVIZ_FALSE, FVIZ_FALSE) !=
            FVIZ_OK ||
        fviz_algorithm_configure_input_port(filter->algorithm, 1u, FVIZ_TYPE_DATA_OBJECT, FVIZ_FALSE, FVIZ_FALSE) !=
            FVIZ_OK ||
...
FVizResult fviz_resample_with_data_set_set_input_data(FVizResampleWithDataSet* f, FVizPolyData* d)
{
    return f != NULL ? fviz_algorithm_set_input_data(f->algorithm, 0u, (FVizDataObject*)d)
                     : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_resample_with_data_set_set_input_connection(FVizResampleWithDataSet* f, FVizAlgorithmOutput* d)
{
    return f != NULL ? fviz_algorithm_set_input_connection(f->algorithm, 0u, d) : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_resample_with_data_set_set_source_data(FVizResampleWithDataSet* f, FVizDataObject* d)
{
    return f != NULL ? fviz_algorithm_set_input_data(f->algorithm, 1u, d) : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_resample_with_data_set_set_source_connection(FVizResampleWithDataSet* f, FVizAlgorithmOutput* d)
{
    return f != NULL ? fviz_algorithm_set_input_connection(f->algorithm, 1u, d) : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizAlgorithm* fviz_resample_with_data_set_algorithm(FVizResampleWithDataSet* f)
{
    return f != NULL ? f->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_resample_with_data_set_output_port(FVizResampleWithDataSet* f)
{
    return f != NULL ? fviz_algorithm_output_port(f->algorithm, 0u) : NULL;
}

FVizPolyData* fviz_resample_with_data_set_output(FVizResampleWithDataSet* f)
{
    return f != NULL ? (FVizPolyData*)fviz_algorithm_output_data(f->algorithm, 0u) : NULL;
}

FVizResult fviz_resample_with_data_set_update(FVizResampleWithDataSet* f)
{
