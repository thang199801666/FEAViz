...
static FVizSymmetricTensor3d fviz_array_calc_tensor(const FVizArrayCalcContext* context, FVizSize tuple)
{
    ...
    (void)fviz_symmetric_tensor3d_from_components(components, context->source_components, &tensor);
    return tensor;
}
...
static void fviz_array_calc_range(FVizSize begin, FVizSize end, void* user_data)
{
    ...
    for (i = begin; i < end; ++i)
    {
        ...
        else
        {
            ...
            {
                c->output[i] = sqrt(0.5 * ((t.xx - t.yy) * (t.xx - t.yy) + (t.yy - t.zz) * (t.yy - t.zz) +
                                           (t.zz - t.xx) * (t.zz - t.xx)) +
            ...
            else if (c->options.operation == FVIZ_ARRAY_CALC_PRINCIPAL_MAX)
                c->output[i] = principal[0];
            else if (c->options.operation == FVIZ_ARRAY_CALC_PRINCIPAL_MID)
                c->output[i] = principal[1];
            else if (c->options.operation == FVIZ_ARRAY_CALC_PRINCIPAL_MIN)
                c->output[i] = principal[2];
            else if (c->options.operation == FVIZ_ARRAY_CALC_HALF_PRINCIPAL_SPAN)
                c->output[i] = 0.5 * (principal[0] - principal[2]);
            else if (c->options.operation == FVIZ_ARRAY_CALC_PRINCIPAL_SPAN)
                c->output[i] = principal[0] - principal[2];
            ...
            else if (c->options.operation == FVIZ_ARRAY_CALC_DEVIATORIC_TENSOR)
            {
                ...
                tuple[0] = deviatoric.xx;
                tuple[1] = deviatoric.yy;
                tuple[2] = deviatoric.zz;
                tuple[3] = deviatoric.xy;
                tuple[4] = deviatoric.yz;
                tuple[5] = deviatoric.xz;
            }
...
FVizResult fviz_array_calculator_compute(const FVizDataArray* source, const FVizArrayCalculatorOptions* options,
                                         FVizDataArray** out_array)
{
    ...
    *out_array = NULL;
    if (source == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "array calculator source must not be NULL");
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "array calculator options are invalid");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    if (options->operation == FVIZ_ARRAY_CALC_COMPONENT && options->component >= fviz_data_array_components(source))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "array calculator component is out of range");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    if (options->operation >= FVIZ_ARRAY_CALC_EQUIVALENT_DEVIATORIC &&
        options->operation != FVIZ_ARRAY_CALC_SCALE_OFFSET && fviz_data_array_components(source) != 6u &&
        fviz_data_array_components(source) != 9u)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "tensor calculation requires 6 or 9 tensor components");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    ...
    else if (options->operation == FVIZ_ARRAY_CALC_PRINCIPAL_VALUES)
        output_components = 3u;
    else if (options->operation == FVIZ_ARRAY_CALC_DEVIATORIC_TENSOR)
        output_components = 6u;
    else if (options->operation == FVIZ_ARRAY_CALC_PRINCIPAL_DIRECTIONS)
        output_components = 9u;
    ...
    if (tuple_count >= options->parallel_threshold && options->parallel_threshold != 0u)
    {
        ...
        fviz_array_calc_range(0u, tuple_count, &context);
    *out_array = output;
    return FVIZ_OK;
}
...
static const FVizObjectClass g_fviz_array_calculator_filter_class = {FVIZ_TYPE_ARRAY_CALCULATOR_FILTER,
                                                                     "FVizArrayCalculatorFilter", &g_fviz_object_class,
...
static FVizResult fviz_array_calculator_filter_copy_name(char* dst, FVizSize capacity, const char* src,
                                                         const char* what)
{
...
static FVizResult fviz_array_calculator_filter_process(FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request,
                                                       void* state)
{
    ...
    if (filter->array_name[0] == '\0' || filter->result_name[0] == '\0')
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE,
                                "array calculator filter requires input and result array names");
        return FVIZ_ERROR_INVALID_STATE;
    }
    source_set = filter->association == FVIZ_ARRAY_CALC_POINT_DATA ? fviz_poly_data_const_point_data(input)
                                                                   : fviz_poly_data_const_cell_data(input);
    ...
    if (fviz_poly_data_deep_copy(input, &output) != FVIZ_OK ||
        ...
        goto fail;
    destination_set = filter->association == FVIZ_ARRAY_CALC_POINT_DATA ? fviz_poly_data_point_data(output)
                                                                        : fviz_poly_data_cell_data(output);
    ...
    if (filter->active_scalars != FVIZ_FALSE && fviz_data_array_components(result_array) == 1u &&
        fviz_attribute_set_set_active(destination_set, FVIZ_ATTRIBUTE_SCALARS, filter->result_name) != FVIZ_OK)
        goto fail;
...
FVizResult fviz_array_calculator_filter_create(FVizArrayCalculatorFilter** out_filter)
{
    ...
    *out_filter = NULL;
    filter = (FVizArrayCalculatorFilter*)fviz_internal_object_allocate(sizeof(*filter),
                                                                       &g_fviz_array_calculator_filter_class, NULL);
...
FVizResult fviz_array_calculator_filter_set_input_connection(FVizArrayCalculatorFilter* filter,
...
FVizResult fviz_array_calculator_filter_set_array(FVizArrayCalculatorFilter* filter,
                                                  FVizArrayCalculatorAssociation association, const char* array_name)
{
    ...
    if (fviz_array_calculator_filter_copy_name(filter->array_name, sizeof(filter->array_name), array_name,
                                               "array calculator input array name is invalid") != FVIZ_OK)
        return fviz_last_error_code();
...
FVizResult fviz_array_calculator_filter_set_result_name(FVizArrayCalculatorFilter* filter, const char* result_name)
{
    ...
    if (fviz_array_calculator_filter_copy_name(filter->result_name, sizeof(filter->result_name), result_name,
                                               "array calculator result array name is invalid") != FVIZ_OK)
        return fviz_last_error_code();
    fviz_object_modified((FVizObject*)filter);
    return FVIZ_OK;
}

FVizResult fviz_array_calculator_filter_set_options(FVizArrayCalculatorFilter* filter,
...
FVizAlgorithm* fviz_array_calculator_filter_algorithm(FVizArrayCalculatorFilter* filter)
{
    return filter != NULL ? filter->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_array_calculator_filter_output_port(FVizArrayCalculatorFilter* filter)
{
    return filter != NULL ? fviz_algorithm_output_port(filter->algorithm, 0u) : NULL;
}

FVizPolyData* fviz_array_calculator_filter_output(FVizArrayCalculatorFilter* filter)
{
    return filter != NULL ? (FVizPolyData*)fviz_algorithm_output_data(filter->algorithm, 0u) : NULL;
}

FVizResult fviz_array_calculator_filter_update(FVizArrayCalculatorFilter* filter)
{
