...
static const FVizObjectClass g_fviz_tube_class = {FVIZ_TYPE_TUBE_FILTER, "FVizTubeFilter", &g_fviz_object_class,
                                                  fviz_tube_destroy, NULL};
...
static FVizResult fviz_tube_map_attributes(const FVizAttributeSet* source, const FVizSize* map, FVizSize map_count,
                                           FVizAttributeSet* destination)
{
    ...
    for (a = 0u; a < fviz_attribute_set_count(source); ++a)
    {
        ...
        for (i = 0u; i < map_count; ++i)
        {
            ...
            {
                fviz_release(dst);
                fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "tube attribute tuple mapping is out of range");
    ...
    for (a = 0u; a < (FVizSize)FVIZ_ATTRIBUTE_ROLE_COUNT; ++a)
    {
        ...
        {
            if (fviz_attribute_set_set_active(destination, (FVizAttributeRole)a, active) != FVIZ_OK)
                return fviz_last_error_code();
        }
    }
    return FVIZ_OK;
...
static FVizResult fviz_tube_emit_segment(FVizPolyData* output, FVizVec3 p0, FVizVec3 p1, double radius, uint32_t sides,
                                         FVizBool capping, FVizSize source_p0, FVizSize source_p1, FVizSize source_cell,
                                         FVizSize* point_map, FVizSize* point_cursor, FVizSize* cell_map,
                                         FVizSize* cell_cursor)
{
    ...
    for (j = 0u; j < sides; ++j)
    {
        ...
        const FVizVec3 offset = fviz_vec3_add(fviz_vec3_scale(u, (float)(radius * cosf(angle))),
                                              fviz_vec3_scale(v, (float)(radius * sinf(angle))));
        uint32_t index;
        if (fviz_poly_data_add_point(output, fviz_vec3_add(p0, offset), &index) != FVIZ_OK)
            return fviz_last_error_code();
    ...
    for (j = 0u; j < sides; ++j)
    {
        ...
        const FVizVec3 offset = fviz_vec3_add(fviz_vec3_scale(u, (float)(radius * cosf(angle))),
                                              fviz_vec3_scale(v, (float)(radius * sinf(angle))));
        uint32_t index;
        if (fviz_poly_data_add_point(output, fviz_vec3_add(p1, offset), &index) != FVIZ_OK)
            return fviz_last_error_code();
    ...
    for (j = 0u; j < sides; ++j)
    {
        ...
        if (fviz_poly_data_add_triangle(output, base + j, base + k, base + sides + k) != FVIZ_OK ||
            fviz_poly_data_add_triangle(output, base + j, base + sides + k, base + sides + j) != FVIZ_OK)
            return fviz_last_error_code();
    ...
    if (capping != FVIZ_FALSE && sides >= 3u)
    {
        for (j = 1u; j + 1u < sides; ++j)
        {
            if (fviz_poly_data_add_triangle(output, base, base + j + 1u, base + j) != FVIZ_OK ||
                fviz_poly_data_add_triangle(output, base + sides, base + sides + j, base + sides + j + 1u) != FVIZ_OK)
                return fviz_last_error_code();
            cell_map[(*cell_cursor)++] = source_cell;
            cell_map[(*cell_cursor)++] = source_cell;
        }
...
static FVizResult fviz_tube_process(FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request, void* state)
{
    ...
    if (input == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "tube filter has no input");
        return FVIZ_ERROR_INVALID_STATE;
    }
    if (fviz_poly_data_validate(input) != FVIZ_OK) return fviz_last_error_code();
    lines = fviz_poly_data_lines(input);
    line_cells = fviz_cell_array_count(lines);
    ...
    if (fviz_size_multiply(segments, (FVizSize)(2u * filter->sides), &points_needed) != FVIZ_OK)
        return fviz_last_error_code();
    {
        const FVizSize per_segment = (FVizSize)(2u * filter->sides) +
                                     (filter->capping != FVIZ_FALSE ? (FVizSize)(2u * (filter->sides - 2u)) : 0u);
    ...
    if (fviz_poly_data_create(&output) != FVIZ_OK ||
        fviz_poly_data_reserve(output, points_needed, triangles_needed) != FVIZ_OK)
        goto fail;
    if (points_needed != 0u)
    {
        point_map = (FVizSize*)fviz_alloc(point_map_bytes);
        if (point_map == NULL) goto fail;
    }
    if (triangles_needed != 0u)
    {
        cell_map = (FVizSize*)fviz_alloc(cell_map_bytes);
        if (cell_map == NULL) goto fail;
    }
    for (line_id = 0u; line_id < line_cells; ++line_id)
    {
        FVizCellView view;
        ...
        for (j = 0u; j + 1u < view.point_count; ++j)
        {
            ...
            if (id0 > UINT32_MAX || id1 > UINT32_MAX ||
                ...
                fviz_poly_data_get_point(input, (FVizSize)id1, &p1) != FVIZ_OK)
                goto fail;
            if (fviz_tube_emit_segment(output, p0, p1, filter->radius, filter->sides, filter->capping, (FVizSize)id0,
                                       (FVizSize)id1, fviz_poly_data_vert_cell_count(input) + line_id, point_map, &pc,
                                       cell_map, &cc) != FVIZ_OK)
                goto fail;
        }
    }
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "tube filter internal topology accounting failed");
        goto fail;
    }
    if (fviz_tube_map_attributes(fviz_poly_data_const_point_data(input), point_map, pc,
                                 fviz_poly_data_point_data(output)) != FVIZ_OK ||
        fviz_tube_map_attributes(fviz_poly_data_const_cell_data(input), cell_map, cc,
                                 fviz_poly_data_cell_data(output)) != FVIZ_OK)
        goto fail;
    if (fviz_tube_copy_fields(fviz_poly_data_const_field_data(input), fviz_poly_data_field_data(output)) != FVIZ_OK)
        goto fail;
...
FVizResult fviz_tube_filter_create(FVizTubeFilter** out_filter)
{
    ...
    *out_filter = NULL;
    ...
    filter->radius = 0.05;
    filter->sides = 12u;
    filter->capping = FVIZ_TRUE;
    fviz_algorithm_callbacks_initialize(&callbacks);
    callbacks.process_request = fviz_tube_process;
    callbacks.get_state_mtime = fviz_tube_state_mtime;
...
FVizResult fviz_tube_filter_set_input_data(FVizTubeFilter* f, FVizPolyData* input)
{
    return f != NULL ? fviz_algorithm_set_input_data(f->algorithm, 0u, (FVizDataObject*)input)
                     : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_tube_filter_set_input_connection(FVizTubeFilter* f, FVizAlgorithmOutput* input)
{
    return f != NULL ? fviz_algorithm_set_input_connection(f->algorithm, 0u, input) : FVIZ_ERROR_INVALID_ARGUMENT;
}

void fviz_tube_filter_set_radius(FVizTubeFilter* f, double v)
{
    if (f != NULL && isfinite(v) && v > 0.0 && f->radius != v)
    {
        f->radius = v;
...
double fviz_tube_filter_radius(const FVizTubeFilter* f)
{
    return f != NULL ? f->radius : 0.0;
}

void fviz_tube_filter_set_sides(FVizTubeFilter* f, uint32_t v)
{
    if (f != NULL)
    {
        if (v < 3u) v = 3u;
...
uint32_t fviz_tube_filter_sides(const FVizTubeFilter* f)
{
    return f != NULL ? f->sides : 0u;
}

void fviz_tube_filter_set_capping(FVizTubeFilter* f, FVizBool v)
{
    if (f != NULL)
    {
...
FVizBool fviz_tube_filter_capping(const FVizTubeFilter* f)
{
    return f != NULL ? f->capping : FVIZ_FALSE;
}

FVizAlgorithm* fviz_tube_filter_algorithm(FVizTubeFilter* f)
{
    return f != NULL ? f->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_tube_filter_output_port(FVizTubeFilter* f)
{
    return f != NULL ? fviz_algorithm_output_port(f->algorithm, 0u) : NULL;
}

FVizPolyData* fviz_tube_filter_output(FVizTubeFilter* f)
{
    return f != NULL ? (FVizPolyData*)fviz_algorithm_output_data(f->algorithm, 0u) : NULL;
}

FVizResult fviz_tube_filter_update(FVizTubeFilter* f)
{
