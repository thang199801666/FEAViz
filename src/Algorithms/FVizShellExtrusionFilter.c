...
static const FVizObjectClass g_fviz_shell_extrusion_class = {FVIZ_TYPE_SHELL_EXTRUSION_FILTER,
                                                             "FVizShellExtrusionFilter", &g_fviz_object_class,
...
{
    FVizSize p = 1u;
    while (p < v && p <= ((FVizSize)-1) / 2u)
        p *= 2u;
    return p >= v ? p : 0u;
}

static FVizResult fviz_shell_map_attributes(const FVizAttributeSet* source, const FVizSize* map, FVizSize map_count,
                                            FVizAttributeSet* destination)
{
    ...
    for (a = 0u; a < fviz_attribute_set_count(source); ++a)
    {
        ...
        if (fviz_data_array_create(fviz_data_array_type(src), fviz_data_array_components(src), &dst) != FVIZ_OK ||
            fviz_data_array_reserve(dst, map_count) != FVIZ_OK)
        {
            fviz_release(dst);
        ...
        for (i = 0u; i < map_count; ++i)
        {
            ...
            {
                fviz_release(dst);
                fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "shell extrusion attribute mapping is out of range");
                return FVIZ_ERROR_INVALID_STATE;
            }
            tuple = fviz_data_array_const_tuple(src, map[i]);
            if (tuple == NULL || fviz_data_array_append_tuple(dst, tuple) != FVIZ_OK)
            {
                fviz_release(dst);
                return fviz_last_error_code();
            }
        }
        if (fviz_attribute_set_add(destination, name, dst) != FVIZ_OK)
        {
            fviz_release(dst);
            return fviz_last_error_code();
        }
    ...
    for (a = 0u; a < (FVizSize)FVIZ_ATTRIBUTE_ROLE_COUNT; ++a)
    {
        ...
        if (active != NULL && fviz_attribute_set_const_get(destination, active) != NULL &&
            fviz_attribute_set_set_active(destination, (FVizAttributeRole)a, active) != FVIZ_OK)
            return fviz_last_error_code();
    }
    return FVIZ_OK;
}
...
static FVizResult fviz_shell_copy_fields(const FVizAttributeSet* source, FVizAttributeSet* destination)
{
    ...
    for (i = 0u; i < fviz_attribute_set_count(source); ++i)
    {
        ...
        if (name != NULL && array != NULL && fviz_attribute_set_add(destination, name, array) != FVIZ_OK)
            return fviz_last_error_code();
    }
    return FVIZ_OK;
}

static FVizResult fviz_shell_build_edges(const uint32_t* triangles, FVizSize triangle_count, FVizShellEdge** out_edges,
                                         FVizSize* out_edge_count, FVizSize* out_boundary_count)
{
    FVizShellEdge* edges = NULL;
    FVizSize* slots = NULL;
    ...
    *out_boundary_count = 0u;
    ...
    for (t = 0u; t < triangle_count; ++t)
    {
        ...
        for (e = 0u; e < 3u; ++e)
        {
            uint32_t a = tri[e], b = tri[(e + 1u) % 3u];
            FVizSize slot;
            if (a > b)
            {
            ...
            if (slots[slot] == 0u)
            {
                ...
                created->b = b;
                created->count = 1u;
                created->source_triangle = t;
                slots[slot] = edge_count + 1u;
                edge_count += 1u;
            }
        }
    }
    for (t = 0u; t < edge_count; ++t)
        if (edges[t].count == 1u) boundary += 1u;
    fviz_free(slots);
    *out_edges = edges;
    *out_edge_count = edge_count;
    *out_boundary_count = boundary;
    return FVIZ_OK;
}

static FVizResult fviz_shell_extrusion_process(FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request,
                                               void* state)
{
    FVizShellExtrusionFilter* filter = (FVizShellExtrusionFilter*)state;
    FVizPolyData* input;
    FVizPolyData* output = NULL;
    FVizVec3* normals = NULL;
    FVizSize* point_map = NULL;
    FVizSize* cell_map = NULL;
    FVizShellEdge* edges = NULL;
    FVizSize edge_count = 0u, boundary_count = 0u;
    ...
    if (input == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "shell extrusion filter has no input");
        return FVIZ_ERROR_INVALID_STATE;
    }
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED, "shell extrusion requires triangle-only PolyData");
        return FVIZ_ERROR_NOT_SUPPORTED;
    }
    if (point_count > UINT32_MAX / 2u)
    {
        fviz_internal_set_error(FVIZ_ERROR_OVERFLOW, "shell extrusion exceeds uint32 render-index capacity");
        return FVIZ_ERROR_OVERFLOW;
    }
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "shell triangle cache is unavailable");
        return FVIZ_ERROR_INVALID_STATE;
    }
    if (fviz_shell_build_edges(triangles, triangle_count, &edges, &edge_count, &boundary_count) != FVIZ_OK)
        return fviz_last_error_code();
    if (fviz_size_multiply(boundary_count, 2u, &output_triangles) != FVIZ_OK ||
        output_triangles > ((FVizSize)-1) - 2u * triangle_count)
    {
        fviz_free(edges);
        fviz_internal_set_error(FVIZ_ERROR_OVERFLOW, "shell extrusion triangle count overflow");
        return FVIZ_ERROR_OVERFLOW;
    }
    ...
    if (fviz_size_multiply(point_count, sizeof(*normals), &normal_bytes) != FVIZ_OK ||
        ...
        fviz_size_multiply(output_triangles, sizeof(*cell_map), &cell_map_bytes) != FVIZ_OK)
        goto fail;
    ...
    if ((point_count != 0u && (normals == NULL || point_map == NULL)) || (output_triangles != 0u && cell_map == NULL))
        goto fail;
    ...
    for (t = 0u; t < triangle_count; ++t)
    {
        ...
        normals[a] = fviz_vec3_add(normals[a], n);
        normals[b] = fviz_vec3_add(normals[b], n);
        normals[c] = fviz_vec3_add(normals[c], n);
    }
    if (fviz_poly_data_create(&output) != FVIZ_OK ||
        fviz_poly_data_reserve(output, 2u * point_count, output_triangles) != FVIZ_OK)
        goto fail;
    for (i = 0u; i < point_count; ++i)
    {
        FVizVec3 n = fviz_vec3_normalize(normals[i]);
        uint32_t id;
        if (fviz_vec3_length(n) <= 1.0e-20f) n = fviz_vec3(0, 0, 1);
        if (fviz_poly_data_add_point(output, fviz_vec3_add(points[i], fviz_vec3_scale(n, half)), &id) != FVIZ_OK)
            goto fail;
        point_map[i] = i;
    }
    for (i = 0u; i < point_count; ++i)
    {
        ...
        uint32_t id;
        if (fviz_vec3_length(n) <= 1.0e-20f) n = fviz_vec3(0, 0, 1);
        if (fviz_poly_data_add_point(output, fviz_vec3_sub(points[i], fviz_vec3_scale(n, half)), &id) != FVIZ_OK)
            goto fail;
    ...
    for (i = 0u; i < edge_count; ++i)
        if (edges[i].count == 1u)
        {
            ...
            if (fviz_poly_data_add_triangle(output, a, n + a, n + b) != FVIZ_OK ||
                fviz_poly_data_add_triangle(output, a, n + b, b) != FVIZ_OK)
                goto fail;
            cell_map[cc++] = edges[i].source_triangle;
            cell_map[cc++] = edges[i].source_triangle;
        }
    if (cc != output_triangles)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "shell extrusion topology accounting failed");
        goto fail;
    }
    if (fviz_shell_map_attributes(fviz_poly_data_const_point_data(input), point_map, 2u * point_count,
                                  fviz_poly_data_point_data(output)) != FVIZ_OK ||
        fviz_shell_map_attributes(fviz_poly_data_const_cell_data(input), cell_map, output_triangles,
                                  fviz_poly_data_cell_data(output)) != FVIZ_OK ||
        ...
        fviz_algorithm_set_output_data(algorithm, 0u, (FVizDataObject*)output) != FVIZ_OK)
        goto fail;
    fviz_free(normals);
    fviz_free(point_map);
    fviz_free(cell_map);
...
{
    FVizShellExtrusionFilter* filter;
    ...
    *out_filter = NULL;
    filter =
        (FVizShellExtrusionFilter*)fviz_internal_object_allocate(sizeof(*filter), &g_fviz_shell_extrusion_class, NULL);
    if (filter == NULL) return fviz_last_error_code();
    filter->thickness = 0.1;
    fviz_algorithm_callbacks_initialize(&callbacks);
    callbacks.process_request = fviz_shell_extrusion_process;
    callbacks.get_state_mtime = fviz_shell_extrusion_state_mtime;
...
FVizResult fviz_shell_extrusion_filter_set_input_data(FVizShellExtrusionFilter* f, FVizPolyData* input)
{
    return f != NULL ? fviz_algorithm_set_input_data(f->algorithm, 0u, (FVizDataObject*)input)
                     : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_shell_extrusion_filter_set_input_connection(FVizShellExtrusionFilter* f, FVizAlgorithmOutput* input)
{
    return f != NULL ? fviz_algorithm_set_input_connection(f->algorithm, 0u, input) : FVIZ_ERROR_INVALID_ARGUMENT;
}

void fviz_shell_extrusion_filter_set_thickness(FVizShellExtrusionFilter* f, double v)
{
    if (f != NULL && isfinite(v) && v > 0.0 && f->thickness != v)
    {
        f->thickness = v;
        fviz_object_modified((FVizObject*)f);
    }
}

double fviz_shell_extrusion_filter_thickness(const FVizShellExtrusionFilter* f)
{
    return f != NULL ? f->thickness : 0.0;
}

FVizAlgorithm* fviz_shell_extrusion_filter_algorithm(FVizShellExtrusionFilter* f)
{
    return f != NULL ? f->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_shell_extrusion_filter_output_port(FVizShellExtrusionFilter* f)
{
    return f != NULL ? fviz_algorithm_output_port(f->algorithm, 0u) : NULL;
}

FVizPolyData* fviz_shell_extrusion_filter_output(FVizShellExtrusionFilter* f)
{
    return f != NULL ? (FVizPolyData*)fviz_algorithm_output_data(f->algorithm, 0u) : NULL;
}

FVizResult fviz_shell_extrusion_filter_update(FVizShellExtrusionFilter* f)
{
    return f != NULL ? fviz_algorithm_update(f->algorithm) : FVIZ_ERROR_INVALID_ARGUMENT;
}
