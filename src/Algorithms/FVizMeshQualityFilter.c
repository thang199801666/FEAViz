...
static FVizResult fviz_quality_cell_points(const FVizUnstructuredGrid* grid, FVizSize cell_id, FVizCellView* out_view,
                                           FVizVec3* points, FVizSize capacity)
{
    FVizSize i;
    ...
    if (fviz_cell_array_cell_view(fviz_unstructured_grid_cells((FVizUnstructuredGrid*)grid), cell_id, out_view) !=
        FVIZ_OK)
        return fviz_last_error_code();
    if (out_view->point_count > capacity)
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                "mesh quality cell has more points than the current quality kernel supports");
...
static double fviz_quality_measure(FVizCellType type, const FVizVec3* p, FVizSize count)
{
    ...
    if (traits.dimension == 1u && count >= 2u)
    {
            ...
            return fviz_quality_length(fviz_vec3_sub(p[2], p[0])) + fviz_quality_length(fviz_vec3_sub(p[1], p[2]));
        {
            ...
            double length = 0.0;
            for (i = 1u; i < count; ++i)
                length += fviz_quality_length(fviz_vec3_sub(p[i], p[i - 1u]));
            return length;
        }
    }
    if (traits.dimension == 2u && count >= 3u)
    {
        const FVizSize corners =
            (type == FVIZ_CELL_QUADRATIC_TRIANGLE)
                ? 3u
                : ((type == FVIZ_CELL_QUADRATIC_QUAD || type == FVIZ_CELL_BIQUADRATIC_QUAD) ? 4u : count);
        FVizSize i;
        double area = 0.0;
        for (i = 1u; i + 1u < corners; ++i)
            area += fviz_quality_triangle_area(p[0], p[i], p[i + 1u]);
        return area;
    }
...
static double fviz_quality_scaled_jacobian(FVizCellType type, const FVizVec3* p)
{
    ...
    {
        static const uint8_t neighbors[8][3] = {{1, 3, 4}, {2, 0, 5}, {3, 1, 6}, {0, 2, 7},
                                                {7, 5, 0}, {4, 6, 1}, {5, 7, 2}, {6, 4, 3}};
        double minimum = INFINITY;
        uint32_t corner;
        for (corner = 0u; corner < 8u; ++corner)
        {
            const double value = fviz_quality_scaled_det(fviz_vec3_sub(p[neighbors[corner][0]], p[corner]),
         ...
         * wedge/pyramid cells; tetra/hex above retain signed inversion detection. */
        {
            ...
            for (edge = 0u; edge < traits.edge_count && found < 3u; ++edge)
            {
                ...
                if (local[0] == 0u) vectors[found++] = fviz_vec3_sub(p[local[1]], p[0]);
                    ...
                    vectors[found++] = fviz_vec3_sub(p[local[0]], p[0]);
            }
...
static double fviz_quality_corner_angle(FVizCellType type, const FVizVec3* p, FVizBool maximum)
{
    ...
    if (traits.dimension == 2u)
    {
        ...
        {
            const uint32_t prev = local[(i + face_count - 1u) % face_count], cur = local[i],
                           next = local[(i + 1u) % face_count];
            const double angle =
                fviz_quality_angle_degrees(fviz_vec3_sub(p[prev], p[cur]), fviz_vec3_sub(p[next], p[cur]));
...
static double fviz_quality_warpage(FVizCellType type, const FVizVec3* p)
{
    ...
    for (face = 0u; face < face_total; ++face)
    {
        ...
        else if (fviz_cell_type_face(type, face, local, 9u, &count) != FVIZ_OK)
            continue;
...
static double fviz_quality_value(FVizCellType type, const FVizVec3* p, FVizSize count, FVizMeshQualityMetric metric)
{
    switch (metric)
    {
        case FVIZ_MESH_QUALITY_MEASURE:
            return fviz_quality_measure(type, p, count);
        case FVIZ_MESH_QUALITY_EDGE_RATIO:
            return fviz_quality_edge_ratio(type, p);
        case FVIZ_MESH_QUALITY_SCALED_JACOBIAN:
            return fviz_quality_scaled_jacobian(type, p);
        case FVIZ_MESH_QUALITY_MIN_CORNER_ANGLE:
            return fviz_quality_corner_angle(type, p, FVIZ_FALSE);
        case FVIZ_MESH_QUALITY_MAX_CORNER_ANGLE:
            return fviz_quality_corner_angle(type, p, FVIZ_TRUE);
        case FVIZ_MESH_QUALITY_WARPAGE:
            return fviz_quality_warpage(type, p);
        default:
            return NAN;
    }
}
...
FVizResult fviz_mesh_quality_compute(const FVizUnstructuredGrid* input, FVizMeshQualityMetric metric,
                                     FVizDataArray** out_quality)
{
    ...
    *out_quality = NULL;
    ...
    {
        fviz_release(quality);
        return fviz_last_error_code();
    }
    context.grid = input;
    context.metric = metric;
    context.output = (double*)fviz_data_array_data(quality);
    if (cell_count >= 4096u)
    {
        ...
        fviz_mesh_quality_range(0u, cell_count, &context);
    *out_quality = quality;
    return FVIZ_OK;
}
...
static const FVizObjectClass g_fviz_mesh_quality_filter_class = {FVIZ_TYPE_MESH_QUALITY_FILTER, "FVizMeshQualityFilter",
                                                                 &g_fviz_object_class, fviz_mesh_quality_filter_destroy,
                                                                 NULL};
...
static FVizResult fviz_mesh_quality_filter_process(FVizAlgorithm* algorithm, const FVizPipelineRequestInfo* request,
                                                   void* state)
{
    ...
    if (input == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "mesh quality filter has no input");
        return FVIZ_ERROR_INVALID_STATE;
    }
    ...
    if (filter->active_scalars != FVIZ_FALSE &&
        fviz_attribute_set_set_active(fviz_unstructured_grid_cell_data(output), FVIZ_ATTRIBUTE_SCALARS,
                                      filter->result_name) != FVIZ_OK)
        goto fail;
...
FVizResult fviz_mesh_quality_filter_create(FVizMeshQualityFilter** out_filter)
{
    ...
    *out_filter = NULL;
    filter =
        (FVizMeshQualityFilter*)fviz_internal_object_allocate(sizeof(*filter), &g_fviz_mesh_quality_filter_class, NULL);
...
{
    if (filter == NULL || metric < FVIZ_MESH_QUALITY_MEASURE || metric > FVIZ_MESH_QUALITY_WARPAGE ||
...
FVizResult fviz_mesh_quality_filter_set_result_name(FVizMeshQualityFilter* filter, const char* name)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "mesh quality result name is invalid");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    if (strcmp(filter->result_name, name) == 0) return FVIZ_OK;
    (void)memcpy(filter->result_name, name, length + 1u);
    fviz_object_modified((FVizObject*)filter);
    return FVIZ_OK;
}
...
FVizAlgorithm* fviz_mesh_quality_filter_algorithm(FVizMeshQualityFilter* filter)
{
    return filter != NULL ? filter->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_mesh_quality_filter_output_port(FVizMeshQualityFilter* filter)
{
    return filter != NULL ? fviz_algorithm_output_port(filter->algorithm, 0u) : NULL;
}

FVizUnstructuredGrid* fviz_mesh_quality_filter_output(FVizMeshQualityFilter* filter)
{
    return filter != NULL ? (FVizUnstructuredGrid*)fviz_algorithm_output_data(filter->algorithm, 0u) : NULL;
}

FVizResult fviz_mesh_quality_filter_update(FVizMeshQualityFilter* filter)
{
