...
static void fviz_warp_vector_range(FVizSize begin,FVizSize end,void* user_data)
{
    ...
    for (i=begin;i<end;++i)
    {
        ...
        c->output[i]=fviz_vec3(
            c->input[i].x+(float)(x*c->scale),
            c->input[i].y+(float)(y*c->scale),
...
static const FVizObjectClass g_fviz_warp_vector_filter_class={
    FVIZ_TYPE_WARP_VECTOR_FILTER,"FVizWarpVectorFilter",&g_fviz_object_class,
    fviz_warp_vector_filter_destroy,NULL
};
...
static FVizResult fviz_warp_vector_filter_process(
    FVizAlgorithm* algorithm,const FVizPipelineRequestInfo* request,void* state)
{
    ...
    const FVizSize point_count=request->type==FVIZ_PIPELINE_REQUEST_DATA?
        fviz_poly_data_point_count((FVizPolyData*)fviz_algorithm_resolved_input(algorithm,0u,0u)):0u;
    ...
    if (input==NULL)
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE,"warp vector filter has no input"); return FVIZ_ERROR_INVALID_STATE; }
    vectors=fviz_attribute_set_const_get(fviz_poly_data_const_point_data(input),filter->vector_name);
    if (vectors==NULL || fviz_data_array_components(vectors)<3u || fviz_data_array_tuple_count(vectors)!=point_count)
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE,"warp vector array is missing or has an invalid shape"); return FVIZ_ERROR_INVALID_STATE; }
    ...
    else fviz_warp_vector_range(0u,point_count,&context);
    fviz_release(output->points); output->points=new_points; new_points=NULL;
    fviz_release(output->normals); output->normals=new_normals; new_normals=NULL;
    output->bounds=fviz_bounds_empty(); output->bounds_dirty=FVIZ_TRUE; output->normals_dirty=FVIZ_TRUE;
    ...
    fviz_release(output); return FVIZ_OK;
fail:
    fviz_release(new_points); fviz_release(new_normals); fviz_release(output); return fviz_last_error_code();
}
...
FVizResult fviz_warp_vector_filter_create(FVizWarpVectorFilter** out_filter)
{
    ...
    *out_filter=NULL;
    filter=(FVizWarpVectorFilter*)fviz_internal_object_allocate(sizeof(*filter),&g_fviz_warp_vector_filter_class,NULL);
    if (filter==NULL) return fviz_last_error_code();
    (void)memcpy(filter->vector_name,"Vectors",8u);
    filter->scale=1.0; filter->recompute_normals=FVIZ_TRUE;
...
FVizResult fviz_warp_vector_filter_set_vector_name(FVizWarpVectorFilter* filter,const char* name)
{
    ...
    { fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,"warp vector name is invalid"); return FVIZ_ERROR_INVALID_ARGUMENT; }
    if (strcmp(filter->vector_name,name)!=0)
    { memcpy(filter->vector_name,name,length+1u); fviz_object_modified((FVizObject*)filter); }
    return FVIZ_OK;
}
const char* fviz_warp_vector_filter_vector_name(const FVizWarpVectorFilter* filter){return filter!=NULL?filter->vector_name:NULL;}
void fviz_warp_vector_filter_set_scale(FVizWarpVectorFilter* filter,double scale)
{ if (filter!=NULL && isfinite(scale) && filter->scale!=scale) { filter->scale=scale; fviz_object_modified((FVizObject*)filter); } }
double fviz_warp_vector_filter_scale(const FVizWarpVectorFilter* filter){return filter!=NULL?filter->scale:0.0;}
void fviz_warp_vector_filter_set_recompute_normals(FVizWarpVectorFilter* filter,FVizBool enabled)
{ if (filter!=NULL) { enabled=enabled!=FVIZ_FALSE?FVIZ_TRUE:FVIZ_FALSE; if (filter->recompute_normals!=enabled) { filter->recompute_normals=enabled; fviz_object_modified((FVizObject*)filter); } } }
FVizBool fviz_warp_vector_filter_recompute_normals(const FVizWarpVectorFilter* filter){return filter!=NULL?filter->recompute_normals:FVIZ_FALSE;}
FVizAlgorithm* fviz_warp_vector_filter_algorithm(FVizWarpVectorFilter* filter){return filter!=NULL?filter->algorithm:NULL;}
FVizAlgorithmOutput* fviz_warp_vector_filter_output_port(FVizWarpVectorFilter* filter){return filter!=NULL?fviz_algorithm_output_port(filter->algorithm,0u):NULL;}
FVizPolyData* fviz_warp_vector_filter_output(FVizWarpVectorFilter* filter){return filter!=NULL?(FVizPolyData*)fviz_algorithm_output_data(filter->algorithm,0u):NULL;}
FVizResult fviz_warp_vector_filter_update(FVizWarpVectorFilter* filter){return filter!=NULL?fviz_algorithm_update(filter->algorithm):FVIZ_ERROR_INVALID_ARGUMENT;}
