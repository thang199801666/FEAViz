...
static const FVizObjectClass g_fviz_ug_geometry_class={
    FVIZ_TYPE_UNSTRUCTURED_GRID_GEOMETRY_FILTER,"FVizUnstructuredGridGeometryFilter",
    &g_fviz_object_class,fviz_ug_geometry_destroy,NULL
};
...
static FVizResult fviz_ug_geometry_process(
...
FVizResult fviz_unstructured_grid_geometry_filter_create(FVizUnstructuredGridGeometryFilter** out_filter)
{
    ...
    *out_filter=NULL;
    filter=(FVizUnstructuredGridGeometryFilter*)fviz_internal_object_allocate(sizeof(*filter),&g_fviz_ug_geometry_class,NULL);
...
FVizResult fviz_unstructured_grid_geometry_filter_set_input_data(FVizUnstructuredGridGeometryFilter* filter,FVizUnstructuredGrid* input)
{return filter!=NULL?fviz_algorithm_set_input_data(filter->algorithm,0u,(FVizDataObject*)input):FVIZ_ERROR_INVALID_ARGUMENT;}
FVizResult fviz_unstructured_grid_geometry_filter_set_input_connection(FVizUnstructuredGridGeometryFilter* filter,FVizAlgorithmOutput* input)
{return filter!=NULL?fviz_algorithm_set_input_connection(filter->algorithm,0u,input):FVIZ_ERROR_INVALID_ARGUMENT;}
FVizAlgorithm* fviz_unstructured_grid_geometry_filter_algorithm(FVizUnstructuredGridGeometryFilter* filter){return filter!=NULL?filter->algorithm:NULL;}
FVizAlgorithmOutput* fviz_unstructured_grid_geometry_filter_output_port(FVizUnstructuredGridGeometryFilter* filter){return filter!=NULL?fviz_algorithm_output_port(filter->algorithm,0u):NULL;}
FVizPolyData* fviz_unstructured_grid_geometry_filter_output(FVizUnstructuredGridGeometryFilter* filter){return filter!=NULL?(FVizPolyData*)fviz_algorithm_output_data(filter->algorithm,0u):NULL;}
FVizResult fviz_unstructured_grid_geometry_filter_update(FVizUnstructuredGridGeometryFilter* filter){return filter!=NULL?fviz_algorithm_update(filter->algorithm):FVIZ_ERROR_INVALID_ARGUMENT;}
