...
static void fviz_composite_geometry_destroy(FVizObject* object)
{
    ...
    fviz_release(filter->cache_entries);
    filter->cache_entries = NULL;
    fviz_release(filter->cache_index);
    filter->cache_index = NULL;
    fviz_release(filter->algorithm);
    filter->algorithm = NULL;
}

static const FVizObjectClass g_fviz_composite_geometry_class = {FVIZ_TYPE_COMPOSITE_GEOMETRY_FILTER,
                                                                "FVizCompositeGeometryFilter", &g_fviz_object_class,
...
static FVizResult fviz_composite_geometry_assign(FVizDataObject* parent, FVizSize index, FVizDataObject* child)
{
...
static FVizBool fviz_composite_geometry_cache_lookup(FVizCompositeGeometryFilter* filter, const FVizDataObject* input,
                                                     FVizMTime input_mtime, FVizCompositeGeometryCacheEntry** out_entry,
...
static void fviz_composite_geometry_cache_remove_at(FVizCompositeGeometryFilter* filter, FVizSize index,
                                                    FVizBool eviction)
{
...
{
    while (filter != NULL && filter->cache_byte_capacity != 0u && filter->cache_bytes > filter->cache_byte_capacity &&
...
static FVizResult fviz_composite_geometry_cache_commit(FVizCompositeGeometryFilter* filter, const FVizDataObject* input,
                                                       FVizMTime input_mtime, FVizCompositeGeometryCacheEntry* entry,
                                                       FVizDataObject* converted)
{
...
static FVizResult fviz_composite_geometry_convert_leaf(const FVizDataObject* input, FVizDataObject** out_data)
{
    *out_data = NULL;
...
#define FVIZ_CONVERT_WITH_FILTER(TypeMacro, FilterType, create_fn, set_fn, update_fn, output_fn)                       \
    ...
    {                                                                                                                  \
        ...
        if (result == FVIZ_OK)                                                                                         \
        {                                                                                                              \
            output = output_fn(leaf_filter);                                                                           \
            if (output == NULL) result = FVIZ_ERROR_INVALID_STATE;                                                     \
        }                                                                                                              \
        if (result == FVIZ_OK) *out_data = (FVizDataObject*)fviz_retain(output);                                       \
        fviz_release(leaf_filter);                                                                                     \
        return result;                                                                                                 \
    }
    FVIZ_CONVERT_WITH_FILTER(
        FVIZ_TYPE_UNSTRUCTURED_GRID, FVizUnstructuredGridGeometryFilter, fviz_unstructured_grid_geometry_filter_create,
        fviz_unstructured_grid_geometry_filter_set_input_data, fviz_unstructured_grid_geometry_filter_update,
        fviz_unstructured_grid_geometry_filter_output)
    FVIZ_CONVERT_WITH_FILTER(FVIZ_TYPE_STRUCTURED_GRID, FVizStructuredGridGeometryFilter,
                             fviz_structured_grid_geometry_filter_create,
                             fviz_structured_grid_geometry_filter_set_input_data,
    ...
    FVIZ_CONVERT_WITH_FILTER(FVIZ_TYPE_RECTILINEAR_GRID, FVizRectilinearGridGeometryFilter,
                             fviz_rectilinear_grid_geometry_filter_create,
                             fviz_rectilinear_grid_geometry_filter_set_input_data,
                             fviz_rectilinear_grid_geometry_filter_update, fviz_rectilinear_grid_geometry_filter_output)
    FVIZ_CONVERT_WITH_FILTER(FVIZ_TYPE_IMAGE_DATA, FVizImageDataGeometryFilter, fviz_image_data_geometry_filter_create,
                             fviz_image_data_geometry_filter_set_input_data, fviz_image_data_geometry_filter_update,
                             fviz_image_data_geometry_filter_output)
...
static FVizResult fviz_composite_geometry_parallel_convert(FVizSize begin, FVizSize end, void* user_data)
{
    ...
    {
        FVizResult result =
            fviz_composite_geometry_convert_leaf(context->leaves[i].input, &context->leaves[i].converted);
        if (result != FVIZ_OK) return result;
    }
    return FVIZ_OK;
}

static FVizResult fviz_composite_geometry_prepare_composite(const FVizDataObject* input, FVizDataObject** out_data,
                                                            FVizArray* jobs)
{
    FVizSize count, i;
    *out_data = NULL;
    if (fviz_object_is_type((const FVizObject*)input, FVIZ_TYPE_MULTI_BLOCK_DATA_SET) != FVIZ_FALSE)
    {
        ...
        if (fviz_multi_block_data_set_resize(dest, count) != FVIZ_OK)
        {
            fviz_release(dest);
        ...
        for (i = 0u; i < count; ++i)
        {
            ...
            if (name != NULL && fviz_multi_block_data_set_set_block_name(dest, i, name) != FVIZ_OK)
            {
                fviz_release(dest);
                return fviz_last_error_code();
            }
    ...
    if (fviz_object_is_type((const FVizObject*)input, FVIZ_TYPE_PARTITIONED_DATA_SET) != FVIZ_FALSE)
    {
        ...
        if (fviz_partitioned_data_set_resize(dest, count) != FVIZ_OK)
        {
            fviz_release(dest);
            return fviz_last_error_code();
        }
        for (i = 0u; i < count; ++i)
        {
            ...
            FVizCompositeGeometryJob job = {fviz_partitioned_data_set_const_partition(source, i), (FVizDataObject*)dest,
                                            i};
            if (name != NULL && fviz_partitioned_data_set_set_partition_name(dest, i, name) != FVIZ_OK)
            {
                fviz_release(dest);
                return fviz_last_error_code();
            }
...
static FVizResult fviz_composite_geometry_process_request(FVizAlgorithm* algorithm,
                                                          const FVizPipelineRequestInfo* request, void* state)
{
    ...
    for (i = 0u; i < fviz_multi_block_data_set_count(input); ++i)
    {
        ...
        FVizCompositeGeometryJob job = {fviz_multi_block_data_set_const_block(input, i), (FVizDataObject*)output, i};
    ...
    while (fviz_array_count(jobs) != 0u)
    {
        FVizCompositeGeometryJob job =
            *(const FVizCompositeGeometryJob*)fviz_array_const_at(jobs, fviz_array_count(jobs) - 1u);
        ...
        if (request->cancellation != NULL && fviz_cancellation_token_is_cancelled(request->cancellation) != FVIZ_FALSE)
        {
        ...
        if (result != FVIZ_ERROR_NOT_FOUND) goto done;
        {
            ...
            if (fviz_composite_geometry_cache_lookup(filter, job.input, leaf.input_mtime, &leaf.cache_entry,
                                                     &converted) != FVIZ_FALSE)
            {
                result = fviz_composite_geometry_assign(job.output_parent, job.index, converted);
                fviz_release(converted);
                if (result != FVIZ_OK) goto done;
    ...
    {
        FVizCompositeGeometryPendingLeaf* leaves = (FVizCompositeGeometryPendingLeaf*)fviz_array_data(pending);
        const FVizSize count = fviz_array_count(pending);
        const FVizBool use_parallel = filter->parallel_enabled != FVIZ_FALSE && count >= filter->parallel_threshold &&
        ...
        if (use_parallel != FVIZ_FALSE)
        {
            ...
            result = fviz_parallel_context_for(fviz_parallel_default_context(), 0u, count, 1u,
        ...
        for (i = 0u; i < count; ++i)
        {
            result = fviz_composite_geometry_cache_commit(filter, leaves[i].input, leaves[i].input_mtime,
                                                          leaves[i].cache_entry, leaves[i].converted);
            if (result == FVIZ_OK)
                result = fviz_composite_geometry_assign(leaves[i].output_parent, leaves[i].index, leaves[i].converted);
            if (result != FVIZ_OK) goto done;
        }
    }
    result = fviz_algorithm_set_output_data(algorithm, request->requested_output_port, (FVizDataObject*)output);
    if (result == FVIZ_OK)
    {
...
FVizResult fviz_composite_geometry_filter_create(FVizCompositeGeometryFilter** out_filter)
{
    ...
    *out_filter = NULL;
    filter = (FVizCompositeGeometryFilter*)fviz_internal_object_allocate(sizeof(*filter),
                                                                         &g_fviz_composite_geometry_class, NULL);
    ...
    callbacks.process_request = fviz_composite_geometry_process_request;
    callbacks.get_state_mtime = fviz_composite_geometry_state_mtime;
    callbacks.state_object = (FVizObject*)filter;
    if (fviz_algorithm_create(1u, 1u, &callbacks, filter, &filter->algorithm) != FVIZ_OK ||
        fviz_algorithm_configure_input_port(filter->algorithm, 0u, FVIZ_TYPE_MULTI_BLOCK_DATA_SET, FVIZ_FALSE,
                                            FVIZ_FALSE) != FVIZ_OK ||
...
{
    return f != NULL && in != NULL ? fviz_algorithm_set_input_data(f->algorithm, 0u, (FVizDataObject*)in)
...
{
    return f != NULL && in != NULL ? fviz_algorithm_set_input_connection(f->algorithm, 0u, in)
                                   : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizAlgorithm* fviz_composite_geometry_filter_algorithm(FVizCompositeGeometryFilter* f)
{
    return f ? f->algorithm : NULL;
}

FVizAlgorithmOutput* fviz_composite_geometry_filter_output_port(FVizCompositeGeometryFilter* f)
{
    return f ? fviz_algorithm_output_port(f->algorithm, 0u) : NULL;
}

FVizMultiBlockDataSet* fviz_composite_geometry_filter_output(FVizCompositeGeometryFilter* f)
{
    return f ? (FVizMultiBlockDataSet*)fviz_algorithm_output_data(f->algorithm, 0u) : NULL;
}

FVizResult fviz_composite_geometry_filter_update(FVizCompositeGeometryFilter* f)
{
    return f ? fviz_algorithm_update(f->algorithm) : FVIZ_ERROR_INVALID_ARGUMENT;
}

FVizResult fviz_composite_geometry_filter_set_parallel_enabled(FVizCompositeGeometryFilter* filter, FVizBool enabled)
{
...
FVizBool fviz_composite_geometry_filter_parallel_enabled(const FVizCompositeGeometryFilter* filter)
{
    return filter != NULL ? filter->parallel_enabled : FVIZ_FALSE;
}

FVizResult fviz_composite_geometry_filter_set_parallel_threshold(FVizCompositeGeometryFilter* filter,
                                                                 FVizSize leaf_count)
{
...
FVizSize fviz_composite_geometry_filter_parallel_threshold(const FVizCompositeGeometryFilter* filter)
{
    return filter != NULL ? filter->parallel_threshold : 0u;
}

FVizResult fviz_composite_geometry_filter_set_cache_byte_capacity(FVizCompositeGeometryFilter* filter,
...
FVizSize fviz_composite_geometry_filter_cache_byte_capacity(const FVizCompositeGeometryFilter* filter)
{
