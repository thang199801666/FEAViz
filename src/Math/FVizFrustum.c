...
FVizFrustum fviz_frustum_from_view_projection(FVizMat4 m)
{
    ...
    result.planes[FVIZ_FRUSTUM_LEFT] = fviz_frustum_plane(
        m.m[3] + m.m[0], m.m[7] + m.m[4], m.m[11] + m.m[8], m.m[15] + m.m[12], &valid);
    result.planes[FVIZ_FRUSTUM_RIGHT] = fviz_frustum_plane(
        m.m[3] - m.m[0], m.m[7] - m.m[4], m.m[11] - m.m[8], m.m[15] - m.m[12], &valid);
    result.planes[FVIZ_FRUSTUM_BOTTOM] = fviz_frustum_plane(
        m.m[3] + m.m[1], m.m[7] + m.m[5], m.m[11] + m.m[9], m.m[15] + m.m[13], &valid);
    result.planes[FVIZ_FRUSTUM_TOP] = fviz_frustum_plane(
        m.m[3] - m.m[1], m.m[7] - m.m[5], m.m[11] - m.m[9], m.m[15] - m.m[13], &valid);
    result.planes[FVIZ_FRUSTUM_NEAR] = fviz_frustum_plane(
        m.m[3] + m.m[2], m.m[7] + m.m[6], m.m[11] + m.m[10], m.m[15] + m.m[14], &valid);
    result.planes[FVIZ_FRUSTUM_FAR] = fviz_frustum_plane(
        m.m[3] - m.m[2], m.m[7] - m.m[6], m.m[11] - m.m[10], m.m[15] - m.m[14], &valid);
    result.valid = valid;
    return result;
}
...
FVizBool fviz_frustum_contains_point(const FVizFrustum* frustum, FVizVec3 point)
{
    ...
    for (i = 0u; i < FVIZ_FRUSTUM_PLANE_COUNT; ++i)
        if (fviz_plane_distance_to_point(frustum->planes[i], point) < 0.0f)
            return FVIZ_FALSE;
    return FVIZ_TRUE;
}
...
FVizBool fviz_frustum_intersects_bounds(const FVizFrustum* frustum, FVizBounds bounds)
{
    ...
    for (i = 0u; i < FVIZ_FRUSTUM_PLANE_COUNT; ++i)
    {
        ...
        const FVizVec3 positive = fviz_vec3(
        ...
        if (fviz_plane_distance_to_point(plane, positive) < 0.0f)
            return FVIZ_FALSE;
    }
    return FVIZ_TRUE;
}
