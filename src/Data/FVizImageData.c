...
static const FVizObjectClass g_fviz_image_data_class = {
    FVIZ_TYPE_IMAGE_DATA, "FVizImageData", &g_fviz_data_object_class,
    fviz_image_data_destroy, fviz_image_data_mtime
};

static FVizBool fviz_image_data_dependency_modified(
    FVizObject* caller, FVizEventId event_id, void* call_data, void* client_data)
{
    ...
    if (image != NULL && image->dependency_suppression == 0u)
...
FVizResult fviz_image_data_create(FVizImageData** out_image)
{
    ...
    *out_image = NULL;
    ...
    if (fviz_object_add_observer(
            (FVizObject*)image->data_set, FVIZ_EVENT_MODIFIED, 0.0f,
            fviz_image_data_dependency_modified, image, &image->data_set_modified_tag) != FVIZ_OK)
    {
        fviz_release(image);
        return fviz_last_error_code();
    }
    image->extent[0]=0; image->extent[1]=-1;
    image->extent[2]=0; image->extent[3]=-1;
    image->extent[4]=0; image->extent[5]=-1;
...
void fviz_image_data_clear(FVizImageData* image)
{
    ...
    changed = (memcmp(image->extent, empty, sizeof(empty)) != 0 ||
        ...
        fviz_attribute_set_count(fviz_data_set_field_data(image->data_set)) != 0u) ? FVIZ_TRUE : FVIZ_FALSE;
...
FVizResult fviz_image_data_set_extent(FVizImageData* image, const int64_t extent[6])
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "image extent conflicts with existing attribute tuple counts");
...
void fviz_image_data_extent(const FVizImageData* image, int64_t out_extent[6])
{
    ...
    if (image == NULL) { const int64_t e[6]={0,-1,0,-1,0,-1}; (void)memcpy(out_extent,e,sizeof(e)); return; }
...
void fviz_image_data_origin(const FVizImageData* image, double out_origin[3])
{
    ...
    else (void)memcpy(out_origin, image->origin, 3u*sizeof(double));
}

FVizResult fviz_image_data_set_spacing(FVizImageData* image, const double spacing[3])
{
    if (image == NULL || fviz_image_finite3(spacing) == FVIZ_FALSE || spacing[0] == 0.0 || spacing[1] == 0.0 || spacing[2] == 0.0)
    {
...
void fviz_image_data_spacing(const FVizImageData* image, double out_spacing[3])
{
    ...
    else (void)memcpy(out_spacing, image->spacing, 3u*sizeof(double));
}
...
void fviz_image_data_direction(const FVizImageData* image, double out_direction[9])
{
    ...
    else (void)memcpy(out_direction, image->direction, 9u*sizeof(double));
}

FVizSize fviz_image_data_point_count(const FVizImageData* image) { return image != NULL ? fviz_data_set_point_count(image->data_set) : 0u; }
FVizSize fviz_image_data_cell_count(const FVizImageData* image) { return image != NULL ? fviz_data_set_cell_count(image->data_set) : 0u; }

FVizCellType fviz_image_data_cell_type(const FVizImageData* image)
{
    switch (fviz_image_data_dimension(image))
    {
        case 0u: return FVIZ_CELL_VERTEX;
...
FVizResult fviz_image_data_index_to_physical(
    const FVizImageData* image, const double index[3], double out_physical[3])
{
    double scaled[3];
    ...
    scaled[0]=index[0]*image->spacing[0]; scaled[1]=index[1]*image->spacing[1]; scaled[2]=index[2]*image->spacing[2];
    out_physical[0]=image->origin[0]+image->direction[0]*scaled[0]+image->direction[1]*scaled[1]+image->direction[2]*scaled[2];
    out_physical[1]=image->origin[1]+image->direction[3]*scaled[0]+image->direction[4]*scaled[1]+image->direction[5]*scaled[2];
    out_physical[2]=image->origin[2]+image->direction[6]*scaled[0]+image->direction[7]*scaled[1]+image->direction[8]*scaled[2];
    return FVIZ_OK;
}

FVizResult fviz_image_data_physical_to_continuous_index(
    const FVizImageData* image, const double physical[3], double out_index[3])
{
    double delta[3];
    double local[3];
    ...
    delta[0]=physical[0]-image->origin[0]; delta[1]=physical[1]-image->origin[1]; delta[2]=physical[2]-image->origin[2];
    local[0]=image->inverse_direction[0]*delta[0]+image->inverse_direction[1]*delta[1]+image->inverse_direction[2]*delta[2];
    local[1]=image->inverse_direction[3]*delta[0]+image->inverse_direction[4]*delta[1]+image->inverse_direction[5]*delta[2];
    local[2]=image->inverse_direction[6]*delta[0]+image->inverse_direction[7]*delta[1]+image->inverse_direction[8]*delta[2];
    out_index[0]=local[0]/image->spacing[0]; out_index[1]=local[1]/image->spacing[1]; out_index[2]=local[2]/image->spacing[2];
    return FVIZ_OK;
}

FVizResult fviz_image_data_point_id(
    const FVizImageData* image, int64_t i, int64_t j, int64_t k, FVizId* out_point_id)
{
    FVizSize dims[3];
    FVizSize x,y,z,plane,id;
    if (out_point_id != NULL) *out_point_id = FVIZ_INVALID_ID;
    if (image == NULL || out_point_id == NULL || i < image->extent[0] || i > image->extent[1] ||
        j < image->extent[2] || j > image->extent[3] || k < image->extent[4] || k > image->extent[5])
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "structured point index is outside image extent");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    fviz_image_data_dimensions(image,dims);
    x=(FVizSize)(i-image->extent[0]); y=(FVizSize)(j-image->extent[2]); z=(FVizSize)(k-image->extent[4]);
    if (fviz_size_multiply(dims[0],dims[1],&plane)!=FVIZ_OK || fviz_size_multiply(z,plane,&id)!=FVIZ_OK) return FVIZ_ERROR_OVERFLOW;
    id += y*dims[0] + x;
...
FVizResult fviz_image_data_point(const FVizImageData* image, FVizId point_id, FVizVec3* out_point)
{
    ...
    index[0]=(double)ijk[0]; index[1]=(double)ijk[1]; index[2]=(double)ijk[2];
    if (fviz_image_data_index_to_physical(image,index,physical)!=FVIZ_OK) return fviz_last_error_code();
    out_point->x=(float)physical[0]; out_point->y=(float)physical[1]; out_point->z=(float)physical[2];
    return FVIZ_OK;
}
...
static void fviz_image_cell_dimensions(const FVizImageData* image, FVizSize cell_dims[3])
{
    ...
    uint32_t axis;
    fviz_image_data_dimensions(image, dims);
    for (axis = 0u; axis < 3u; ++axis) cell_dims[axis] = dims[axis] > 1u ? dims[axis] - 1u : (dims[axis] == 1u ? 1u : 0u);
}

FVizResult fviz_image_data_cell_id(
    const FVizImageData* image, int64_t i, int64_t j, int64_t k, FVizId* out_cell_id)
{
    FVizSize dims[3];
...
FVizResult fviz_image_data_cell_point_ids(
    const FVizImageData* image, FVizId cell_id, FVizId out_point_ids[8], uint32_t* out_point_count)
{
    int64_t base[3];
    FVizSize dims[3];
    uint32_t active[3];
    ...
    for (corner=0u; corner<count; ++corner)
    {
        ...
        else if (active_count >= 2u && (corner & 3u) == 3u) logical_corner = corner - 1u;
...
FVizBounds fviz_image_data_bounds(const FVizImageData* image)
{
    ...
    {
        double index[3] = {
            (double)image->extent[(corner&1u)?1u:0u],
        ...
        p.x=(float)physical[0]; p.y=(float)physical[1]; p.z=(float)physical[2];
        fviz_bounds_include_point(&bounds,p);
    }
    return bounds;
}

static FVizResult fviz_image_sample_array_impl(
    const FVizImageData* image, const FVizDataArray* array, const double physical[3], uint32_t component, double* out_value)
{
    double index[3];
    FVizSize dims[3];
    int64_t lower[3];
    ...
    if (image==NULL || array==NULL || physical==NULL || out_value==NULL || component>=fviz_data_array_components(array) ||
        fviz_data_array_tuple_count(array)!=fviz_image_data_point_count(image))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "image sampling arguments or array association are invalid");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
...
FVizResult fviz_image_data_sample_point_array(
    const FVizImageData* image, const char* array_name, const double physical[3], uint32_t component, double* out_value)
{
...
FVizResult fviz_image_data_sample_active_scalars(
    const FVizImageData* image, const double physical[3], uint32_t component, double* out_value)
{
    const FVizDataArray* array = image != NULL
        ? fviz_attribute_set_const_active(fviz_image_data_const_point_data(image),FVIZ_ATTRIBUTE_SCALARS) : NULL;
    if (array==NULL)
    {
...
FVizAttributeSet* fviz_image_data_point_data(FVizImageData* image) { return image != NULL ? fviz_data_set_point_data(image->data_set) : NULL; }
FVizAttributeSet* fviz_image_data_cell_data(FVizImageData* image) { return image != NULL ? fviz_data_set_cell_data(image->data_set) : NULL; }
FVizAttributeSet* fviz_image_data_field_data(FVizImageData* image) { return image != NULL ? fviz_data_set_field_data(image->data_set) : NULL; }
const FVizAttributeSet* fviz_image_data_const_point_data(const FVizImageData* image) { return image != NULL ? fviz_data_set_point_data(image->data_set) : NULL; }
const FVizAttributeSet* fviz_image_data_const_cell_data(const FVizImageData* image) { return image != NULL ? fviz_data_set_cell_data(image->data_set) : NULL; }
const FVizAttributeSet* fviz_image_data_const_field_data(const FVizImageData* image) { return image != NULL ? fviz_data_set_field_data(image->data_set) : NULL; }

static FVizResult fviz_image_allocate_scalars(
    FVizImageData* image, FVizAttributeSet* attributes, FVizSize tuples,
    const char* name, FVizDataType type, uint32_t components, FVizDataArray** out_array)
{
    ...
    ++image->dependency_suppression;
    if (fviz_data_array_create(type,components,&array)!=FVIZ_OK || fviz_data_array_resize(array,tuples)!=FVIZ_OK ||
...
FVizResult fviz_image_data_allocate_point_scalars(
    FVizImageData* image, const char* name, FVizDataType type, uint32_t components, FVizDataArray** out_array)
{
    return fviz_image_allocate_scalars(image,fviz_image_data_point_data(image),fviz_image_data_point_count(image),name,type,components,out_array);
}

FVizResult fviz_image_data_allocate_cell_scalars(
    FVizImageData* image, const char* name, FVizDataType type, uint32_t components, FVizDataArray** out_array)
{
    return fviz_image_allocate_scalars(image,fviz_image_data_cell_data(image),fviz_image_data_cell_count(image),name,type,components,out_array);
}
