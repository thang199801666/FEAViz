...
static const uint8_t* fviz_unstructured_grid_ghost_cell_flags(const FVizUnstructuredGrid* grid)
{
    ...
    ghosts = fviz_attribute_set_const_get(fviz_unstructured_grid_cell_data((FVizUnstructuredGrid*)grid),
                                          FVIZ_GHOST_ARRAY_NAME);
    if (ghosts == NULL || fviz_data_array_type(ghosts) != FVIZ_DATA_UINT8 || fviz_data_array_components(ghosts) != 1u ||
...
static FVizBool fviz_unstructured_grid_cell_is_render_ghost(const uint8_t* ghost_flags, FVizSize cell_id)
{
    return ghost_flags != NULL && (ghost_flags[cell_id] & (uint8_t)(FVIZ_GHOST_DUPLICATE | FVIZ_GHOST_HIDDEN)) != 0u
               ? FVIZ_TRUE
               : FVIZ_FALSE;
}
...
static const FVizObjectClass g_fviz_unstructured_grid_class = {
    FVIZ_TYPE_UNSTRUCTURED_GRID, "FVizUnstructuredGrid", &g_fviz_data_object_class, fviz_unstructured_grid_destroy,
    fviz_unstructured_grid_mtime};

static FVizBool fviz_unstructured_grid_dependency_modified(FVizObject* caller, FVizEventId event_id, void* call_data,
                                                           void* client_data)
{
...
static FVizResult fviz_unstructured_grid_observe_dependency(FVizUnstructuredGrid* grid, FVizObject* dependency,
                                                            FVizObserverTag* out_tag)
{
    ...
    *out_tag = FVIZ_OBSERVER_TAG_INVALID;
    ...
    return fviz_object_add_observer(dependency, FVIZ_EVENT_MODIFIED, 0.0f, fviz_unstructured_grid_dependency_modified,
                                    grid, out_tag);
}
...
FVizResult fviz_unstructured_grid_create(FVizUnstructuredGrid** out_grid)
{
    ...
    *out_grid = NULL;
    grid = (FVizUnstructuredGrid*)fviz_internal_object_allocate(sizeof(FVizUnstructuredGrid),
                                                                &g_fviz_unstructured_grid_class, NULL);
    if (grid == NULL) return fviz_last_error_code();
    if (fviz_points_create(&grid->points) != FVIZ_OK || fviz_cell_array_create(&grid->cells) != FVIZ_OK ||
    ...
    if (fviz_unstructured_grid_observe_dependency(grid, (FVizObject*)grid->points, &grid->points_modified_tag) !=
            FVIZ_OK ||
        fviz_unstructured_grid_observe_dependency(grid, (FVizObject*)grid->cells, &grid->cells_modified_tag) !=
            FVIZ_OK ||
        fviz_unstructured_grid_observe_dependency(grid, (FVizObject*)grid->data_set, &grid->data_set_modified_tag) !=
            FVIZ_OK)
    {
        fviz_release(grid);
        return fviz_last_error_code();
...
void fviz_unstructured_grid_clear(FVizUnstructuredGrid* grid)
{
    FVizBool changed;
    if (grid == NULL) return;
    changed = (fviz_points_count(grid->points) != 0u || fviz_cell_array_count(grid->cells) != 0u ||
...
FVizPoints* fviz_unstructured_grid_points(FVizUnstructuredGrid* grid)
{
    return grid != NULL ? grid->points : NULL;
}

FVizCellArray* fviz_unstructured_grid_cells(FVizUnstructuredGrid* grid)
{
    return grid != NULL ? grid->cells : NULL;
}

FVizResult fviz_unstructured_grid_reserve(FVizUnstructuredGrid* grid, FVizSize point_capacity, FVizSize cell_capacity,
...
FVizResult fviz_unstructured_grid_add_points(FVizUnstructuredGrid* grid, const FVizVec3* points, FVizSize point_count,
...
FVizResult fviz_unstructured_grid_add_points_ids(FVizUnstructuredGrid* grid, const FVizVec3* points,
...
FVizResult fviz_unstructured_grid_add_cells_fixed(FVizUnstructuredGrid* grid, FVizCellType type,
...
FVizResult fviz_unstructured_grid_add_cells_fixed_ids(FVizUnstructuredGrid* grid, FVizCellType type,
                                                      FVizSize points_per_cell, FVizSize cell_count,
...
FVizResult fviz_unstructured_grid_add_cell_ids(FVizUnstructuredGrid* grid, FVizCellType type, FVizSize point_count,
                                               const FVizId* point_ids)
{
    return fviz_unstructured_grid_add_cells_fixed_ids(grid, type, point_count, 1u, point_ids);
}

FVizResult fviz_unstructured_grid_add_cell(FVizUnstructuredGrid* grid, FVizCellType type, FVizSize point_count,
                                           const uint32_t* point_ids)
{
    return fviz_unstructured_grid_add_cells_fixed(grid, type, point_count, 1u, point_ids);
}

FVizAttributeSet* fviz_unstructured_grid_point_data(FVizUnstructuredGrid* grid)
{
    return grid != NULL ? fviz_data_set_point_data(grid->data_set) : NULL;
}

FVizAttributeSet* fviz_unstructured_grid_cell_data(FVizUnstructuredGrid* grid)
{
    return grid != NULL ? fviz_data_set_cell_data(grid->data_set) : NULL;
}

FVizAttributeSet* fviz_unstructured_grid_field_data(FVizUnstructuredGrid* grid)
{
    return grid != NULL ? fviz_data_set_field_data(grid->data_set) : NULL;
}

FVizSize fviz_unstructured_grid_point_count(const FVizUnstructuredGrid* grid)
{
    return grid != NULL ? fviz_points_count(grid->points) : 0u;
}

FVizSize fviz_unstructured_grid_cell_count(const FVizUnstructuredGrid* grid)
{
    return grid != NULL ? fviz_cell_array_count(grid->cells) : 0u;
}

FVizBounds fviz_unstructured_grid_bounds(const FVizUnstructuredGrid* grid)
{
...
static FVizBool fviz_surface_definition(FVizCellType type, FVizSurfaceDefinition* definition)
{
    ...
    if (traits.dimension != 3u || traits.face_count == 0u || traits.face_count > 6u) return FVIZ_FALSE;
...
static uint64_t fviz_surface_face_hash(const uint32_t* sorted, uint32_t count)
{
    ...
    uint64_t y = ((uint64_t)sorted[2] << 32u) | (uint64_t)(count == 4u ? sorted[3] : UINT32_C(0xffffffff));
...
static FVizResult fviz_surface_face_table_create(FVizSize face_capacity, FVizSurfaceFaceTable* table)
{
...
static FVizResult fviz_surface_face_table_insert(FVizSurfaceFaceTable* table, const uint32_t* ids, uint32_t count,
                                                 FVizId source_cell, FVizId source_face)
{
    ...
    while (probe_count < table->slot_count)
    {
        ...
        else
        {
            ...
            if (face->count == count && memcmp(face->sorted, sorted, (FVizSize)count * sizeof(uint32_t)) == 0)
            {
                ++face->occurrences;
                return FVIZ_OK;
            }
...
static FVizResult fviz_copy_filtered_attribute_set(const FVizAttributeSet* source, FVizAttributeSet* destination,
                                                   const uint8_t* selected, FVizSize source_count,
                                                   FVizSize selected_count)
{
    ...
    for (array_index = 0u; array_index < fviz_attribute_set_count(source); ++array_index)
    {
        ...
        if (fviz_data_array_create(fviz_data_array_type(input), fviz_data_array_components(input), &output) !=
                FVIZ_OK ||
...
FVizResult fviz_unstructured_grid_shallow_copy(const FVizUnstructuredGrid* source, FVizUnstructuredGrid** out_copy)
{
...
static FVizResult fviz_append_source_cell(FVizUnstructuredGrid* destination, const FVizCellArray* source_cells,
                                          FVizSize cell_id)
{
    FVizCellView view;
    ...
    if (view.id_storage == FVIZ_ID_STORAGE_UINT64)
        return fviz_unstructured_grid_add_cell_ids(destination, view.type, view.point_count,
                                                   (const FVizId*)view.point_ids);
...
static FVizResult fviz_clone_topology(const FVizUnstructuredGrid* grid, FVizUnstructuredGrid** out_result)
{
    ...
    if (fviz_unstructured_grid_reserve(result, fviz_points_count(grid->points), fviz_cell_array_count(grid->cells),
...
FVizResult fviz_unstructured_grid_transform(const FVizUnstructuredGrid* grid, const FVizTransform* transform,
                                            FVizUnstructuredGrid** out_grid)
{
    ...
    *out_grid = NULL;
    ...
    if (fviz_parallel_for(0u, fviz_points_count(result->points), 256u, fviz_transform_point_range, &context) != FVIZ_OK)
    {
        fviz_release(result);
        return fviz_last_error_code();
    }
...
static void fviz_warp_point_range(FVizSize begin, FVizSize end, void* user_data)
{
    ...
    for (i = begin; i < end; ++i)
    {
        ...
        context->displaced[i] = fviz_vec3(context->points[i].x + (float)(vx * context->scale),
                                          context->points[i].y + (float)(vy * context->scale),
                                          context->points[i].z + (float)(vz * context->scale));
    }
}

FVizResult fviz_unstructured_grid_warp_by_array(const FVizUnstructuredGrid* grid, const FVizDataArray* vectors,
                                                double scale, FVizUnstructuredGrid** out_grid)
{
    ...
    if (grid == NULL || vectors == NULL || out_grid == NULL || !isfinite(scale))
    {
        ...
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "warp requires grid, vector array, finite scale and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    *out_grid = NULL;
...
FVizResult fviz_unstructured_grid_warp_by_vector(const FVizUnstructuredGrid* grid, const char* vector_name,
                                                 double scale, FVizUnstructuredGrid** out_grid)
{
...
static double fviz_cell_to_point_component(const FVizCellToPointRangeContext* context, FVizSize cell_id,
                                           uint32_t component)
{
    ...
    {
        case FVIZ_DATA_INT8:
            return ((const int8_t*)context->source_values)[index];
        case FVIZ_DATA_UINT8:
            return ((const uint8_t*)context->source_values)[index];
        case FVIZ_DATA_INT16:
            return ((const int16_t*)context->source_values)[index];
        case FVIZ_DATA_UINT16:
            return ((const uint16_t*)context->source_values)[index];
        case FVIZ_DATA_INT32:
            return ((const int32_t*)context->source_values)[index];
        case FVIZ_DATA_UINT32:
            return ((const uint32_t*)context->source_values)[index];
        case FVIZ_DATA_INT64:
            return (double)((const int64_t*)context->source_values)[index];
        case FVIZ_DATA_UINT64:
            return (double)((const uint64_t*)context->source_values)[index];
        case FVIZ_DATA_FLOAT32:
            return ((const float*)context->source_values)[index];
        case FVIZ_DATA_FLOAT64:
            return ((const double*)context->source_values)[index];
        default:
            return 0.0;
    }
}
    ...
    for (point_id = begin; point_id < end; ++point_id)
    {
        ...
        for (component = 0u; component < context->components; ++component)
        {
            ...
            if (context->output_type == FVIZ_DATA_FLOAT32)
                ((float*)context->output_values)[output_base + component] =
                    degree != 0u ? (float)(sum / (double)degree) : 0.0f;
            else
...
static FVizBool fviz_cell_to_point_should_average(const FVizAttributeSet* cell_data, const char* name)
{
    const char* global_ids;
    if (cell_data == NULL || name == NULL) return FVIZ_FALSE;
    if (strcmp(name, FVIZ_GHOST_ARRAY_NAME) == 0 || strcmp(name, FVIZ_GHOST_LEVEL_ARRAY_NAME) == 0 ||
...
static FVizResult fviz_unstructured_grid_cell_data_to_point_data_serial(const FVizUnstructuredGrid* grid,
                                                                        FVizUnstructuredGrid** out_grid)
{
    FVizUnstructuredGrid* result = NULL;
    ...
    for (i = 0u; i < fviz_attribute_set_count(cell_data); ++i)
    {
        ...
        uint32_t component;
        if (array == NULL || name == NULL || components == 0u || fviz_data_array_tuple_count(array) != cell_count ||
        ...
        for (cell_id = 0u; cell_id < cell_count; ++cell_id)
        {
            ...
            for (component = 0u; component < components; ++component)
            {
                ...
                {
                    fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                            "cell attribute type is unsupported for point averaging");
                    goto field_fail;
                }
            }
            ...
            for (point_id = 0u; point_id < point_count; ++point_id)
            {
                ...
                for (component = 0u; component < components; ++component)
                    values[base + component] =
                        counts[point_id] != 0u ? (float)(sums[base + component] / divisor) : 0.0f;
            }
        }
        else
        {
...
FVizResult fviz_unstructured_grid_cell_data_to_point_data(const FVizUnstructuredGrid* grid,
                                                          FVizUnstructuredGrid** out_grid)
{
    FVizUnstructuredGrid* result = NULL;
    FVizAttributeSet* cell_data;
    ...
    *out_grid = NULL;
    if (fviz_unstructured_grid_validate(grid) != FVIZ_OK) return fviz_last_error_code();
    {
        ...
        if (averaging_fields <= 1u) return fviz_unstructured_grid_cell_data_to_point_data_serial(grid, out_grid);
    }
    /* Geometry/topology and existing attributes are immutable for this operation;
     * share them instead of copying an entire large FEA mesh.  The copy owns an
     * independent DataSet/AttributeSet so newly generated point arrays cannot
...
FVizResult fviz_unstructured_grid_threshold_cells(const FVizUnstructuredGrid* grid, const char* scalar_name,
                                                  double minimum, double maximum, FVizUnstructuredGrid** out_grid)
{
    ...
    *out_grid = NULL;
    ...
    if (scalar == NULL || fviz_data_array_components(scalar) != 1u || fviz_data_array_tuple_count(scalar) != cell_count)
    {
    ...
    points = fviz_points_data(grid->points);
    if (fviz_unstructured_grid_reserve(result, fviz_points_count(grid->points), selected_count,
                                       fviz_cell_array_connectivity_size(grid->cells)) != FVIZ_OK ||
        fviz_unstructured_grid_add_points_ids(result, points, fviz_points_count(grid->points), NULL) != FVIZ_OK)
        goto fail;
    for (i = 0u; i < cell_count; ++i)
    ...
    if (fviz_copy_attribute_set(fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)grid),
        ...
        fviz_copy_attribute_set(fviz_unstructured_grid_field_data((FVizUnstructuredGrid*)grid),
                                fviz_unstructured_grid_field_data(result)) != FVIZ_OK ||
        fviz_copy_filtered_attribute_set(fviz_unstructured_grid_cell_data((FVizUnstructuredGrid*)grid),
                                         fviz_unstructured_grid_cell_data(result), selected, cell_count,
                                         selected_count) != FVIZ_OK)
        goto fail;
    fviz_free(selected);
    *out_grid = result;
    return FVIZ_OK;
...
static FVizResult fviz_high_order_surface_table_create(FVizSize max_faces, FVizHighOrderSurfaceTable* table)
{
    ...
    if (max_faces > ((FVizSize)-1) / 10u * 7u)
    {
        fviz_free(table->faces);
        (void)memset(table, 0, sizeof(*table));
        return FVIZ_ERROR_OVERFLOW;
    ...
    if (fviz_size_multiply(slots, sizeof(*table->slots), &slot_bytes) != FVIZ_OK)
    {
        fviz_free(table->faces);
        (void)memset(table, 0, sizeof(*table));
        return fviz_last_error_code();
...
static FVizBool fviz_high_order_face_equal(const FVizHighOrderSurfaceFace* face, const uint32_t* sorted, uint32_t count)
{
    ...
    for (i = 0u; i < count; ++i)
        if (face->sorted[i] != sorted[i]) return FVIZ_FALSE;
    return FVIZ_TRUE;
}

static FVizResult fviz_high_order_surface_table_insert(FVizHighOrderSurfaceTable* table, const uint32_t* ids,
                                                       uint32_t count, FVizId cell_id, FVizId face_id)
{
    uint32_t sorted[9];
    uint64_t hash;
    FVizSize slot;
    ...
    for (;;)
    {
        ...
        if (stored == 0u)
        {
            ...
            face->occurrences = 1u;
            face->source_cell = cell_id;
            face->source_face = face_id;
            for (i = 0u; i < count; ++i)
            {
                face->ids[i] = ids[i];
                face->sorted[i] = sorted[i];
...
static FVizResult fviz_append_surface_triangle(FVizPolyData* surface, FVizDataArray* original_cell_ids,
                                               FVizDataArray* original_face_ids, uint32_t a, uint32_t b, uint32_t c,
                                               FVizId source_cell, FVizId source_face)
{
...
static FVizResult fviz_tessellate_high_order_face(FVizPolyData* surface, FVizDataArray* original_cell_ids,
                                                  FVizDataArray* original_face_ids,
                                                  const FVizHighOrderSurfaceFace* face)
{
    const uint32_t* p = face->ids;
    if (face->count == 3u)
        return fviz_append_surface_triangle(surface, original_cell_ids, original_face_ids, p[0], p[1], p[2],
                                            face->source_cell, face->source_face);
    if (face->count == 4u)
    {
        if (fviz_append_surface_triangle(surface, original_cell_ids, original_face_ids, p[0], p[1], p[2],
                                         face->source_cell, face->source_face) != FVIZ_OK)
            return fviz_last_error_code();
        return fviz_append_surface_triangle(surface, original_cell_ids, original_face_ids, p[0], p[2], p[3],
                                            face->source_cell, face->source_face);
    }
    if (face->count == 6u) /* quadratic triangle: c0,c1,c2,m01,m12,m20 */
    {
        ...
        uint32_t i;
        for (i = 0u; i < 4u; ++i)
            if (fviz_append_surface_triangle(surface, original_cell_ids, original_face_ids, p[tris[i][0]],
                                             p[tris[i][1]], p[tris[i][2]], face->source_cell,
                                             face->source_face) != FVIZ_OK)
                return fviz_last_error_code();
        return FVIZ_OK;
    }
    if (face->count == 8u) /* quadratic quad: fan around ordered curved boundary */
    {
        ...
        uint32_t i;
        for (i = 1u; i + 1u < 8u; ++i)
            if (fviz_append_surface_triangle(surface, original_cell_ids, original_face_ids, p[ring[0]], p[ring[i]],
                                             p[ring[i + 1u]], face->source_cell, face->source_face) != FVIZ_OK)
                return fviz_last_error_code();
        return FVIZ_OK;
    }
    if (face->count == 9u) /* biquadratic quad: center node 8 */
    {
        ...
        uint32_t i;
        for (i = 0u; i < 8u; ++i)
            if (fviz_append_surface_triangle(surface, original_cell_ids, original_face_ids, p[8], p[ring[i]],
                                             p[ring[(i + 1u) & 7u]], face->source_cell, face->source_face) != FVIZ_OK)
                return fviz_last_error_code();
        return FVIZ_OK;
    }
...
{
    return type == FVIZ_CELL_TETRA || type == FVIZ_CELL_HEXAHEDRON || type == FVIZ_CELL_WEDGE ||
                   type == FVIZ_CELL_PYRAMID
               ? FVIZ_TRUE
               : FVIZ_FALSE;
}

static FVizResult fviz_extract_surface_generic_volume(const FVizUnstructuredGrid* grid, FVizPolyData** out_surface,
                                                      FVizBool with_scalars)
{
    ...
    *out_surface = NULL;
    ...
    if (fviz_high_order_surface_table_create(max_faces, &table) != FVIZ_OK ||
            ...
            for (j = 0u; j < count; ++j)
            {
                ...
                if (id > (FVizId)UINT32_MAX)
                {
                    fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                            "high-order surface point ID exceeds renderable UINT32 range");
                    goto fail;
                }
                ids[j] = (uint32_t)id;
            }
            if (fviz_high_order_surface_table_insert(&table, ids, count, (FVizId)cell_id, (FVizId)face_id) != FVIZ_OK)
                goto fail;
        }
    }
    points = fviz_points_data(grid->points);
    ...
    if (fviz_attribute_set_add(fviz_poly_data_point_data(surface), "FVizOriginalPointIds", original_point_ids) !=
        FVIZ_OK)
        goto fail;
    for (i = 0u; i < table.face_count; ++i)
        if (table.faces[i].occurrences == 1u &&
            fviz_unstructured_grid_cell_is_render_ghost(ghost_flags, (FVizSize)table.faces[i].source_cell) ==
                FVIZ_FALSE &&
            fviz_tessellate_high_order_face(surface, original_cell_ids, original_face_ids, &table.faces[i]) != FVIZ_OK)
            goto fail;
    if (fviz_attribute_set_add(fviz_poly_data_cell_data(surface), "FVizOriginalCellIds", original_cell_ids) !=
            FVIZ_OK ||
        fviz_attribute_set_add(fviz_poly_data_cell_data(surface), "FVizOriginalFaceIds", original_face_ids) != FVIZ_OK)
        goto fail;
...
static FVizResult fviz_extract_surface_internal(const FVizUnstructuredGrid* grid, FVizPolyData** out_surface,
                                                FVizBool with_scalars)
{
    ...
    FVizBool fast_hex32 = fviz_cell_array_id_storage(grid != NULL ? grid->cells : NULL) == FVIZ_ID_STORAGE_UINT32
                              ? FVIZ_TRUE
                              : FVIZ_FALSE;
    ...
    *out_surface = NULL;
    ...
    for (cell_id = 0u; cell_id < fviz_cell_array_count(grid->cells); ++cell_id)
    {
        ...
        if (cell_type != FVIZ_CELL_HEXAHEDRON || fviz_cell_array_point_count(grid->cells, cell_id) != 8u)
            fast_hex32 = FVIZ_FALSE;
    ...
    {
        static const uint8_t face_local[6][4] = {{0, 3, 2, 1}, {4, 5, 6, 7}, {0, 1, 5, 4},
        ...
        for (cell_id = 0u; cell_id < fviz_cell_array_count(grid->cells); ++cell_id)
        {
            ...
            {
                uint32_t ids[4];
                ids[0] = cell[face_local[face_id][0]];
                ids[1] = cell[face_local[face_id][1]];
                ids[2] = cell[face_local[face_id][2]];
                ids[3] = cell[face_local[face_id][3]];
    ...
    else
    {
        for (cell_id = 0u; cell_id < fviz_cell_array_count(grid->cells); ++cell_id)
        {
            ...
            for (face_id = 0u; face_id < definition.face_count; ++face_id)
            {
                ...
                if (fviz_surface_face_table_insert(&face_table, ids, count, (FVizId)cell_id, (FVizId)face_id) !=
                    FVIZ_OK)
                    goto fail;
            }
        }
...

    {
        ...
        for (i = 0u; i < face_table.face_count; ++i)
        {
            ...
            if (face->occurrences != 1u ||
                fviz_unstructured_grid_cell_is_render_ghost(ghost_flags, (FVizSize)face->source_cell) != FVIZ_FALSE)
                continue;
        ...
        if (fviz_size_multiply(boundary_triangle_count, 3u, &triangle_index_count) != FVIZ_OK) goto fail;
...
static FVizResult fviz_geometry_append_provenance(FVizDataArray* cell_ids, FVizDataArray* face_ids, FVizId cell_id,
                                                  FVizId face_id, FVizSize repeat)
{
    FVizSize i;
    for (i = 0u; i < repeat; ++i)
...
static FVizResult fviz_geometry_emit_line(FVizPolyData* out, FVizDataArray* cell_ids, FVizDataArray* face_ids,
...
static FVizResult fviz_geometry_emit_triangle(FVizPolyData* out, FVizDataArray* cell_ids, FVizDataArray* face_ids,
                                              uint32_t a, uint32_t b, uint32_t c, FVizId source_cell,
                                              FVizId source_face)
{
...
static FVizResult fviz_geometry_emit_surface_cell(FVizPolyData* out, FVizDataArray* cell_ids, FVizDataArray* face_ids,
                                                  const FVizCellView* view, FVizId source_cell)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                "geometry extraction surface cell exceeds supported point count");
        return FVIZ_ERROR_NOT_SUPPORTED;
    }
    for (i = 0u; i < view->point_count; ++i)
    {
        ...
        {
            fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                    "geometry extraction point ID exceeds renderable UINT32 range");
            return FVIZ_ERROR_NOT_SUPPORTED;
        }
    ...
    switch (view->type)
    {
        ...
        case FVIZ_CELL_QUAD:
            if (fviz_geometry_emit_triangle(out, cell_ids, face_ids, p[0], p[1], p[2], source_cell, no_face) != FVIZ_OK)
                return fviz_last_error_code();
        ...
        case FVIZ_CELL_QUADRATIC_TRIANGLE:
            {
                ...
                uint32_t k;
                for (k = 0u; k < 4u; ++k)
                    if (fviz_geometry_emit_triangle(out, cell_ids, face_ids, p[t[k][0]], p[t[k][1]], p[t[k][2]],
                                                    source_cell, no_face) != FVIZ_OK)
                        return fviz_last_error_code();
                return FVIZ_OK;
            }
        case FVIZ_CELL_QUADRATIC_QUAD:
            {
                ...
                uint32_t k;
                for (k = 1u; k + 1u < 8u; ++k)
                    if (fviz_geometry_emit_triangle(out, cell_ids, face_ids, p[ring[0]], p[ring[k]], p[ring[k + 1u]],
                                                    source_cell, no_face) != FVIZ_OK)
                        return fviz_last_error_code();
                return FVIZ_OK;
            }
        case FVIZ_CELL_BIQUADRATIC_QUAD:
            {
                ...
                uint32_t k;
                for (k = 0u; k < 8u; ++k)
                    if (fviz_geometry_emit_triangle(out, cell_ids, face_ids, p[8], p[ring[k]], p[ring[(k + 1u) & 7u]],
                                                    source_cell, no_face) != FVIZ_OK)
                        return fviz_last_error_code();
                return FVIZ_OK;
            }
        case FVIZ_CELL_POLYGON:
            ...
            for (i = 1u; i + 1u < view->point_count; ++i)
                if (fviz_geometry_emit_triangle(out, cell_ids, face_ids, p[0], p[i], p[i + 1u], source_cell, no_face) !=
                    FVIZ_OK)
                    return fviz_last_error_code();
            return FVIZ_OK;
        case FVIZ_CELL_TRIANGLE_STRIP:
            ...
            for (i = 0u; i + 2u < view->point_count; ++i)
            {
                ...
                if (fviz_geometry_emit_triangle(out, cell_ids, face_ids, a, b, p[i + 2u], source_cell, no_face) !=
                    FVIZ_OK)
                    return fviz_last_error_code();
            }
            return FVIZ_OK;
...
static FVizResult fviz_geometry_copy_point_and_field_data(const FVizUnstructuredGrid* grid, FVizPolyData* out)
{
    ...
    for (i = 0u; i < fviz_attribute_set_count(source); ++i)
    {
        ...
        if (name != NULL && array != NULL &&
            fviz_data_array_tuple_count(array) == fviz_unstructured_grid_point_count(grid))
    ...
    for (i = 0u; i < fviz_attribute_set_count(source); ++i)
    {
        const char* name = fviz_attribute_set_name_at(source, i);
        FVizDataArray* array = fviz_attribute_set_array_at(source, i);
        if (name != NULL && array != NULL && fviz_attribute_set_add(dest, name, array) != FVIZ_OK)
            return fviz_last_error_code();
    }
    return FVIZ_OK;
}

static FVizResult fviz_geometry_copy_cell_data(const FVizUnstructuredGrid* grid, FVizPolyData* out,
                                               const FVizDataArray* provenance)
{
    ...
    for (a = 0u; a < fviz_attribute_set_count(source); ++a)
    {
        ...
        FVizAttributeRole role;
        if (name == NULL || src == NULL || strcmp(name, "FVizOriginalCellIds") == 0 ||
            fviz_data_array_tuple_count(src) != source_count)
            continue;
        if (fviz_data_array_create(fviz_data_array_type(src), fviz_data_array_components(src), &dst) != FVIZ_OK ||
            fviz_data_array_reserve(dst, fviz_data_array_tuple_count(provenance)) != FVIZ_OK)
            goto field_fail;
        for (t = 0u; t < fviz_data_array_tuple_count(provenance); ++t)
        {
            ...
            if (id >= source_count ||
                fviz_data_array_append_tuple(dst, fviz_data_array_const_tuple(src, id)) != FVIZ_OK)
                goto field_fail;
        }
...
FVizResult fviz_unstructured_grid_extract_geometry(const FVizUnstructuredGrid* grid, FVizPolyData** out_geometry)
{
    ...
    *out_geometry = NULL;
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                "mixed geometry extraction requires renderable UINT32 point IDs");
        return FVIZ_ERROR_NOT_SUPPORTED;
    }
    for (cell_id = 0u; cell_id < fviz_unstructured_grid_cell_count(grid); ++cell_id)
    {
        ...
        {
            fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                    "mixed geometry extraction encountered an unsupported cell type");
            return FVIZ_ERROR_NOT_SUPPORTED;
        }
    ...
    if (has_volume != FVIZ_FALSE && fviz_high_order_surface_table_create(max_faces, &volume_faces) != FVIZ_OK)
        goto fail;
    if (fviz_poly_data_create(&out) != FVIZ_OK ||
        ...
        fviz_data_array_create(FVIZ_DATA_UINT64, 1u, &original_point_ids) != FVIZ_OK ||
        fviz_data_array_resize(original_point_ids, fviz_unstructured_grid_point_count(grid)) != FVIZ_OK)
        goto fail;
    points = fviz_points_data(grid->points);
    ...
    if (fviz_attribute_set_add(fviz_poly_data_point_data(out), "FVizOriginalPointIds", original_point_ids) != FVIZ_OK ||
        fviz_geometry_copy_point_and_field_data(grid, out) != FVIZ_OK)
        goto fail;
    ...
    for (cell_id = 0u; cell_id < fviz_unstructured_grid_cell_count(grid); ++cell_id)
    {
        ...
        for (j = 0u; j < view.point_count; ++j)
        {
            ...
            if (id > (FVizId)UINT32_MAX || fviz_poly_data_add_vertex(out, (uint32_t)id) != FVIZ_OK ||
                fviz_geometry_append_provenance(cell_ids, face_ids, (FVizId)cell_id, none, 1u) != FVIZ_OK)
                goto fail;
        }
    }
    /* 1D cells next. */
    for (cell_id = 0u; cell_id < fviz_unstructured_grid_cell_count(grid); ++cell_id)
    {
        ...
        {
            const FVizId a = fviz_cell_view_point_id(&view, 0u), b = fviz_cell_view_point_id(&view, 1u),
                         m = fviz_cell_view_point_id(&view, 2u);
            if (a > UINT32_MAX || b > UINT32_MAX || m > UINT32_MAX ||
                fviz_geometry_emit_line(out, cell_ids, face_ids, (uint32_t)a, (uint32_t)m, (FVizId)cell_id) !=
                    FVIZ_OK ||
                fviz_geometry_emit_line(out, cell_ids, face_ids, (uint32_t)m, (uint32_t)b, (FVizId)cell_id) != FVIZ_OK)
                goto fail;
        }
        else
        {
            for (j = 0u; j + 1u < view.point_count; ++j)
            {
                ...
                if (a > UINT32_MAX || b > UINT32_MAX ||
                    fviz_geometry_emit_line(out, cell_ids, face_ids, (uint32_t)a, (uint32_t)b, (FVizId)cell_id) !=
                        FVIZ_OK)
                    goto fail;
            }
        }
    ...
    for (cell_id = 0u; cell_id < fviz_unstructured_grid_cell_count(grid); ++cell_id)
    {
        ...
        if (fviz_cell_type_traits(view.type).dimension == 2u &&
            fviz_geometry_emit_surface_cell(out, cell_ids, face_ids, &view, (FVizId)cell_id) != FVIZ_OK)
            goto fail;
    }
    ...
    if (has_volume != FVIZ_FALSE)
    {
        for (cell_id = 0u; cell_id < fviz_unstructured_grid_cell_count(grid); ++cell_id)
        {
            ...
            for (face = 0u; face < traits.face_count; ++face)
            {
                ...
                for (j = 0u; j < count; ++j)
                {
                    ...
                    {
                        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                                "volume boundary point ID exceeds UINT32 range");
                        goto fail;
                    }
                    ids[j] = (uint32_t)id;
                }
                if (fviz_high_order_surface_table_insert(&volume_faces, ids, count, (FVizId)cell_id, (FVizId)face) !=
                    FVIZ_OK)
                    goto fail;
            }
        }
        for (i = 0u; i < volume_faces.face_count; ++i)
        {
            ...
            FVizSize before;
            if (f->occurrences != 1u ||
                fviz_unstructured_grid_cell_is_render_ghost(ghost_flags, (FVizSize)f->source_cell) != FVIZ_FALSE)
                continue;
    ...
    if (fviz_attribute_set_add(fviz_poly_data_cell_data(out), "FVizOriginalCellIds", cell_ids) != FVIZ_OK ||
        ...
        fviz_geometry_copy_cell_data(grid, out, cell_ids) != FVIZ_OK)
        goto fail;
    /* Preserve transitive provenance when the input itself was already derived. */
    {
        const FVizDataArray* upstream = fviz_attribute_set_const_get(
            fviz_unstructured_grid_cell_data((FVizUnstructuredGrid*)grid), "FVizOriginalCellIds");
        if (upstream != NULL && fviz_data_array_tuple_count(upstream) == fviz_unstructured_grid_cell_count(grid))
        {
            FVizDataArray* remapped = NULL;
            if (fviz_data_array_create(fviz_data_array_type(upstream), fviz_data_array_components(upstream),
                                       &remapped) != FVIZ_OK ||
                fviz_data_array_reserve(remapped, fviz_data_array_tuple_count(cell_ids)) != FVIZ_OK)
            {
            ...
            for (i = 0u; i < fviz_data_array_tuple_count(cell_ids); ++i)
            {
                ...
                if (fviz_data_array_get_component(cell_ids, i, 0u, &id) != FVIZ_OK ||
                    ...
                    fviz_data_array_append_tuple(remapped, fviz_data_array_const_tuple(upstream, (FVizSize)id)) !=
                        FVIZ_OK)
                {
                    fviz_release(remapped);
                    goto fail;
            ...
            if (fviz_attribute_set_add(fviz_poly_data_cell_data(out), "FVizOriginalCellIds", remapped) != FVIZ_OK)
            {
                fviz_release(remapped);
                goto fail;
            }
    ...
    fviz_release(original_point_ids);
    fviz_high_order_surface_table_destroy(&volume_faces);
    *out_geometry = out;
    return FVIZ_OK;
fail:
    fviz_release(cell_ids);
...
static const uint32_t g_fviz_edges_tetra[6][2] = {{0, 1}, {0, 2}, {0, 3}, {1, 2}, {1, 3}, {2, 3}};
static const uint32_t g_fviz_edges_hex[12][2] = {{0, 1}, {1, 2}, {2, 3}, {3, 0}, {4, 5}, {5, 6},
                                                 {6, 7}, {7, 4}, {0, 4}, {1, 5}, {2, 6}, {3, 7}};
static const uint32_t g_fviz_edges_wedge[9][2] = {{0, 1}, {1, 2}, {2, 0}, {3, 4}, {4, 5},
                                                  {5, 3}, {0, 3}, {1, 4}, {2, 5}};
static const uint32_t g_fviz_edges_pyramid[8][2] = {{0, 1}, {1, 2}, {2, 3}, {3, 0}, {0, 4}, {1, 4}, {2, 4}, {3, 4}};
...
static const uint32_t* fviz_cell_edge_table(FVizCellType type, uint32_t* out_edge_count)
{
    switch (type)
    {
        case FVIZ_CELL_TETRA:
            *out_edge_count = 6u;
            return &g_fviz_edges_tetra[0][0];
        case FVIZ_CELL_HEXAHEDRON:
            *out_edge_count = 12u;
            return &g_fviz_edges_hex[0][0];
        case FVIZ_CELL_WEDGE:
            *out_edge_count = 9u;
            return &g_fviz_edges_wedge[0][0];
        case FVIZ_CELL_PYRAMID:
            *out_edge_count = 8u;
            return &g_fviz_edges_pyramid[0][0];
        default:
            *out_edge_count = 0u;
            return NULL;
    }
...
static void fviz_slice_classify_range(FVizSize begin, FVizSize end, void* user_data)
{
    ...
    for (cell_id = begin; cell_id < end; ++cell_id)
    {
        ...
        if (fviz_unstructured_grid_cell_is_render_ghost(context->ghost_flags, cell_id) != FVIZ_FALSE) continue;
        FVizCellView view;
        ...
        FVizSize i;
        if (edges == NULL || count > 8u || fviz_cell_array_cell_view(context->grid->cells, cell_id, &view) != FVIZ_OK)
            continue;
        for (i = 0u; i < count; ++i)
        {
        ...
        context->intersects[cell_id] = positive != FVIZ_FALSE && negative != FVIZ_FALSE ? FVIZ_TRUE : FVIZ_FALSE;
    }
}

/* A single intersecting cell's slice polygon, computed independently so the
 * construction phase can run in parallel. The value pool holds
 * polygon_count * field_count floats per cell. */
typedef struct FVizSlicePolygonResult
{
    FVizSize polygon_count;
    FVizBool overflow;
    FVizVec3 positions[FVIZ_SLICE_MAX_VERTICES];
    FVizSize order[FVIZ_SLICE_MAX_VERTICES];
    float* values;
} FVizSlicePolygonResult;

typedef struct FVizSliceConstructionContext
{
    const FVizUnstructuredGrid* grid;
    const FVizVec3* points;
    const uint8_t* intersects;
    const FVizSliceField* fields;
    FVizSize field_count;
    FVizPlane plane;
    FVizVec3 u_basis;
    FVizVec3 v_basis;
    FVizSlicePolygonResult* results;
} FVizSliceConstructionContext;

static void fviz_slice_construct_range(FVizSize begin, FVizSize end, void* user_data)
{
    FVizSliceConstructionContext* context = (FVizSliceConstructionContext*)user_data;
    const FVizUnstructuredGrid* grid = context->grid;
    const FVizVec3* points = context->points;
    FVizSize cell_id;
    for (cell_id = begin; cell_id < end; ++cell_id)
    {
        const FVizCellType type = fviz_cell_array_type(grid->cells, cell_id);
        ...
        FVizSize k;
        FVizSlicePolygonResult* result = &context->results[cell_id];
        result->polygon_count = 0u;
        result->overflow = FVIZ_FALSE;
        if (context->intersects != NULL && context->intersects[cell_id] == FVIZ_FALSE) continue;
        if (edges == NULL || cell_point_count > 8u || fviz_cell_array_cell_view(grid->cells, cell_id, &view) != FVIZ_OK)
            continue;
        for (k = 0u; k < cell_point_count; ++k)
            cell_ids[k] = fviz_cell_view_point_id(&view, k);

        for (k = 0u; k < cell_point_count; ++k)
        {
            distances[k] = fviz_plane_distance_to_point(context->plane, points[(FVizSize)cell_ids[k]]);
        }
        {
        ...
        for (k = 0u; k < cell_point_count; ++k)
        {
            ...
            {
                FVizSize f;
                if (polygon_count >= FVIZ_SLICE_MAX_VERTICES)
                {
                    result->overflow = FVIZ_TRUE;
                    return;
                }
                polygon_positions[polygon_count] = points[(FVizSize)cell_ids[k]];
                for (f = 0u; f < context->field_count; ++f)
                {
                    double value = 0.0;
                    (void)fviz_scalar_value(context->fields[f].array, (FVizSize)cell_ids[k], &value);
                    result->values[polygon_count * context->field_count + f] = (float)value;
                }
                ++polygon_count;
            }
        }
        for (k = 0u; k < edge_count; ++k)
        {
            ...
            polygon_positions[polygon_count] = fviz_vec3_add(
                points[(FVizSize)a], fviz_vec3_scale(fviz_vec3_sub(points[(FVizSize)b], points[(FVizSize)a]), t));
            for (f = 0u; f < context->field_count; ++f)
            {
                double va = 0.0;
                double vb = 0.0;
                (void)fviz_scalar_value(context->fields[f].array, (FVizSize)a, &va);
                (void)fviz_scalar_value(context->fields[f].array, (FVizSize)b, &vb);
                result->values[polygon_count * context->field_count + f] = (float)(va + (vb - va) * t);
            }
            ++polygon_count;
        }
        ...
        if (polygon_count >= 3u)
        {
            ...
            for (i = 0u; i < polygon_count; ++i)
            {
                ...
                angles[i] = atan2f(fviz_vec3_dot(context->v_basis, rel), fviz_vec3_dot(context->u_basis, rel));
                result->order[i] = i;
            }
            fviz_slice_sort_angles(angles, result->order, polygon_count);
            for (i = 0u; i < polygon_count; ++i)
            {
                result->positions[i] = polygon_positions[result->order[i]];
            }
        }
    }
}

FVizResult fviz_unstructured_grid_slice(const FVizUnstructuredGrid* grid, FVizPlane plane, FVizPolyData** out_slice)
{
    ...
    *out_slice = NULL;
    ...
    if (fviz_cell_array_count(grid->cells) > 0u)
    {
        ...
        if (fviz_parallel_for(0u, fviz_cell_array_count(grid->cells), 64u, fviz_slice_classify_range,
                              &classification) != FVIZ_OK)
            goto fail;
    }
    ...
    {
        FVizSliceConstructionContext construction;
        FVizSlicePolygonResult* results = NULL;
        float* value_pool = NULL;
        FVizSize cell_count = fviz_cell_array_count(grid->cells);
        if (cell_count > 0u)
        {
            results = (FVizSlicePolygonResult*)fviz_alloc(cell_count * sizeof(FVizSlicePolygonResult));
            if (results == NULL) goto fail;
            (void)memset(results, 0, cell_count * sizeof(FVizSlicePolygonResult));
            if (field_count > 0u)
            {
                FVizSize pool_bytes = 0u;
                if (fviz_size_multiply(cell_count * FVIZ_SLICE_MAX_VERTICES, field_count * sizeof(float), &pool_bytes) !=
                    FVIZ_OK)
                {
                    fviz_free(results);
                    goto fail;
                }
                value_pool = (float*)fviz_alloc(pool_bytes);
                if (value_pool == NULL)
                {
                    fviz_free(results);
                    goto fail;
                ...
                for (cell_id = 0u; cell_id < cell_count; ++cell_id)
                    results[cell_id].values = value_pool + cell_id * FVIZ_SLICE_MAX_VERTICES * field_count;
            }
            construction.grid = grid;
            construction.points = points;
            construction.intersects = intersects;
            construction.fields = (const FVizSliceField*)fviz_array_const_data(fields);
            construction.field_count = field_count;
            construction.plane = plane;
            construction.u_basis = u_basis;
            construction.v_basis = v_basis;
            construction.results = results;
            if (fviz_parallel_for(0u, cell_count, 32u, fviz_slice_construct_range, &construction) != FVIZ_OK)
            {
                fviz_free(value_pool);
                fviz_free(results);
                goto fail;
            }
            for (cell_id = 0u; cell_id < cell_count; ++cell_id)
            {
                FVizSlicePolygonResult* result = &results[cell_id];
                FVizSize i;
                if (result->overflow != FVIZ_FALSE)
                {
                    fviz_free(value_pool);
                    fviz_free(results);
                    goto free_values_fail;
                }
                if (result->polygon_count < 3u) continue;
                {
                    uint32_t new_ids[FVIZ_SLICE_MAX_VERTICES];
                    for (i = 0u; i < result->polygon_count; ++i)
                    {
                        if (fviz_poly_data_add_point(slice, result->positions[i], &new_ids[i]) != FVIZ_OK)
                        {
                            fviz_free(value_pool);
                            fviz_free(results);
                            goto free_values_fail;
                    ...
                    for (i = 1u; i + 1u < result->polygon_count; ++i)
                    {
                    ...
                    {
                        FVizSize v;
                        for (v = 0u; v < result->polygon_count; ++v)
                        {
                            const float value = result->values[result->order[v] * field_count + field_id];
                            if (fviz_data_array_append_tuple(outputs[field_id], &value) != FVIZ_OK)
                            {
                                fviz_free(value_pool);
                                fviz_free(results);
                                goto free_values_fail;
...
FVizResult fviz_unstructured_grid_gradient(const FVizUnstructuredGrid* grid, const char* scalar_array_name,
                                           const char* output_name, FVizUnstructuredGrid** out_grid)
{
    ...
    FVizSize i;
    if (out_grid != NULL) *out_grid = NULL;
    if (grid == NULL || scalar_array_name == NULL || scalar_array_name[0] == '\0' || output_name == NULL ||
        output_name[0] == '\0' || out_grid == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
    ...
    scalars =
        fviz_attribute_set_const_get(fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)grid), scalar_array_name);
    if (scalars == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_FOUND, "gradient input array was not found on point data");
    ...
    if (fviz_cell_links_build(grid->cells, point_count, &links) != FVIZ_OK) return fviz_last_error_code();
    ...
    for (i = 0u; i < cell_count; ++i)
    {
        ...
        for (row = 0u; row < 4u; ++row)
            for (col = 0u; col < 4u; ++col)
            {
                double sum = 0.0;
                for (s = 0u; s < point_count_in_cell; ++s)
                    sum += basis[s * 4u + row] * basis[s * 4u + col];
        ...
        for (r = 0u; r < 4u; ++r)
        {
            ...
            if (pivot != r)
            {
                for (c = 0u; c < 4u; ++c)
                {
                    const double t = a[r * 4u + c];
                    a[r * 4u + c] = a[pivot * 4u + c];
                    a[pivot * 4u + c] = t;
                }
                for (c = 0u; c < components; ++c)
                {
                    const double t = b[r * components + c];
                    b[r * components + c] = b[pivot * components + c];
                    b[pivot * components + c] = t;
                }
            }
            pivot_value = a[r * 4u + r];
            if (fabs(pivot_value) < 1.0e-12) continue;
            for (c = r; c < 4u; ++c)
                a[r * 4u + c] /= pivot_value;
            for (c = 0u; c < components; ++c)
                b[r * components + c] /= pivot_value;
            for (rr = r + 1u; rr < 4u; ++rr)
            {
                const double factor = a[rr * 4u + r];
                if (factor == 0.0) continue;
                for (c = r; c < 4u; ++c)
                    a[rr * 4u + c] -= factor * a[r * 4u + c];
                for (c = 0u; c < components; ++c)
                    b[rr * components + c] -= factor * b[r * components + c];
            }
        }
        for (r = 4u; r-- > 0u;)
            for (c = 0u; c < components; ++c)
            {
                ...
                for (rr = r + 1u; rr < 4u; ++rr)
                    sum -= a[r * 4u + rr] * coefficients[rr * components + c];
    ...
    for (i = 0u; i < point_count; ++i)
    {
        ...
        for (c = 0u; c < count_for_point; ++c)
        {
            ...
            {
                double value = 0.0;
                if (fviz_data_array_get_component(cell_gradients, (FVizSize)incident_cells[c], (uint32_t)k, &value) ==
                    FVIZ_OK)
                    sum[k] += value;
            }
        }
...
FVizResult fviz_unstructured_grid_cell_derivatives(const FVizUnstructuredGrid* grid, const char* scalar_array_name,
                                                   const char* output_name, FVizUnstructuredGrid** out_grid)
{
    const FVizDataArray* scalars;
    const FVizVec3* points;
    ...
    if (scalars == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_FOUND, "cell derivatives input array was not found on point data");
        return FVIZ_ERROR_NOT_FOUND;
    }
    point_count = fviz_unstructured_grid_point_count(grid);
    cell_count = fviz_unstructured_grid_cell_count(grid);
    ...
    if (fviz_unstructured_grid_shallow_copy(grid, &result) != FVIZ_OK) goto fail;
    if (fviz_attribute_set_add(fviz_unstructured_grid_cell_data(result), output_name, derivatives) != FVIZ_OK)
        goto fail;
...
FVizResult fviz_unstructured_grid_warp_scalar(const FVizUnstructuredGrid* grid, const char* scalar_array_name,
                                              ...
                                              FVizUnstructuredGrid** out_grid)
{
    ...
    if (scalars == NULL)
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_FOUND, "warp scalar input array was not found on point data");
        return FVIZ_ERROR_NOT_FOUND;
    }
    if (fviz_data_array_tuple_count(scalars) != fviz_unstructured_grid_point_count(grid))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "warp scalar input must have one tuple per point");
        return FVIZ_ERROR_INVALID_STATE;
    }
    if (normal_array_name != NULL && normal_array_name[0] != '\0')
        normals = fviz_attribute_set_const_get(fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)grid),
                                               normal_array_name);
    point_count = fviz_unstructured_grid_point_count(grid);
    points = fviz_points_data(grid->points);
    ...
    for (i = 0u; i < point_count; ++i)
    {
        ...
        if (normals != NULL && i < fviz_data_array_tuple_count(normals) && fviz_data_array_components(normals) >= 3u)
        {
            ...
            displaced[i] = fviz_vec3_add(
                points[i], fviz_vec3_scale(fviz_vec3((float)nx, (float)ny, (float)nz), (float)(value * scale)));
        }
        else
        {
            displaced[i] = fviz_vec3_add(points[i], fviz_vec3(0.0f, 0.0f, (float)(value * scale)));
        }
    }
    /* Build a fresh grid so the source points are not mutated. */
    if (fviz_unstructured_grid_create(&result) != FVIZ_OK ||
        fviz_unstructured_grid_reserve(result, point_count, fviz_cell_array_count(grid->cells),
                                       fviz_cell_array_connectivity_size(grid->cells)) != FVIZ_OK ||
...
FVizResult fviz_unstructured_grid_stream_tracer(const FVizUnstructuredGrid* grid, const char* vector_array_name,
                                                const FVizVec3* seed_points, FVizSize seed_count, double step_length,
                                                FVizSize max_steps, FVizPolyData** out_lines)
{
    const FVizDataArray* vectors;
    ...
    vectors =
        fviz_attribute_set_const_get(fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)grid), vector_array_name);
    if (vectors == NULL || fviz_data_array_components(vectors) < 3u)
    {
        fviz_internal_set_error(FVIZ_ERROR_NOT_FOUND, "stream tracer vector array was not found on point data");
        return FVIZ_ERROR_NOT_FOUND;
    }
    if (fviz_point_locator_create(&locator) != FVIZ_OK || fviz_point_locator_set_grid(locator, grid) != FVIZ_OK ||
        ...
        for (step = 0u; step < max_steps; ++step)
        {
            ...
            if (speed < 1.0e-9f) break;
            {
                ...
                /* RK2 midpoint refinement. */
                {
                    ...
                    next = fviz_vec3_add(
                        point, fviz_vec3_scale(fviz_vec3_scale(fviz_vec3_add(v, vm), 0.5f), (float)step_length));
                }
                {
                    uint32_t index = 0u;
                    if (fviz_poly_data_add_point(output, next, &index) != FVIZ_OK) goto fail;
                    if (fviz_poly_data_add_line(output, (uint32_t)(first_point_index + step), index) != FVIZ_OK)
                        goto fail;
                }
                point = next;
            }
...
FVizResult fviz_unstructured_grid_cutter(const FVizUnstructuredGrid* grid, const FVizPlane* planes,
                                         FVizSize plane_count, FVizPolyData** out_cut)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "cutter requires a grid, planes, and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
 ...
 * are above. */
FVizResult fviz_unstructured_grid_iso_surface(const FVizUnstructuredGrid* grid, const char* scalar_array_name,
                                              double iso_value, FVizPolyData** out_surface)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "iso-surface requires a grid, scalar name, and output");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    scalars =
        fviz_attribute_set_const_get(fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)grid), scalar_array_name);
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_STATE, "iso-surface scalar must be a one-component point array");
        return FVIZ_ERROR_INVALID_STATE;
    }
    ...
    for (i = 0u; i < cell_count; ++i)
    {
        ...
        if (view.point_count == 4u)
        {
            ...
            FVizSize e;
            for (e = 0u; e < 4u; ++e)
                ids[e] = fviz_cell_view_point_id(&view, e);
            {
                double v[4];
                FVizVec3 p[4];
                uint32_t above[4];
                ...
                if (above_count == 1u || above_count == 3u)
                {
                        ...
                        if (e != iso) other[o++] = (uint32_t)e;
                    {
                        ...
                        for (t = 0u; t < 3u; ++t)
                        {
                            ...
                            FVizVec3 interp =
                                fviz_vec3_add(p[a], fviz_vec3_scale(fviz_vec3_sub(p[b], p[a]), (float)t_frac));
                            uint32_t pid = 0u;
        ...
        else if (view.point_count == 8u)
        {
             ...
             * march each. */
            FVizId ids[8];
            const uint32_t tetra_split[5][4] = {{0, 1, 2, 5}, {0, 2, 3, 7}, {0, 4, 5, 7}, {2, 5, 6, 7}, {0, 2, 5, 7}};
            FVizSize t;
            FVizSize e;
            for (e = 0u; e < 8u; ++e)
                ids[e] = fviz_cell_view_point_id(&view, e);
    ...
    if (fviz_attribute_set_add(fviz_poly_data_point_data(output), scalar_array_name, scalar_values) != FVIZ_OK ||
        fviz_poly_data_validate(output) != FVIZ_OK)
        goto fail;
