...
static const FVizAttributeSet* fviz_field_stats_attributes(const FVizDataObject* data,FVizFieldAssociation association)
{
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_POLY_DATA))
        return association==FVIZ_FIELD_POINT_DATA?
            fviz_poly_data_const_point_data((const FVizPolyData*)data):fviz_poly_data_const_cell_data((const FVizPolyData*)data);
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_UNSTRUCTURED_GRID))
        return association==FVIZ_FIELD_POINT_DATA?
            fviz_unstructured_grid_point_data((FVizUnstructuredGrid*)data):fviz_unstructured_grid_cell_data((FVizUnstructuredGrid*)data);
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_IMAGE_DATA))
        return association==FVIZ_FIELD_POINT_DATA?
            fviz_image_data_const_point_data((const FVizImageData*)data):fviz_image_data_const_cell_data((const FVizImageData*)data);
...
{
    const FVizCellArray* groups[4]={fviz_poly_data_verts(poly),fviz_poly_data_lines(poly),fviz_poly_data_polys(poly),fviz_poly_data_strips(poly)};
    FVizSize group;
    ...
    for (group=0u;group<4u;++group)
    {
        ...
        if (cell_id<count)
        {
            ...
            FVizSize i;
            if (fviz_cell_array_cell_view(groups[group],cell_id,&view)!=FVIZ_OK || view.point_count==0u) return FVIZ_FALSE;
...
static FVizBool fviz_field_stats_position(const FVizDataObject* data,FVizFieldAssociation association,FVizSize tuple,FVizVec3* out)
{
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_POLY_DATA))
    {
        ...
        if (association==FVIZ_FIELD_POINT_DATA)
        {
            ...
            *out=fviz_poly_data_points(poly)[tuple]; return FVIZ_TRUE;
        }
    ...
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_UNSTRUCTURED_GRID))
    {
        ...
        if (association==FVIZ_FIELD_POINT_DATA)
        {
            ...
            *out=points[tuple]; return FVIZ_TRUE;
        }
        else
        {
            FVizCellView view;
            ...
            if (tuple>=fviz_unstructured_grid_cell_count(grid) ||
                fviz_cell_array_cell_view(fviz_unstructured_grid_cells((FVizUnstructuredGrid*)grid),tuple,&view)!=FVIZ_OK ||
                view.point_count==0u) return FVIZ_FALSE;
            for (i=0u;i<view.point_count;++i)
            {
                const FVizId id=fviz_cell_view_point_id(&view,i);
            ...
            *out=fviz_vec3_scale(sum,1.0f/(float)view.point_count); return FVIZ_TRUE;
        }
    }
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_STRUCTURED_GRID))
    {
        ...
        {
            FVizId ids[8]; uint32_t count=0u,i; FVizVec3 sum=fviz_vec3(0,0,0);
            if (fviz_structured_grid_cell_point_ids(grid,(FVizId)tuple,ids,&count)!=FVIZ_OK || count==0u) return FVIZ_FALSE;
            for (i=0u;i<count;++i) { FVizVec3 p; if (fviz_structured_grid_point(grid,ids[i],&p)!=FVIZ_OK) return FVIZ_FALSE; sum=fviz_vec3_add(sum,p); }
            *out=fviz_vec3_scale(sum,1.0f/(float)count); return FVIZ_TRUE;
        }
    }
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_RECTILINEAR_GRID))
    {
        ...
        {
            FVizId ids[8]; uint32_t count=0u,i; FVizVec3 sum=fviz_vec3(0,0,0);
            if (fviz_rectilinear_grid_cell_point_ids(grid,(FVizId)tuple,ids,&count)!=FVIZ_OK || count==0u) return FVIZ_FALSE;
            for (i=0u;i<count;++i) { FVizVec3 p; if (fviz_rectilinear_grid_point(grid,ids[i],&p)!=FVIZ_OK) return FVIZ_FALSE; sum=fviz_vec3_add(sum,p); }
            *out=fviz_vec3_scale(sum,1.0f/(float)count); return FVIZ_TRUE;
        }
    }
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_IMAGE_DATA))
    {
        ...
        {
            FVizId ids[8]; uint32_t count=0u,i; FVizVec3 sum=fviz_vec3(0,0,0);
            if (fviz_image_data_cell_point_ids(image,(FVizId)tuple,ids,&count)!=FVIZ_OK || count==0u) return FVIZ_FALSE;
            for (i=0u;i<count;++i) { FVizVec3 p; if (fviz_image_data_point(image,ids[i],&p)!=FVIZ_OK) return FVIZ_FALSE; sum=fviz_vec3_add(sum,p); }
            *out=fviz_vec3_scale(sum,1.0f/(float)count); return FVIZ_TRUE;
        }
    }
    return FVIZ_FALSE;
...
static void fviz_field_stats_set_extremum(
    FVizFieldExtremum* e,double value,FVizSize tuple,const FVizDataObject* leaf,const FVizFieldStatsTraversal* t)
{
    FVizVec3 position;
    memset(e,0,sizeof(*e));
    e->value=value; e->tuple_id=tuple; e->leaf=leaf;
    e->has_partition=t->has_partition; e->partition_index=t->partition_index;
    e->has_time=t->has_time; e->temporal_index=t->temporal_index; e->time=t->time;
    if (fviz_field_stats_position(leaf,t->options.association,tuple,&position)!=FVIZ_FALSE)
    { e->world_position=position; e->has_world_position=FVIZ_TRUE; }
}
...
static double fviz_field_stats_raw_component(
    const FVizFieldStatsRangeContext* context, FVizSize tuple, uint32_t component)
{
    const FVizSize index = tuple * (FVizSize)context->components + (FVizSize)component;
    switch (context->type)
    {
        case FVIZ_DATA_INT8: return ((const int8_t*)context->values)[index];
        case FVIZ_DATA_UINT8: return ((const uint8_t*)context->values)[index];
        case FVIZ_DATA_INT16: return ((const int16_t*)context->values)[index];
        case FVIZ_DATA_UINT16: return ((const uint16_t*)context->values)[index];
        case FVIZ_DATA_INT32: return ((const int32_t*)context->values)[index];
        case FVIZ_DATA_UINT32: return ((const uint32_t*)context->values)[index];
        case FVIZ_DATA_INT64: return (double)((const int64_t*)context->values)[index];
        case FVIZ_DATA_UINT64: return (double)((const uint64_t*)context->values)[index];
        case FVIZ_DATA_FLOAT32: return ((const float*)context->values)[index];
        case FVIZ_DATA_FLOAT64: return ((const double*)context->values)[index];
        default: return NAN;
    }
}
...
static FVizResult fviz_field_stats_visit_leaf(const FVizDataObject* data,FVizFieldStatsTraversal* t)
{
    ...
    partial_count = tuple_count / FVIZ_FIELD_STATS_PARALLEL_GRAIN +
        (tuple_count % FVIZ_FIELD_STATS_PARALLEL_GRAIN != 0u ? 1u : 0u);
    if (fviz_size_multiply(partial_count, sizeof(*partials), &partial_bytes) != FVIZ_OK)
    ...
    if (t->options.ignore_ghosts != FVIZ_FALSE)
    {
        ...
        if (ghosts != NULL && fviz_data_array_type(ghosts) == FVIZ_DATA_UINT8 &&
            ...
            fviz_data_array_tuple_count(ghosts) == tuple_count)
    ...
    if (partial_count == 1u)
        fviz_field_stats_range(0u, tuple_count, &context);
    else if (fviz_parallel_for(
                 0u, tuple_count, FVIZ_FIELD_STATS_PARALLEL_GRAIN,
                 fviz_field_stats_range, &context) != FVIZ_OK)
    {
        fviz_free(partials);
        return fviz_last_error_code();
    }
    ...
    for (i = 0u; i < partial_count; ++i)
    {
        ...
        else
        {
            ...
            if (t->compute_moments != FVIZ_FALSE)
            {
                ...
                leaf.m2 += partial->m2 + delta * delta *
                    (double)left_count * (double)right_count / (double)merged_count;
                leaf.mean_square =
                    (leaf.mean_square * (double)left_count +
                     partial->mean_square * (double)right_count) / (double)merged_count;
            }
        ...
        fviz_field_stats_set_extremum(&t->stats->maximum, leaf.maximum, leaf.maximum_tuple, data, t);
    {
        ...
        if (t->compute_moments != FVIZ_FALSE)
        {
            ...
            else
            {
                ...
                t->aggregate_m2 += leaf.m2 + delta * delta *
                    (double)left_count * (double)right_count / (double)merged_count;
                t->aggregate_mean_square =
                    (t->aggregate_mean_square * (double)left_count +
                     leaf.mean_square * (double)right_count) / (double)merged_count;
            }
        }
...
static FVizResult fviz_field_stats_multiblock_leaf(
...
{
    (void)parent; (void)index; (void)name; (void)depth;
...
static FVizResult fviz_field_stats_visit(const FVizDataObject* data,FVizFieldStatsTraversal* t)
{
    ...
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_MULTI_BLOCK_DATA_SET))
        return fviz_multi_block_data_set_visit(
            (const FVizMultiBlockDataSet*)data, FVIZ_TRUE, FVIZ_TRUE,
    ...
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_PARTITIONED_DATA_SET))
    {
        ...
        FVizSize i; const FVizBool old=t->has_partition; const FVizSize old_index=t->partition_index;
        for (i=0u;i<fviz_partitioned_data_set_count(partitions);++i)
        {
            t->has_partition=FVIZ_TRUE; t->partition_index=i;
            if (fviz_field_stats_visit(fviz_partitioned_data_set_const_partition(partitions,i),t)!=FVIZ_OK) return fviz_last_error_code();
        }
        t->has_partition=old; t->partition_index=old_index; return FVIZ_OK;
    }
    if (fviz_object_is_type((const FVizObject*)data,FVIZ_TYPE_TEMPORAL_DATA_SET))
    {
        ...
        FVizSize i; const FVizBool old=t->has_time; const FVizSize old_index=t->temporal_index; const double old_time=t->time;
        for (i=0u;i<fviz_temporal_data_set_step_count(temporal);++i)
        {
            t->has_time=FVIZ_TRUE; t->temporal_index=i; t->time=fviz_temporal_data_set_time(temporal,i);
            if (fviz_field_stats_visit(fviz_temporal_data_set_const_data(temporal,i),t)!=FVIZ_OK) return fviz_last_error_code();
        }
        t->has_time=old; t->temporal_index=old_index; t->time=old_time; return FVIZ_OK;
    }
...
void fviz_field_statistics_options_initialize(FVizFieldStatisticsOptions* options)
{
    ...
    memset(options,0,sizeof(*options)); options->struct_size=(uint32_t)sizeof(*options); options->association=FVIZ_FIELD_POINT_DATA; options->ignore_ghosts=FVIZ_TRUE;
}

static FVizResult fviz_field_statistics_compute_internal(
    const FVizDataObject* data, const char* array_name,
    ...
    FVizFieldMoments* out_moments)
{
    ...
    traversal.array_name=array_name; traversal.options=defaults; traversal.stats=out_statistics;
...
FVizResult fviz_field_statistics_compute(
    const FVizDataObject* data,const char* array_name,
    const FVizFieldStatisticsOptions* options,FVizFieldStatistics* out_statistics)
{
    return fviz_field_statistics_compute_internal(
        data,array_name,options,out_statistics,NULL);
}

FVizResult fviz_field_statistics_compute_moments(
    const FVizDataObject* data,const char* array_name,
    const FVizFieldStatisticsOptions* options,FVizFieldMoments* out_moments)
{
    FVizFieldStatistics statistics;
    if (out_moments==NULL) return FVIZ_ERROR_INVALID_ARGUMENT;
    return fviz_field_statistics_compute_internal(
        data,array_name,options,&statistics,out_moments);
}
