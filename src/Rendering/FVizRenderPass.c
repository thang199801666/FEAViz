...
static void fviz_render_pass_destroy(FVizObject* object)
{
    ...
    if (pass->destroy_user_data != NULL)
        pass->destroy_user_data(pass->user_data);
    pass->user_data = NULL;
}

static const FVizObjectClass g_fviz_render_pass_class = {
    FVIZ_TYPE_RENDER_PASS,
    "FVizRenderPass",
    &g_fviz_object_class,
    fviz_render_pass_destroy,
    NULL
};

FVizResult fviz_render_pass_create(
    ...
    FVizRenderPass** out_pass)
{
    ...
    *out_pass = NULL;
    pass = (FVizRenderPass*)fviz_internal_object_allocate(
        sizeof(*pass), &g_fviz_render_pass_class, NULL);
...
FVizResult fviz_render_pass_execute(
    FVizRenderPass* pass,
    FVizRenderer* renderer,
    const FVizRenderPassContext* context)
{
    if (pass == NULL || renderer == NULL || context == NULL ||
        context->struct_size < sizeof(*context))
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "render pass execution context is invalid");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    return pass->execute != NULL
        ? pass->execute(pass, renderer, context, pass->user_data)
        : FVIZ_OK;
}
