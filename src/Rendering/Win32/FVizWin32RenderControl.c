...
static LRESULT CALLBACK fviz_win32_render_control_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam);
...
{
    return hwnd != NULL ? (FVizWin32RenderControlState*)GetWindowLongPtrA(hwnd, GWLP_USERDATA) : NULL;
}
...
static LRESULT CALLBACK fviz_win32_render_control_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (message)
    {
        case WM_CREATE:
            {
                ...
                if (fviz_renderer_widget_create_attached_with_options(hwnd, width, height, &params->options,
                                                                      &state->widget) != FVIZ_OK)
                {
        ...
        case WM_DPICHANGED_AFTERPARENT:
            if (state != NULL && state->widget != NULL) (void)fviz_renderer_widget_sync_host_size(state->widget);
            return 0;
#endif
        case WM_SETFOCUS:
        ...
        case WM_SHOWWINDOW:
            if (state != NULL && state->widget != NULL)
            {
                if (wparam != 0)
                {
                    (void)fviz_renderer_widget_sync_host_size(state->widget);
                    (void)fviz_renderer_widget_show(state->widget);
                    fviz_render_window_request_render(fviz_renderer_widget_window(state->widget));
            ...
            return 0;
        case WM_TIMER:
            if (state != NULL && state->widget != NULL && state->timer_id != 0u && wparam == state->timer_id)
            {
                FVizRenderWindowInteractor* interactor = fviz_renderer_widget_interactor(state->widget);
                if (interactor != NULL)
                    (void)fviz_render_window_interactor_process_timers(interactor, (double)GetTickCount64() / 1000.0);
                return 0;
            }
            break;
        case WM_ERASEBKGND:
...
FVizResult fviz_win32_render_control_create(void* parent_hwnd, int control_id, int x, int y, int width, int height,
                                            const FVizRenderWindowOptions* options, void** out_control_hwnd)
{
    ...
    if (parent == NULL || IsWindow(parent) == FALSE || out_control_hwnd == NULL || width <= 0 || height <= 0)
    {
    ...
    control = CreateWindowExA(0, FVIZ_WIN32_RENDER_CONTROL_CLASS, "",
                              WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, x, y, width,
                              height, parent, (HMENU)(INT_PTR)control_id, GetModuleHandleA(NULL), &params);
    if (control == NULL)
    {
...
FVizResult fviz_win32_render_control_set_bounds(void* control_hwnd, int x, int y, int width, int height)
{
    HWND control = (HWND)control_hwnd;
    if (control == NULL || IsWindow(control) == FALSE || width <= 0 || height <= 0) return FVIZ_ERROR_INVALID_ARGUMENT;
    return SetWindowPos(control, NULL, x, y, width, height, SWP_NOZORDER | SWP_NOACTIVATE) != FALSE
               ? FVIZ_OK
               : FVIZ_ERROR_GRAPHICS;
}
...
{
    FVizRendererWidget* widget = fviz_win32_render_control_renderer_widget(control_hwnd);
    const FVizRenderWindow* window = widget != NULL ? fviz_renderer_widget_window(widget) : NULL;
    return window != NULL ? fviz_render_window_render_requested(window) : FVIZ_FALSE;
}

FVizResult fviz_win32_render_control_set_timer_pump(void* control_hwnd, FVizBool enabled,
                                                    uint32_t interval_milliseconds)
{
    HWND control = (HWND)control_hwnd;
    FVizWin32RenderControlState* state = fviz_win32_render_control_state(control);
    if (control == NULL || IsWindow(control) == FALSE || state == NULL) return FVIZ_ERROR_INVALID_ARGUMENT;
    if (enabled != FVIZ_FALSE && interval_milliseconds == 0u) return FVIZ_ERROR_INVALID_ARGUMENT;
    ...
    if (interval_milliseconds != 0u) state->timer_interval_milliseconds = interval_milliseconds;
