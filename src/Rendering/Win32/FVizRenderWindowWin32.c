...
typedef BOOL(WINAPI* PFNFVIZWGLCHOOSEPIXELFORMATARBPROC)(HDC dc, const int* int_attrib_list,
                                                         const FLOAT* float_attrib_list, UINT max_formats,
                                                         int* out_pixel_format, UINT* out_num_formats);
typedef HGLRC(WINAPI* PFNFVIZWGLCREATECONTEXTATTRIBSARBPROC)(HDC dc, HGLRC share_context, const int* attrib_list);
...
static uint32_t fviz_win32_query_window_dpi(HWND hwnd)
{
    ...
    get_dpi_for_window = user32 != NULL ? (PFNFVIZGETDPIFORWINDOWPROC)GetProcAddress(user32, "GetDpiForWindow") : NULL;
...
static void fviz_win32_render_legacy_scene(FVizRenderWindow* window, FVizRenderer* renderer, int viewport_width,
                                           int viewport_height);
...
static FVizResult fviz_win32_make_render_context_current(FVizRenderWindow* window)
{
    ...
    if (window->native_dc == NULL || window->native_gl_context == NULL) return FVIZ_ERROR_INVALID_STATE;
    return wglMakeCurrent((HDC)window->native_dc, (HGLRC)window->native_gl_context) != FALSE ? FVIZ_OK
                                                                                             : FVIZ_ERROR_GRAPHICS;
}

static void fviz_win32_done_render_context(FVizRenderWindow* window)
{
    if (window != NULL && window->external_opengl != FVIZ_FALSE && window->external_surface.done_current != NULL)
...
static FVizResult fviz_win32_initialize_external_gl(FVizRenderWindow* window)
{
    ...
    GLint samples = 0;
    if (window == NULL || window->external_opengl == FVIZ_FALSE) return FVIZ_ERROR_INVALID_ARGUMENT;
    ...
    if (result != FVIZ_OK)
    {
        ...
        fviz_internal_set_error(FVIZ_ERROR_NOT_SUPPORTED,
                                "external OpenGL context must provide OpenGL 3.3-class entry points");
    ...
    window->weighted_oit_supported =
        fviz_internal_gl_device_weighted_oit_supported((const FVizGLDevice*)window->gl_device);
    window->shader_lines_supported =
        fviz_internal_gl_device_shader_lines_supported((const FVizGLDevice*)window->gl_device);
    window->text_rendering_supported = fviz_internal_gl_device_text_supported((const FVizGLDevice*)window->gl_device);
    window->integer_selection_supported =
        fviz_internal_gl_device_integer_selection_supported((const FVizGLDevice*)window->gl_device);
...
{
    return CreateWindowExA(0, FVIZ_WINDOW_CLASS_NAME, "FEAVizGLCtxProbe", WS_OVERLAPPED, 0, 0, 8, 8, NULL, NULL,
...
static FVizBool fviz_win32_load_wgl_extensions(void)
{
    ...
    {
        fviz_wgl_choose_pixel_format_arb =
            (PFNFVIZWGLCHOOSEPIXELFORMATARBPROC)fviz_win32_wgl_get_proc_address("wglChoosePixelFormatARB");
    }
    if (fviz_wgl_create_context_attribs_arb == NULL)
    {
        fviz_wgl_create_context_attribs_arb =
            (PFNFVIZWGLCREATECONTEXTATTRIBSARBPROC)fviz_win32_wgl_get_proc_address("wglCreateContextAttribsARB");
    }
    if (fviz_wgl_swap_interval_ext == NULL)
    {
        fviz_wgl_swap_interval_ext =
            (PFNFVIZWGLSWAPINTERVALEXTPROC)fviz_win32_wgl_get_proc_address("wglSwapIntervalEXT");
    }
    return fviz_wgl_choose_pixel_format_arb != NULL && fviz_wgl_create_context_attribs_arb != NULL ? FVIZ_TRUE
                                                                                                   : FVIZ_FALSE;
}

static FVizBool fviz_win32_choose_modern_pixel_format(HDC dc, uint32_t requested_samples, FVizBool request_srgb,
                                                      int* out_format, FVizBool* out_srgb)
{
    uint32_t samples;
    int srgb_attempt;
    if (out_srgb != NULL) *out_srgb = FVIZ_FALSE;
    if (dc == NULL || out_format == NULL || fviz_wgl_choose_pixel_format_arb == NULL) return FVIZ_FALSE;
    for (srgb_attempt = request_srgb != FVIZ_FALSE ? 1 : 0; srgb_attempt >= 0; --srgb_attempt)
    {
        ...
        while (samples >= 2u)
        {
            ...
            int n = 0;
            UINT num_formats = 0u;
#define FVIZ_WGL_ATTR(name_, value_)                                                                                   \
    do                                                                                                                 \
    {                                                                                                                  \
        attributes[n++] = (name_);                                                                                     \
        attributes[n++] = (value_);                                                                                    \
            ...
            attributes[n] = 0;
#undef FVIZ_WGL_ATTR
            if (fviz_wgl_choose_pixel_format_arb(dc, attributes, NULL, 1u, out_format, &num_formats) != FALSE &&
                num_formats > 0u)
            {
...
static void fviz_win32_apply_gl_state(FVizRenderWindow* window)
{
    ...
    if (window->actual_multisamples > 1u) glEnable(0x809D); /* GL_MULTISAMPLE */
        ...
        glDisable(0x809D);
    if (window->srgb_enabled != FVIZ_FALSE && window->srgb_supported != FVIZ_FALSE) glEnable(FVIZ_GL_FRAMEBUFFER_SRGB);
    else
...
static FVizResult fviz_win32_create_gl_context(FVizRenderWindow* window, HWND hwnd)
{
    ...
    {
        const int context_attribs[] = {FVIZ_WGL_CONTEXT_MAJOR_VERSION_ARB,
                                       3,
                                       FVIZ_WGL_CONTEXT_MINOR_VERSION_ARB,
                                       3,
                                       FVIZ_WGL_CONTEXT_PROFILE_MASK_ARB,
                                       FVIZ_WGL_CONTEXT_CORE_PROFILE_BIT_ARB,
                                       0};
        int modern_format = 0;
        dc = GetDC(hwnd);
        if (dc != NULL &&
            fviz_win32_choose_modern_pixel_format(dc, window->requested_multisamples, window->srgb_enabled,
                                                  &modern_format, &window->srgb_supported) == FVIZ_TRUE)
        {
    ...
    if (context == NULL)
    {
        ...
        if (dc != NULL && (pixel_format_set != FVIZ_FALSE || fviz_win32_set_legacy_pixel_format(dc) == FVIZ_OK))
        {
    ...
    if (modern == FVIZ_TRUE)
    {
        ...
        window->weighted_oit_supported =
            fviz_internal_gl_device_weighted_oit_supported((const FVizGLDevice*)window->gl_device);
        window->depth_peeling_supported =
            fviz_internal_gl_device_depth_peeling_supported((const FVizGLDevice*)window->gl_device);
        window->shader_lines_supported =
            fviz_internal_gl_device_shader_lines_supported((const FVizGLDevice*)window->gl_device);
        window->text_rendering_supported =
            fviz_internal_gl_device_text_supported((const FVizGLDevice*)window->gl_device);
        window->integer_selection_supported =
            fviz_internal_gl_device_integer_selection_supported((const FVizGLDevice*)window->gl_device);
        window->gpu_timing_supported =
            fviz_internal_gl_device_gpu_timing_supported((const FVizGLDevice*)window->gl_device);
    }
    {
        GLint sample_buffers = 0;
        GLint samples = 0;
        ...
        window->actual_multisamples = sample_buffers > 0 && samples > 1 ? (uint32_t)samples : 0u;
    }
    window->swap_control_supported = fviz_wgl_swap_interval_ext != NULL ? FVIZ_TRUE : FVIZ_FALSE;
    if (fviz_wgl_swap_interval_ext != NULL) (void)fviz_wgl_swap_interval_ext(window->swap_interval);
    fviz_win32_apply_gl_state(window);
    return FVIZ_OK;
}
...
FVizResult fviz_internal_render_window_create_platform(FVizRenderWindow* window)
{
    ...
    style = attached != FVIZ_FALSE ? (WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN)
    ...
    hwnd = CreateWindowExA(0, FVIZ_WINDOW_CLASS_NAME, window->title, style, attached != FVIZ_FALSE ? 0 : CW_USEDEFAULT,
                           attached != FVIZ_FALSE ? 0 : CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top,
                           (HWND)window->host_native_handle, NULL, GetModuleHandleA(NULL), window);
    if (hwnd == NULL)
    {
...
FVizResult fviz_internal_render_window_render_platform(FVizRenderWindow* window)
{
    ...
    if ((window->external_opengl == FVIZ_FALSE && (dc == NULL || context == NULL)) ||
        (window->external_opengl != FVIZ_FALSE && window->gl_device == NULL) || renderer_count == 0u)
    {
    ...
    while (rendered_count < renderer_count)
    {
        ...
        for (i = 0u; i < renderer_count; ++i)
        {
            ...
            (void)fviz_object_invoke_event((FVizObject*)renderer, FVIZ_EVENT_RENDER_START, window);
            renderer_result = fviz_renderer_update(renderer);
            if (renderer_result == FVIZ_OK) renderer_result = fviz_renderer_compile_render_graph(renderer);
            if (renderer_result != FVIZ_OK)
            {
                (void)fviz_object_invoke_event((FVizObject*)renderer, FVIZ_EVENT_RENDER_END, &renderer_result);
            ...
            for (pass_index = 0u; pass_index < fviz_render_graph_execution_count(pass_graph); ++pass_index)
            {
                FVizRenderPass* pass = fviz_render_graph_execution_pass(pass_graph, pass_index);
                FVizResult pass_result = FVIZ_OK;
                const FVizRenderGraphPassId graph_pass_id = fviz_render_graph_execution_pass_id(pass_graph, pass_index);
                const char* pass_name = fviz_render_graph_pass_name(pass_graph, graph_pass_id);
                ...
                if (fviz_render_pass_is_custom(pass) != FVIZ_FALSE)
                {
                    ...
                    if (window->gl_modern != FVIZ_FALSE && window->gl_device != NULL)
                    {
                        fviz_internal_gl_device_capture_state((FVizGLDevice*)window->gl_device, &state_snapshot);
                    ...
                    if (restore_state != FVIZ_FALSE)
                    {
                        const FVizResult restore_result =
                            fviz_internal_gl_device_restore_state((FVizGLDevice*)window->gl_device, &state_snapshot);
                    ...
                    switch (fviz_render_pass_stage(pass))
                    {
                        case FVIZ_RENDER_PASS_CLEAR:
                            if (current_layer == 0)
                            {
                                ...
                                if (fviz_renderer_gradient_background(renderer) != FVIZ_FALSE)
                                {
                                    ...
                                    {
                                        float background2[3];
                                        fviz_renderer_get_background2(renderer, &background2[0], &background2[1],
                                                                      &background2[2]);
                        ...
                        case FVIZ_RENDER_PASS_OPAQUE:
                            if (window->gl_modern == FVIZ_TRUE && window->gl_device != NULL)
                                pass_result = fviz_internal_gl_device_render_stage(
                                    (FVizGLDevice*)window->gl_device, renderer, pass_context.aspect_ratio,
                                    viewport_width, viewport_height, FVIZ_RENDER_PASS_OPAQUE);
                            else
                                fviz_win32_render_legacy_scene(window, renderer, viewport_width, viewport_height);
                            break;
                        case FVIZ_RENDER_PASS_TRANSLUCENT:
                                ...
                                window->weighted_oit_supported != FVIZ_FALSE)
                            {
                                pass_result = fviz_internal_gl_device_render_weighted_oit(
                                    (FVizGLDevice*)window->gl_device, renderer, viewport_x, viewport_y, viewport_width,
                                    viewport_height,
                                    window->actual_multisamples > 1u ? window->actual_multisamples : 1u,
                                    pass_context.aspect_ratio, target_framebuffer);
                                if (pass_result == FVIZ_OK) applied_transparency = FVIZ_TRANSPARENCY_WEIGHTED_BLENDED;
                            }
                                     ...
                                     window->depth_peeling_supported != FVIZ_FALSE)
                            {
                                ...
                                if (pass_result == FVIZ_OK) applied_transparency = FVIZ_TRANSPARENCY_DEPTH_PEELING;
                            }
                            else if (window->gl_modern == FVIZ_TRUE && window->gl_device != NULL)
                            {
                                pass_result = fviz_internal_gl_device_render_stage(
                                    (FVizGLDevice*)window->gl_device, renderer, pass_context.aspect_ratio,
                                    viewport_width, viewport_height, FVIZ_RENDER_PASS_TRANSLUCENT);
                        ...
                        case FVIZ_RENDER_PASS_EDGE:
                            if (window->gl_modern == FVIZ_TRUE && window->gl_device != NULL)
                                pass_result = fviz_internal_gl_device_render_stage(
                                    (FVizGLDevice*)window->gl_device, renderer, pass_context.aspect_ratio,
                                    viewport_width, viewport_height, FVIZ_RENDER_PASS_EDGE);
                            break;
                                ...
                                if (window->gl_modern == FVIZ_TRUE && window->gl_device != NULL)
                                {
                                    ...
                                    if (pass_result == FVIZ_OK && window->text_rendering_supported != FVIZ_FALSE)
                                        pass_result = fviz_internal_gl_device_render_text_actors(
                                            (FVizGLDevice*)window->gl_device, renderer, viewport_width, viewport_height,
                                            fviz_render_window_content_scale(window));
                                }
                                break;
                            }
                ...
                fviz_internal_render_window_record_pass_statistics(
                    ...
                    fviz_win32_high_resolution_seconds() - pass_start_seconds, pass_result);
    ...
    {
        const FVizBool apply_fxaa = window->fxaa_enabled != FVIZ_FALSE && window->fxaa_supported != FVIZ_FALSE &&
        ...
        if (apply_fxaa != FVIZ_FALSE)
        {
            ...
            fxaa_result = fviz_internal_gl_device_apply_fxaa(
                (FVizGLDevice*)window->gl_device, window->width, window->height, &window->fxaa_options,
                window->srgb_enabled != FVIZ_FALSE && window->srgb_supported != FVIZ_FALSE, target_framebuffer);
    ...
    present_end_seconds = fviz_win32_high_resolution_seconds();
    {
        ...
        if (window->gl_modern != FVIZ_FALSE && window->gl_device != NULL)
            fviz_internal_gl_device_get_frame_statistics((const FVizGLDevice*)window->gl_device, &gl_statistics);
        ...
        window->last_statistics.gpu_mesh_budget_exceeded = gl_statistics.gpu_mesh_budget_exceeded;
        window->last_statistics.resident_geometry_gpu_bytes = gl_statistics.resident_geometry_gpu_bytes;
        window->last_statistics.resident_attribute_gpu_bytes = gl_statistics.resident_attribute_gpu_bytes;
        window->last_statistics.resident_instance_gpu_bytes = gl_statistics.resident_instance_gpu_bytes;
        window->last_statistics.resident_render_target_gpu_bytes = gl_statistics.resident_render_target_gpu_bytes;
        window->last_statistics.pinned_gpu_resources = gl_statistics.pinned_gpu_resources;
        window->last_statistics.custom_pass_state_restorations = gl_statistics.custom_pass_state_restorations;
        ...
        window->last_statistics.srgb_applied =
            window->srgb_enabled != FVIZ_FALSE && window->srgb_supported != FVIZ_FALSE ? FVIZ_TRUE : FVIZ_FALSE;
        ...
        window->last_statistics.transparency_mode_applied = applied_transparency;
        {
            ...
            {
                FVizRenderer* statistics_renderer = fviz_render_window_renderer_at(window, renderer_index);
                const FVizRenderGraph* graph = fviz_renderer_render_graph(statistics_renderer);
                ...
                if (UINT32_MAX - pass_count < graph_statistics.pass_count) pass_count = UINT32_MAX;
                else
                    pass_count += graph_statistics.pass_count;
            }
                                           ...
                                           int viewport_height)
{
    ...
    (void)window;
    projection = fviz_camera_projection_matrix(camera, (float)viewport_width / (float)viewport_height);
    ...
    for (i = 0u; i < FVIZ_RENDERER_MAX_LIGHTS; ++i)
    ...
    for (i = 0u; i < fviz_renderer_light_count(renderer) && enabled_light_count < FVIZ_RENDERER_MAX_LIGHTS; ++i)
    {
        ...
        position[0] = light_position.x;
        position[1] = light_position.y;
        position[2] = light_position.z;
        position[3] = 1.0f;
        ...
        diffuse[3] = 1.0f;
        ambient[0] = red * 0.15f;
        ambient[1] = green * 0.15f;
        ambient[2] = blue * 0.15f;
        ambient[3] = 1.0f;
        glEnable(slot);
...
static void fviz_win32_release_gl_context(FVizRenderWindow* window, HWND hwnd)
{
    ...
    if (window->gl_device != NULL)
    {
        if (window->external_opengl != FVIZ_FALSE) (void)fviz_win32_make_render_context_current(window);
...
FVizResult fviz_internal_render_window_set_swap_interval_platform(FVizRenderWindow* window, int interval)
{
    ...
    if (window->native_dc == NULL || window->native_gl_context == NULL) return FVIZ_ERROR_INVALID_STATE;
    if (wglMakeCurrent((HDC)window->native_dc, (HGLRC)window->native_gl_context) == FALSE) return FVIZ_ERROR_GRAPHICS;
...
{
    FVizResult result;
    if (window == NULL || window->gl_device == NULL) return FVIZ_ERROR_INVALID_STATE;
    result = fviz_win32_make_render_context_current(window);
    if (result != FVIZ_OK) return result;
    fviz_internal_gl_device_release_mesh_resources((FVizGLDevice*)window->gl_device);
    fviz_win32_done_render_context(window);
...
static FVizInteractionEvent fviz_win32_interaction_event(FVizInteractionEventType type, FVizMouseButton button, int x,
                                                         int y)
{
...
void fviz_internal_render_window_schedule_render_platform(FVizRenderWindow* window)
{
    ...
    hwnd = (HWND)window->native_window;
    if (hwnd != NULL && IsWindow(hwnd) != FALSE) (void)PostMessageA(hwnd, FVIZ_WM_REQUEST_RENDER, 0u, 0);
}
...
FVizResult fviz_internal_render_window_process_events_platform(FVizRenderWindow* window)
{
    ...
    if (window->external_opengl != FVIZ_FALSE)
    {
        (void)fviz_render_window_interactor_process_timers(window->interactor, (double)GetTickCount64() / 1000.0);
        return FVIZ_OK;
    }
    while (PeekMessageA(&message, window->host_native_handle != NULL ? (HWND)window->native_window : NULL, 0u, 0u,
                        PM_REMOVE) != FALSE)
    {
...
FVizResult fviz_internal_render_window_resize_platform(FVizRenderWindow* window)
{
    ...
    rect.right = window->width;
    rect.bottom = window->height;
    if (window->host_native_handle == NULL) (void)AdjustWindowRect(&rect, style, FALSE);
    if (SetWindowPos(hwnd, NULL, window->host_native_handle != NULL ? 0 : rect.left,
                     window->host_native_handle != NULL ? 0 : rect.top, rect.right - rect.left, rect.bottom - rect.top,
                     (window->host_native_handle != NULL ? 0u : SWP_NOMOVE) | SWP_NOZORDER | SWP_NOACTIVATE) == FALSE)
        return FVIZ_ERROR_GRAPHICS;
    return FVIZ_OK;
}
...
FVizResult fviz_internal_render_window_reparent_platform(FVizRenderWindow* window, void* host_native_handle)
{
    ...
    if (SetWindowPos(child, NULL, 0, 0, 1, 1, SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED) == FALSE)
    {
                                                              ...
                                                              float* out_depth)
{
    ...
    FVizResult current_result;
    FVizResult result;
    if (window == NULL || renderer == NULL || out_actor_index == NULL || out_primitive_id == NULL || out_depth == NULL)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    if (window->gl_modern == FVIZ_FALSE || window->gl_device == NULL) return FVIZ_ERROR_NOT_SUPPORTED;
    current_result = fviz_win32_make_render_context_current(window);
    if (current_result != FVIZ_OK) return current_result;
    target_framebuffer = fviz_win32_default_framebuffer(window);
    fviz_internal_gl_device_bind_framebuffer((FVizGLDevice*)window->gl_device, target_framebuffer);
    ...
    glEnable(GL_SCISSOR_TEST);
    glScissor(viewport_x, viewport_y, viewport_width, viewport_height);
    result = fviz_internal_gl_device_select((FVizGLDevice*)window->gl_device, renderer,
                                            (float)viewport_width / (float)viewport_height, x - viewport_x,
                                            (window->height - y - 1) - viewport_y, viewport_width, viewport_height,
                                            association, out_actor_index, out_primitive_id, out_depth);
    glViewport(previous_viewport[0], previous_viewport[1], previous_viewport[2], previous_viewport[3]);
    glScissor(previous_scissor_box[0], previous_scissor_box[1], previous_scissor_box[2], previous_scissor_box[3]);
    if (scissor_was_enabled != GL_FALSE) glEnable(GL_SCISSOR_TEST);
    else
        glDisable(GL_SCISSOR_TEST);
...
static void fviz_win32_render_interaction_if_due(FVizRenderWindow* window, FVizBool handled, FVizBool force)
{
    ...
    fviz_render_window_interactor_get_update_rates(window->interactor, &desired_rate, &still_rate);
        ...
        now - window->last_interaction_render_seconds >= minimum_interval)
    {
        if (force != FVIZ_FALSE)
        {
            if (fviz_render_window_render(window) == FVIZ_OK) window->last_interaction_render_seconds = now;
        }
        else
        {
            ...
            fviz_render_window_request_render_reason(window, window->interaction_active != FVIZ_FALSE
...
static LRESULT CALLBACK fviz_window_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (message)
    {
        case FVIZ_WM_REQUEST_RENDER:
            ...
            if (window->render_requested != FVIZ_FALSE) (void)InvalidateRect(hwnd, NULL, FALSE);
            return 0;
        case WM_ERASEBKGND:
            return 1;
        case WM_DPICHANGED:
            {
                ...
                if (suggested != NULL && window->host_native_handle == NULL && window->offscreen == FVIZ_FALSE)
                {
                    (void)SetWindowPos(hwnd, NULL, suggested->left, suggested->top, suggested->right - suggested->left,
        ...
        case WM_DPICHANGED_AFTERPARENT:
            {
                ...
                if (window->host_native_handle != NULL) (void)fviz_render_window_sync_host_size(window);
                fviz_render_window_request_render(window);
                return 0;
            }
#endif
        case WM_SIZE:
            {
                ...
                const FVizBool size_changed =
                    new_width != window->width || new_height != window->height ? FVIZ_TRUE : FVIZ_FALSE;
                ...
                if (window->width > 0 && window->height > 0) fviz_render_window_request_render(window);
                return 0;
            }
        case WM_PAINT:
            {
                PAINTSTRUCT paint;
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_EXPOSE, FVIZ_MOUSE_BUTTON_NONE, 0, 0);
        ...
        case WM_SETFOCUS:
            {
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_FOCUS_IN, FVIZ_MOUSE_BUTTON_NONE, 0, 0);
                (void)fviz_render_window_interactor_process_event(window->interactor, &event);
        ...
        case WM_KILLFOCUS:
            {
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_FOCUS_OUT, FVIZ_MOUSE_BUTTON_NONE, 0, 0);
                (void)fviz_render_window_interactor_process_event(window->interactor, &event);
        ...
        case WM_LBUTTONDOWN:
            {
                ...
                (void)SetFocus(hwnd);
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_DOWN, FVIZ_MOUSE_BUTTON_LEFT,
        ...
        case WM_MBUTTONDOWN:
            {
                ...
                window->last_mouse_y = GET_Y_LPARAM(lparam);
                (void)SetFocus(hwnd);
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_DOWN, FVIZ_MOUSE_BUTTON_MIDDLE,
                                                     window->last_mouse_x, window->last_mouse_y);
                SetCapture(hwnd);
                (void)fviz_render_window_interactor_process_event(window->interactor, &event);
                return 0;
        ...
        case WM_RBUTTONDOWN:
            {
                ...
                window->last_mouse_y = GET_Y_LPARAM(lparam);
                (void)SetFocus(hwnd);
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_DOWN, FVIZ_MOUSE_BUTTON_RIGHT,
                                                     window->last_mouse_x, window->last_mouse_y);
                SetCapture(hwnd);
                (void)fviz_render_window_interactor_process_event(window->interactor, &event);
                return 0;
        ...
        case WM_LBUTTONUP:
            {
                ...
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_UP, FVIZ_MOUSE_BUTTON_LEFT, x, y);
                window->left_mouse_down = FVIZ_FALSE;
                {
                ...
                if (window->middle_mouse_down == FVIZ_FALSE && window->right_mouse_down == FVIZ_FALSE &&
                    window->x1_mouse_down == FVIZ_FALSE && window->x2_mouse_down == FVIZ_FALSE)
                    ReleaseCapture();
                return 0;
            }
        case WM_MBUTTONUP:
            {
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_UP, FVIZ_MOUSE_BUTTON_MIDDLE,
        ...
        case WM_RBUTTONUP:
            {
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_UP, FVIZ_MOUSE_BUTTON_RIGHT,
                                                 GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
                window->right_mouse_down = FVIZ_FALSE;
                {
                    const FVizBool handled = fviz_render_window_interactor_process_event(window->interactor, &event);
        ...
        case WM_XBUTTONDOWN:
            {
                ...
                const FVizMouseButton button = native_button == XBUTTON1 ? FVIZ_MOUSE_BUTTON_X1 : FVIZ_MOUSE_BUTTON_X2;
                FVizInteractionEvent event;
                if (button == FVIZ_MOUSE_BUTTON_X1) window->x1_mouse_down = FVIZ_TRUE;
                else
                    window->x2_mouse_down = FVIZ_TRUE;
                window->last_mouse_x = GET_X_LPARAM(lparam);
                window->last_mouse_y = GET_Y_LPARAM(lparam);
                (void)SetFocus(hwnd);
                SetCapture(hwnd);
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_DOWN, button, window->last_mouse_x,
                                                     window->last_mouse_y);
                (void)fviz_render_window_interactor_process_event(window->interactor, &event);
                return TRUE;
            }
        case WM_XBUTTONUP:
            {
                const WORD native_button = GET_XBUTTON_WPARAM(wparam);
                const FVizMouseButton button = native_button == XBUTTON1 ? FVIZ_MOUSE_BUTTON_X1 : FVIZ_MOUSE_BUTTON_X2;
                FVizInteractionEvent event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_BUTTON_UP, button,
                                                                          GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
        ...
        case WM_XBUTTONDBLCLK:
            {
                const WORD native_button = GET_XBUTTON_WPARAM(wparam);
                const FVizMouseButton button = native_button == XBUTTON1 ? FVIZ_MOUSE_BUTTON_X1 : FVIZ_MOUSE_BUTTON_X2;
                FVizInteractionEvent event = fviz_win32_interaction_event(FVIZ_INTERACTION_DOUBLE_CLICK, button,
                                                                          GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
                {
                    const FVizBool handled = fviz_render_window_interactor_process_event(window->interactor, &event);
                    fviz_win32_render_interaction_if_due(window, handled, FVIZ_TRUE);
        ...
        case WM_MOUSEMOVE:
            {
                ...
                if (window->mouse_inside == FVIZ_FALSE)
                {
                    ...
                    FVizInteractionEvent enter_event =
                        fviz_win32_interaction_event(FVIZ_INTERACTION_ENTER, FVIZ_MOUSE_BUTTON_NONE, x, y);
                ...
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_MOVE, FVIZ_MOUSE_BUTTON_NONE, x, y);
                event.delta_x = dx;
                event.delta_y = dy;
                {
                    const FVizBool handled = fviz_render_window_interactor_process_event(window->interactor, &event);
        ...
        case WM_MOUSEWHEEL:
            {
                ...
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_MOUSE_WHEEL, FVIZ_MOUSE_BUTTON_NONE,
        ...
        case WM_MOUSELEAVE:
            {
                FVizInteractionEvent event = fviz_win32_interaction_event(
                    FVIZ_INTERACTION_LEAVE, FVIZ_MOUSE_BUTTON_NONE, window->last_mouse_x, window->last_mouse_y);
        ...
        case WM_SYSKEYDOWN:
            {
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_KEY_DOWN, FVIZ_MOUSE_BUTTON_NONE, 0, 0);
        ...
        case WM_SYSKEYUP:
            {
                FVizInteractionEvent event =
                    fviz_win32_interaction_event(FVIZ_INTERACTION_KEY_UP, FVIZ_MOUSE_BUTTON_NONE, 0, 0);
                event.key = wparam == VK_ESCAPE ? FVIZ_KEY_ESCAPE : (int)wparam;
                if (fviz_render_window_interactor_process_event(window->interactor, &event) == FVIZ_TRUE) return 0;
                break;
            }
        case WM_CHAR:
        case WM_SYSCHAR:
        case WM_UNICHAR:
            {
                ...
                event = fviz_win32_interaction_event(FVIZ_INTERACTION_CHAR, FVIZ_MOUSE_BUTTON_NONE, 0, 0);
        ...
        case WM_DESTROY:
            ...
            if (window->host_native_handle == NULL && window->offscreen == FVIZ_FALSE) PostQuitMessage(0);
            return 0;
        default:
            break;
    }
