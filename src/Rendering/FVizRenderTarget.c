...
static const FVizObjectClass g_fviz_render_target_class = {FVIZ_TYPE_RENDER_TARGET, "FVizRenderTarget",
                                                           &g_fviz_object_class, fviz_render_target_destroy, NULL};

static uint32_t fviz_render_format_bytes(FVizRenderFormat format)
{
    switch (format)
    {
        case FVIZ_RENDER_FORMAT_RGBA16_FLOAT:
            return 8u;
        case FVIZ_RENDER_FORMAT_RG32_UINT:
            return 8u;
        ...
        case FVIZ_RENDER_FORMAT_DEPTH32_FLOAT:
            return 4u;
        case FVIZ_RENDER_FORMAT_R16_FLOAT:
            return 2u;
        default:
            return 0u;
    }
...
{
    return point == FVIZ_RENDER_ATTACHMENT_DEPTH || point == FVIZ_RENDER_ATTACHMENT_DEPTH_STENCIL ? FVIZ_TRUE
                                                                                                  : FVIZ_FALSE;
}

static FVizBool fviz_render_format_is_depth(FVizRenderFormat format)
{
    return format == FVIZ_RENDER_FORMAT_DEPTH24_STENCIL8 || format == FVIZ_RENDER_FORMAT_DEPTH32_FLOAT ? FVIZ_TRUE
                                                                                                       : FVIZ_FALSE;
}
...
FVizResult fviz_render_target_desc_add_attachment(FVizRenderTargetDesc* desc, FVizRenderAttachmentPoint point,
                                                  FVizRenderFormat format, FVizBool sampled)
{
    ...
    if (desc == NULL || desc->attachment_count >= FVIZ_RENDER_TARGET_MAX_ATTACHMENTS ||
        fviz_render_attachment_point_is_valid(point) == FVIZ_FALSE || fviz_render_format_bytes(format) == 0u)
...
FVizResult fviz_render_target_desc_validate(const FVizRenderTargetDesc* desc)
{
    ...
    if (desc == NULL || desc->width == 0u || desc->height == 0u || desc->samples == 0u || desc->samples > 32u ||
        desc->attachment_count == 0u || desc->attachment_count > FVIZ_RENDER_TARGET_MAX_ATTACHMENTS)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    for (i = 0u; i < desc->attachment_count; ++i)
    {
        ...
        for (j = i + 1u; j < desc->attachment_count; ++j)
            if (attachment->point == desc->attachments[j].point) return FVIZ_ERROR_INVALID_STATE;
    }
    if (depth_count > 1u) return FVIZ_ERROR_INVALID_ARGUMENT;
    return FVIZ_OK;
}

FVizResult fviz_render_target_create(const FVizRenderTargetDesc* desc, FVizRenderTarget** out_target)
{
    ...
    *out_target = NULL;
    ...
    target = (FVizRenderTarget*)fviz_internal_object_allocate(sizeof(*target), &g_fviz_render_target_class, NULL);
...
FVizResult fviz_render_target_resize(FVizRenderTarget* target, uint32_t width, uint32_t height)
{
...
void fviz_render_target_get_desc(const FVizRenderTarget* target, FVizRenderTargetDesc* out_desc)
{
...
uint64_t fviz_render_target_estimated_bytes(const FVizRenderTarget* target)
{
    ...
    if (target->desc.samples != 0u && pixels > UINT64_MAX / (uint64_t)target->desc.samples) return UINT64_MAX;
