...
static FVizBool fviz_scalar_legend_dependency_modified(
...
static FVizResult fviz_scalar_legend_observe_dependency(
    FVizScalarLegend* legend, FVizObject* dependency, FVizObserverTag* out_tag)
{
...
static void fviz_scalar_legend_destroy(FVizObject* object)
{
    ...
    if (legend->title_text_property != NULL && legend->title_text_property_modified_tag != FVIZ_OBSERVER_TAG_INVALID)
        (void)fviz_object_remove_observer((FVizObject*)legend->title_text_property, legend->title_text_property_modified_tag);
    if (legend->label_text_property != NULL && legend->label_text_property_modified_tag != FVIZ_OBSERVER_TAG_INVALID)
        (void)fviz_object_remove_observer((FVizObject*)legend->label_text_property, legend->label_text_property_modified_tag);
...
FVizResult fviz_scalar_legend_create(FVizScalarLegend** out_legend)
{
    ...
    *out_legend = NULL;
    ...
    legend->panel_color[0] = 0.10f; legend->panel_color[1] = 0.11f;
    legend->panel_color[2] = 0.14f; legend->panel_color[3] = 1.0f;
    legend->border_color[0] = 0.85f; legend->border_color[1] = 0.85f;
    legend->border_color[2] = 0.90f; legend->border_color[3] = 1.0f;
...
void fviz_scalar_legend_set_lookup_table(FVizScalarLegend* legend, FVizLookupTable* table)
{
    ...
    if (table != NULL)
    {
        ...
        if (fviz_scalar_legend_observe_dependency(
                legend, (FVizObject*)table, &new_tag) != FVIZ_OK)
        {
            fviz_release(table);
            return;
        }
    ...
    if (legend->lookup_table != NULL && legend->lookup_table_modified_tag != FVIZ_OBSERVER_TAG_INVALID)
        (void)fviz_object_remove_observer(
            (FVizObject*)legend->lookup_table, legend->lookup_table_modified_tag);
    fviz_release(legend->lookup_table);
...
void fviz_scalar_legend_set_viewport_padding(
    FVizScalarLegend* legend, float horizontal, float vertical)
{
    float normalized_x;
    float normalized_y;
    if (legend == NULL) return;
    normalized_x = isfinite(horizontal) && horizontal >= 0.0f
        ? (horizontal > 1.0f ? 1.0f : horizontal) : -1.0f;
    normalized_y = isfinite(vertical) && vertical >= 0.0f
        ? (vertical > 1.0f ? 1.0f : vertical) : -1.0f;
    if (legend->viewport_padding_x == normalized_x &&
...
void fviz_scalar_legend_get_viewport_padding(
...
FVizResult fviz_scalar_legend_set_label_format(FVizScalarLegend* legend, const char* format)
{
    ...
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "scalar legend label format must contain one floating-point conversion");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
...
void fviz_scalar_legend_set_bar_size(
...
void fviz_scalar_legend_get_bar_size(
...
static void fviz_scalar_legend_set_rgba(
    float target[4], float red, float green, float blue, float alpha)
{
...
void fviz_scalar_legend_set_panel_color(
...
void fviz_scalar_legend_get_panel_color(
...
void fviz_scalar_legend_set_border_color(
    FVizScalarLegend* legend, float red, float green, float blue, float alpha)
{
    float color[4];
    if (legend == NULL) return;
...
void fviz_scalar_legend_get_border_color(
    const FVizScalarLegend* legend, float* red, float* green, float* blue, float* alpha)
{
    if (legend == NULL) return;
...
void fviz_scalar_legend_set_tick_style(
...
void fviz_scalar_legend_get_tick_style(
...
void fviz_scalar_legend_set_layout_spacing(
    FVizScalarLegend* legend, float title_subtitle, float subtitle_bar,
    float bar_label, float bar_statistics)
{
    if (legend == NULL) return;
    if (isfinite(title_subtitle) && title_subtitle >= 0.0f)
        legend->title_subtitle_spacing = title_subtitle;
    if (isfinite(subtitle_bar) && subtitle_bar >= 0.0f)
        legend->subtitle_bar_spacing = subtitle_bar;
    if (isfinite(bar_label) && bar_label >= 0.0f)
        legend->bar_label_spacing = bar_label;
    if (isfinite(bar_statistics) && bar_statistics >= 0.0f)
        legend->bar_statistics_spacing = bar_statistics;
    fviz_object_modified((FVizObject*)legend);
}

void fviz_scalar_legend_get_layout_spacing(
