...
static FVizBool fviz_renderer_dependency_modified(FVizObject* caller, FVizEventId event_id, void* call_data,
                                                  void* client_data)
{
...
static FVizResult fviz_renderer_observe_dependency(FVizRenderer* renderer, FVizObject* dependency, FVizObserverTag* tag)
{
    if (renderer == NULL || dependency == NULL || tag == NULL) return FVIZ_ERROR_INVALID_ARGUMENT;
    *tag = FVIZ_OBSERVER_TAG_INVALID;
    return fviz_object_add_observer(dependency, FVIZ_EVENT_MODIFIED, 0.0f, fviz_renderer_dependency_modified, renderer,
                                    tag);
}

static FVizResult fviz_renderer_observe_child(FVizRenderer* renderer, FVizObject* child, FVizArray* owner_array)
{
    ...
    result =
        fviz_object_add_observer(child, FVIZ_EVENT_MODIFIED, 0.0f, fviz_renderer_dependency_modified, renderer, &tag);
    ...
    record = (FVizRendererChildObserver*)fviz_array_at(renderer->child_dependency_observers, count);
    record->object = child;
...
static void fviz_renderer_unobserve_child(FVizRenderer* renderer, FVizObject* child, FVizArray* owner_array)
{
    FVizSize i;
    FVizSize count;
    ...
    for (i = 0u; i < count; ++i)
    {
        ...
        {
            if (records[i].tag != FVIZ_OBSERVER_TAG_INVALID) (void)fviz_object_remove_observer(child, records[i].tag);
            if (i + 1u < count)
                (void)memmove(&records[i], &records[i + 1u], (size_t)(count - i - 1u) * sizeof(*records));
...
static void fviz_renderer_unobserve_all_children(FVizRenderer* renderer)
{
    ...
    {
        FVizRendererChildObserver* record =
            (FVizRendererChildObserver*)fviz_array_at(renderer->child_dependency_observers, i);
        if (record != NULL && record->object != NULL && record->tag != FVIZ_OBSERVER_TAG_INVALID)
            (void)fviz_object_remove_observer(record->object, record->tag);
    }
    fviz_array_clear(renderer->child_dependency_observers);
}

static const FVizObjectClass g_fviz_renderer_class = {FVIZ_TYPE_RENDERER, "FVizRenderer", &g_fviz_object_class,
                                                      fviz_renderer_destroy, NULL};
...
FVizResult fviz_renderer_create(FVizRenderer** out_renderer)
{
    ...
    *out_renderer = NULL;
    ...
    if (fviz_renderer_observe_dependency(renderer, (FVizObject*)renderer->scene, &renderer->scene_modified_tag) !=
            FVIZ_OK ||
        fviz_renderer_observe_dependency(renderer, (FVizObject*)renderer->camera, &renderer->camera_modified_tag) !=
            FVIZ_OK)
    {
        fviz_release(renderer);
        return fviz_last_error_code();
    }
    {
        ...
        if (fviz_light_create(&headlight) != FVIZ_OK || fviz_renderer_add_light(renderer, headlight) != FVIZ_OK)
        {
...
FVizResult fviz_renderer_set_scene(FVizRenderer* renderer, FVizScene* scene)
{
    ...
    if (renderer->scene == scene) return FVIZ_OK;
    {
        ...
        if (fviz_renderer_observe_dependency(renderer, (FVizObject*)scene, &new_tag) != FVIZ_OK)
        {
            fviz_release(scene);
            return fviz_last_error_code();
        }
...
FVizScene* fviz_renderer_scene(FVizRenderer* renderer)
{
    return renderer != NULL ? renderer->scene : NULL;
}

FVizCamera* fviz_renderer_camera(FVizRenderer* renderer)
{
...
FVizResult fviz_renderer_get_frustum(const FVizRenderer* renderer, float aspect_ratio, FVizFrustum* out_frustum)
{
    FVizRenderer* mutable_renderer;
    FVizMTime camera_mtime;
    if (renderer == NULL || out_frustum == NULL || aspect_ratio <= 0.0f) return FVIZ_ERROR_INVALID_ARGUMENT;
    camera_mtime = fviz_object_mtime((const FVizObject*)renderer->camera);
    mutable_renderer = (FVizRenderer*)renderer;
    if (renderer->frustum_cache_valid == FVIZ_FALSE || renderer->frustum_camera_mtime != camera_mtime ||
        renderer->frustum_aspect_ratio != aspect_ratio)
    {
        const FVizMat4 view_projection = fviz_mat4_multiply(
            fviz_camera_projection_matrix(renderer->camera, aspect_ratio), fviz_camera_view_matrix(renderer->camera));
...
FVizBool fviz_renderer_actor_in_frustum(const FVizRenderer* renderer, const FVizActor* actor, float aspect_ratio)
{
    FVizFrustum frustum;
    if (renderer == NULL || actor == NULL || aspect_ratio <= 0.0f) return FVIZ_FALSE;
    if (renderer->frustum_culling == FVIZ_FALSE) return FVIZ_TRUE;
    if (fviz_renderer_get_frustum(renderer, aspect_ratio, &frustum) != FVIZ_OK) return FVIZ_TRUE;
...
FVizResult fviz_renderer_set_small_object_threshold_pixels(FVizRenderer* renderer, float diameter_pixels)
{
    if (renderer == NULL || !isfinite(diameter_pixels) || diameter_pixels < 0.0f || diameter_pixels > 4096.0f)
        return FVIZ_ERROR_INVALID_ARGUMENT;
...
float fviz_renderer_actor_projected_diameter_pixels(const FVizRenderer* renderer, const FVizActor* actor,
                                                    float aspect_ratio, int viewport_height)
{
    FVizBounds bounds;
    FVizVec3 center;
    FVizVec3 half;
    float radius;
    float diameter;
    if (renderer == NULL || actor == NULL || aspect_ratio <= 0.0f || viewport_height <= 0) return 0.0f;
    ...
    else
    {
        ...
        const float half_fov = fviz_camera_fov_degrees(renderer->camera) * (3.14159265358979323846f / 360.0f);
        float tangent;
...
FVizBool fviz_renderer_actor_is_renderable(const FVizRenderer* renderer, const FVizActor* actor, float aspect_ratio,
                                           int viewport_height)
{
    if (renderer == NULL || actor == NULL || aspect_ratio <= 0.0f || viewport_height <= 0) return FVIZ_FALSE;
    if (fviz_renderer_actor_in_frustum(renderer, actor, aspect_ratio) == FVIZ_FALSE) return FVIZ_FALSE;
    if (renderer->small_object_culling != FVIZ_FALSE && renderer->small_object_threshold_pixels > 0.0f &&
        fviz_renderer_actor_projected_diameter_pixels(renderer, actor, aspect_ratio, viewport_height) <
            renderer->small_object_threshold_pixels)
        return FVIZ_FALSE;
    return FVIZ_TRUE;
}
...
FVizResult fviz_renderer_set_weighted_oit_options(FVizRenderer* renderer, const FVizWeightedOITOptions* options)
{
    if (renderer == NULL || options == NULL ||
        (options->struct_size != 0u && options->struct_size < sizeof(FVizWeightedOITOptions)) ||
        options->weight_scale <= 0.0f || options->weight_scale > 64.0f || options->depth_weight < 0.0f ||
        options->depth_weight > 16.0f || options->minimum_weight <= 0.0f || options->minimum_weight > 1.0f ||
...
void fviz_renderer_get_weighted_oit_options(const FVizRenderer* renderer, FVizWeightedOITOptions* out_options)
{
...
FVizResult fviz_renderer_remove_light(FVizRenderer* renderer, FVizLight* light)
{
    ...
    for (i = 0u; i < count; ++i)
    {
        if (items[i] == light)
        {
            ...
            if (i + 1u < count) (void)memmove(&items[i], &items[i + 1u], (size_t)(count - i - 1u) * sizeof(*items));
...
FVizResult fviz_renderer_update(FVizRenderer* renderer)
{
    ...
    for (i = 0u; renderer->scene != NULL && i < fviz_scene_actor_count(renderer->scene); ++i)
    {
        ...
        if (actor != NULL && fviz_mapper_update(fviz_actor_mapper(actor)) != FVIZ_OK) return fviz_last_error_code();
    }
...
{
    if (renderer == NULL) return;
    if (renderer->background[0] == red && renderer->background[1] == green && renderer->background[2] == blue) return;
    renderer->background[0] = red;
    renderer->background[1] = green;
    renderer->background[2] = blue;
    fviz_object_modified((FVizObject*)renderer);
}
...
{
    if (renderer == NULL) return;
    if (renderer->background2[0] == red && renderer->background2[1] == green && renderer->background2[2] == blue)
        return;
...
void fviz_renderer_get_background2(const FVizRenderer* renderer, float* red, float* green, float* blue)
{
    if (renderer == NULL) return;
...
FVizResult fviz_renderer_set_viewport(FVizRenderer* renderer, float minimum_x, float minimum_y, float maximum_x,
                                      float maximum_y)
{
    if (renderer == NULL || minimum_x < 0.0f || minimum_y < 0.0f || maximum_x > 1.0f || maximum_y > 1.0f ||
        minimum_x >= maximum_x || minimum_y >= maximum_y)
    {
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT,
                                "renderer viewport must be a non-empty normalized rectangle");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
...
void fviz_renderer_get_viewport(const FVizRenderer* renderer, float* minimum_x, float* minimum_y, float* maximum_x,
                                float* maximum_y)
{
    if (renderer == NULL) return;
...
{
    if (renderer != NULL) renderer->interactive = interactive != FVIZ_FALSE ? FVIZ_TRUE : FVIZ_FALSE;
}
...
FVizBool fviz_renderer_contains_normalized_point(const FVizRenderer* renderer, float x, float y)
{
    return renderer != NULL && x >= renderer->viewport[0] && x <= renderer->viewport[2] && y >= renderer->viewport[1] &&
                   y <= renderer->viewport[3]
               ? FVIZ_TRUE
               : FVIZ_FALSE;
}
...
FVizResult fviz_renderer_add_pass(FVizRenderer* renderer, FVizRenderPass* pass)
{
    ...
    index = count;
    while (index > 0u && fviz_render_pass_stage(items[index - 1u]) > fviz_render_pass_stage(pass))
    {
...
{
    return fviz_renderer_add_object_to_array(renderer, renderer != NULL ? renderer->text_actors_2d : NULL,
                                             (FVizObject*)actor);
}

FVizResult fviz_renderer_remove_text_actor_2d(FVizRenderer* renderer, FVizTextActor2D* actor)
{
    return fviz_renderer_remove_object_from_array(renderer, renderer != NULL ? renderer->text_actors_2d : NULL,
                                                  (FVizObject*)actor);
}
...
{
    FVizTextActor2D** slot =
        renderer != NULL ? (FVizTextActor2D**)fviz_array_at(renderer->text_actors_2d, index) : NULL;
    return slot != NULL ? *slot : NULL;
}

FVizResult fviz_renderer_add_billboard_text_actor_3d(FVizRenderer* renderer, FVizBillboardTextActor3D* actor)
{
    return fviz_renderer_add_object_to_array(renderer, renderer != NULL ? renderer->billboard_text_actors_3d : NULL,
                                             (FVizObject*)actor);
}

FVizResult fviz_renderer_remove_billboard_text_actor_3d(FVizRenderer* renderer, FVizBillboardTextActor3D* actor)
{
    return fviz_renderer_remove_object_from_array(
        renderer, renderer != NULL ? renderer->billboard_text_actors_3d : NULL, (FVizObject*)actor);
}
...
{
    FVizBillboardTextActor3D** slot =
        renderer != NULL ? (FVizBillboardTextActor3D**)fviz_array_at(renderer->billboard_text_actors_3d, index) : NULL;
    return slot != NULL ? *slot : NULL;
}

FVizResult fviz_renderer_add_label_set_3d(FVizRenderer* renderer, FVizLabelSet3D* label_set)
{
    return fviz_renderer_add_object_to_array(renderer, renderer != NULL ? renderer->label_sets_3d : NULL,
                                             (FVizObject*)label_set);
}

FVizResult fviz_renderer_remove_label_set_3d(FVizRenderer* renderer, FVizLabelSet3D* label_set)
{
    return fviz_renderer_remove_object_from_array(renderer, renderer != NULL ? renderer->label_sets_3d : NULL,
                                                  (FVizObject*)label_set);
}
...
{
    static const FVizRenderPassStage stages[] = {FVIZ_RENDER_PASS_CLEAR,       FVIZ_RENDER_PASS_OPAQUE,
                                                 FVIZ_RENDER_PASS_TRANSLUCENT, FVIZ_RENDER_PASS_EDGE,
                                                 FVIZ_RENDER_PASS_SELECTION,   FVIZ_RENDER_PASS_OVERLAY};
    FVizSize i;
...
{
    FVizRenderPass** pass = renderer != NULL ? (FVizRenderPass**)fviz_array_at(renderer->passes, index) : NULL;
...
FVizResult fviz_renderer_compile_render_graph(FVizRenderer* renderer)
{
    ...
    if (renderer->render_graph_dirty == FVIZ_FALSE &&
        fviz_render_graph_is_compiled(renderer->render_graph) != FVIZ_FALSE)
        return FVIZ_OK;
    ...
    for (i = 0u; i < fviz_array_count(renderer->passes); ++i)
    {
        ...
        result = fviz_render_graph_add_pass(
            renderer->render_graph, fviz_renderer_pass_stage_name(fviz_render_pass_stage(pass)), pass, &current);
        if (result != FVIZ_OK) return result;
...
static FVizResult fviz_renderer_transform_point(FVizMat4 matrix, FVizVec3 input, FVizBool divide, FVizVec3* output)
{
    float x;
    float y;
    float z;
...
FVizResult fviz_renderer_world_to_view(const FVizRenderer* renderer, FVizVec3 world, FVizVec3* out_view)
{
    if (renderer == NULL || renderer->camera == NULL || out_view == NULL) return FVIZ_ERROR_INVALID_ARGUMENT;
    return fviz_renderer_transform_point(fviz_camera_view_matrix(renderer->camera), world, FVIZ_FALSE, out_view);
}

FVizResult fviz_renderer_view_to_ndc(const FVizRenderer* renderer, FVizVec3 view, float aspect_ratio, FVizVec3* out_ndc)
{
    if (renderer == NULL || renderer->camera == NULL || out_ndc == NULL || aspect_ratio <= 0.0f)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    return fviz_renderer_transform_point(fviz_camera_projection_matrix(renderer->camera, aspect_ratio), view, FVIZ_TRUE,
                                         out_ndc);
}

FVizResult fviz_renderer_ndc_to_display(const FVizRenderer* renderer, FVizVec3 ndc, int window_width, int window_height,
                                        FVizVec3* out_display)
{
    ...
    *out_display = fviz_vec3(renderer->viewport[0] * (float)window_width + (ndc.x * 0.5f + 0.5f) * viewport_width,
...
FVizResult fviz_renderer_world_to_display(const FVizRenderer* renderer, FVizVec3 world, int window_width,
                                          int window_height, FVizVec3* out_display)
{
    FVizVec3 view;
    FVizVec3 ndc;
    float viewport_width;
...
FVizResult fviz_renderer_display_to_world_ray(const FVizRenderer* renderer, float display_x, float display_y,
                                              int window_width, int window_height, FVizRay* out_ray)
{
    int viewport_x;
    int viewport_y;
    int viewport_width;
    int viewport_height;
    if (renderer == NULL || renderer->camera == NULL || out_ray == NULL || window_width <= 0 || window_height <= 0)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    ...
    if (viewport_width < 1 || viewport_height < 1 || display_x < (float)viewport_x || display_y < (float)viewport_y ||
        display_x >= (float)(viewport_x + viewport_width) || display_y >= (float)(viewport_y + viewport_height))
        return FVIZ_ERROR_NOT_FOUND;
    *out_ray = fviz_camera_pick_ray(renderer->camera, viewport_width, viewport_height,
                                    (int)(display_x - (float)viewport_x), (int)(display_y - (float)viewport_y));
    return FVIZ_OK;
}
