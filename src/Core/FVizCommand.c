...
static const FVizObjectClass g_fviz_command_class = {FVIZ_TYPE_COMMAND, "FVizCommand", &g_fviz_object_class, NULL,
                                                     NULL};

FVizResult fviz_command_create(FVizCommandExecuteFn execute, void* client_data, FVizCommand** out_command)
{
    FVizCommand* command;
    if (out_command == NULL || execute == NULL)
    {
        if (out_command != NULL) *out_command = NULL;
        fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "out_command and execute must not be NULL");
        return FVIZ_ERROR_INVALID_ARGUMENT;
    }
    *out_command = NULL;
    command = (FVizCommand*)fviz_internal_object_allocate(sizeof(FVizCommand), &g_fviz_command_class, NULL);
...
{
    if (command != NULL) command->abort_flag = abort_flag != FVIZ_FALSE ? FVIZ_TRUE : FVIZ_FALSE;
}
...
FVizBool fviz_command_execute(FVizCommand* command, FVizObject* caller, FVizEventId event_id, void* call_data)
{
    FVizBool callback_abort;
    if (command == NULL || caller == NULL || command->execute == NULL) return FVIZ_FALSE;
    command->abort_flag = FVIZ_FALSE;
    callback_abort = command->execute(command, caller, event_id, call_data, command->client_data);
