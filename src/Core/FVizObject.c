...
static FVizObserverList* fviz_object_observer_list_ensure(FVizObject* object)
{
    ...
    list = (FVizObserverList*)fviz_allocator_allocate(&object->allocator, sizeof(FVizObserverList),
...
static FVizResult fviz_object_observer_reserve(FVizObject* object, FVizObserverList* list, FVizSize required)
{
    ...
    observers = (FVizObjectObserver*)fviz_allocator_reallocate(&object->allocator, list->observers,
                                                               list->capacity * sizeof(FVizObjectObserver), bytes,
                                                               (FVizSize)FVIZ_INTERNAL_MAX_ALIGNMENT);
    if (observers == NULL)
    {
...
static FVizBool fviz_object_observer_precedes(const FVizObjectObserver* left, const FVizObjectObserver* right)
{
...
static FVizBool fviz_object_observer_matches(FVizEventId observer_event, FVizEventId event_id)
{
    ...
    if (observer_event == FVIZ_EVENT_INTERACTION_ANY && event_id >= FVIZ_EVENT_MOUSE_BUTTON_DOWN &&
        event_id <= FVIZ_EVENT_CHAR)
        return FVIZ_TRUE;
    return FVIZ_FALSE;
}

static FVizBool fviz_object_command_observer_callback(FVizObject* caller, FVizEventId event_id, void* call_data,
                                                      void* client_data)
{
...
static void fviz_object_observer_list_destroy(FVizObject* object)
{
    ...
    if (list->observers != NULL)
    {
        ...
        for (i = 0u; i < list->count; ++i)
            if (list->observers[i].command != NULL) fviz_release(list->observers[i].command);
        fviz_allocator_deallocate(&object->allocator, list->observers, list->capacity * sizeof(FVizObjectObserver),
                                  (FVizSize)FVIZ_INTERNAL_MAX_ALIGNMENT);
    }
    fviz_allocator_deallocate(&object->allocator, list, sizeof(FVizObserverList),
                              (FVizSize)FVIZ_INTERNAL_MAX_ALIGNMENT);
...
const FVizObjectClass g_fviz_object_class = {FVIZ_TYPE_OBJECT, "FVizObject", NULL, fviz_base_object_destroy, NULL};
...
{
    return (object != NULL && object->magic == FVIZ_OBJECT_MAGIC && object->object_class != NULL) ? FVIZ_TRUE
                                                                                                  : FVIZ_FALSE;
}
...
FVizObject* fviz_internal_object_allocate(FVizSize object_size, const FVizObjectClass* object_class,
                                          const FVizAllocator* allocator)
{
    ...
    object =
        (FVizObject*)fviz_allocator_allocate(&selected_allocator, object_size, (FVizSize)FVIZ_INTERNAL_MAX_ALIGNMENT);
    if (object == NULL)
    {
...
FVizResult fviz_object_create_with_allocator(const FVizAllocator* allocator, FVizObject** out_object)
{
    FVizObject* object;
...
void fviz_release(void* object_pointer)
{
    ...
    fviz_allocator_deallocate(&allocator, object, allocation_size, (FVizSize)FVIZ_INTERNAL_MAX_ALIGNMENT);
}
...
void fviz_object_modified(FVizObject* object)
{
    ...
    if (fviz_object_is_valid(object) == FVIZ_FALSE)
    {
        if (object != NULL) fviz_internal_set_error(FVIZ_ERROR_INVALID_ARGUMENT, "object is not a valid FEAViz object");
        return;
    }
    modified = fviz_object_next_mtime();
    expected = fviz_atomic_u64_load(&object->mtime);
    while (expected < modified && !fviz_atomic_u64_compare_exchange(&object->mtime, &expected, modified))
    {
    }
...
FVizMTime fviz_object_mtime(const FVizObject* object)
{
    ...
    return object->object_class->get_mtime != NULL ? object->object_class->get_mtime(object)
...
const char* fviz_event_name(FVizEventId event_id)
{
    switch (event_id)
    {
        case FVIZ_EVENT_ANY:
            return "AnyEvent";
        case FVIZ_EVENT_DELETE:
            return "DeleteEvent";
        case FVIZ_EVENT_START:
            return "StartEvent";
        case FVIZ_EVENT_END:
            return "EndEvent";
        case FVIZ_EVENT_MODIFIED:
            return "ModifiedEvent";
        case FVIZ_EVENT_RENDER_START:
            return "RenderStartEvent";
        case FVIZ_EVENT_RENDER_END:
            return "RenderEndEvent";
        case FVIZ_EVENT_WINDOW_RESIZE:
            return "WindowResizeEvent";
        case FVIZ_EVENT_WINDOW_CLOSE:
            return "WindowCloseEvent";
        case FVIZ_EVENT_WINDOW_FOCUS_IN:
            return "WindowFocusInEvent";
        case FVIZ_EVENT_WINDOW_FOCUS_OUT:
            return "WindowFocusOutEvent";
        case FVIZ_EVENT_WINDOW_DPI_CHANGED:
            return "WindowDpiChangedEvent";
        case FVIZ_EVENT_WINDOW_REPARENTED:
            return "WindowReparentedEvent";
        case FVIZ_EVENT_START_INTERACTION:
            return "StartInteractionEvent";
        case FVIZ_EVENT_INTERACTION:
            return "InteractionEvent";
        case FVIZ_EVENT_END_INTERACTION:
            return "EndInteractionEvent";
        case FVIZ_EVENT_ENABLE:
            return "EnableEvent";
        case FVIZ_EVENT_DISABLE:
            return "DisableEvent";
        case FVIZ_EVENT_START_PICK:
            return "StartPickEvent";
        case FVIZ_EVENT_PICK:
            return "PickEvent";
        case FVIZ_EVENT_END_PICK:
            return "EndPickEvent";
        case FVIZ_EVENT_PROGRESS:
            return "ProgressEvent";
        case FVIZ_EVENT_ABORT_CHECK:
            return "AbortCheckEvent";
        case FVIZ_EVENT_RENDER_REQUESTED:
            return "RenderRequestedEvent";
        case FVIZ_EVENT_INTERACTION_ANY:
            return "AnyInteractionEvent";
        case FVIZ_EVENT_MOUSE_BUTTON_DOWN:
            return "MouseButtonDownEvent";
        case FVIZ_EVENT_MOUSE_BUTTON_UP:
            return "MouseButtonUpEvent";
        case FVIZ_EVENT_MOUSE_MOVE:
            return "MouseMoveEvent";
        case FVIZ_EVENT_MOUSE_WHEEL:
            return "MouseWheelEvent";
        case FVIZ_EVENT_KEY_DOWN:
            return "KeyDownEvent";
        case FVIZ_EVENT_KEY_UP:
            return "KeyUpEvent";
        case FVIZ_EVENT_RESIZE:
            return "ResizeEvent";
        case FVIZ_EVENT_ENTER:
            return "EnterEvent";
        case FVIZ_EVENT_LEAVE:
            return "LeaveEvent";
        case FVIZ_EVENT_EXPOSE:
            return "ExposeEvent";
        case FVIZ_EVENT_FOCUS_IN:
            return "FocusInEvent";
        case FVIZ_EVENT_FOCUS_OUT:
            return "FocusOutEvent";
        case FVIZ_EVENT_TIMER:
            return "TimerEvent";
        case FVIZ_EVENT_DOUBLE_CLICK:
            return "DoubleClickEvent";
        case FVIZ_EVENT_CHAR:
            return "CharEvent";
        default:
            return event_id >= FVIZ_EVENT_USER ? "UserEvent" : "UnknownEvent";
    }
}

FVizResult fviz_object_add_observer(FVizObject* object, FVizEventId event_id, float priority,
                                    FVizObserverCallbackFn callback, void* client_data, FVizObserverTag* out_tag)
{
    FVizObserverList* list;
    ...
    if (fviz_object_observer_reserve(object, list, list->count + 1u) != FVIZ_OK) return fviz_last_error_code();
...
FVizResult fviz_object_add_command_observer(FVizObject* object, FVizEventId event_id, float priority,
                                            FVizCommand* command, FVizObserverTag* out_tag)
{
    ...
    if (fviz_object_is_valid(object) == FVIZ_FALSE || command == NULL || out_tag == NULL ||
        !isfinite((double)priority) || fviz_object_is_type((const FVizObject*)command, FVIZ_TYPE_COMMAND) == FVIZ_FALSE)
    {
    ...
    for (i = 0u; i < list->count; ++i)
    {
        if (list->observers[i].active != FVIZ_FALSE &&
            (event_id == FVIZ_EVENT_ANY ||
             ...
             (event_id == FVIZ_EVENT_INTERACTION_ANY && list->observers[i].event_id >= FVIZ_EVENT_MOUSE_BUTTON_DOWN &&
...
FVizBool fviz_object_invoke_event(FVizObject* object, FVizEventId event_id, void* call_data)
{
    ...
    for (i = 0u; i < dispatch_count; ++i)
    {
        ...
        if (observer->active != FVIZ_FALSE && observer->activation_dispatch < list->dispatch_serial &&
