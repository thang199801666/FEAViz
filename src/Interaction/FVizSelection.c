...
static const FVizObjectClass g_fviz_named_selection_collection_class = {
    FVIZ_TYPE_NAMED_SELECTION_COLLECTION,
    "FVizNamedSelectionCollection",
    &g_fviz_object_class,
    fviz_named_selection_collection_destroy,
    NULL
};
...
static FVizSize fviz_selection_find_identity(
    ...
    FVizSize id)
{
    ...
    for (i = 0u; i < fviz_array_count(selection->items); ++i)
    {
        ...
        if (item != NULL && item->record.actor == actor &&
            item->record.association == association && item->record.rendered_id == id)
            return i;
    }
    return SIZE_MAX;
}
    ...
    FVizSize id)
{
    ...
    if (association == FVIZ_SELECTION_POINT)
    {
        ...
        if (data != NULL && fviz_provenance_resolve(
                fviz_poly_data_const_point_data(data),
                FVIZ_PROVENANCE_POINT,
                id,
                FVIZ_INVALID_ID,
                &record.original_point_id,
                &persistent) == FVIZ_OK)
    ...
    else if (association == FVIZ_SELECTION_CELL)
    {
        ...
        if (data != NULL)
        {
            (void)fviz_provenance_resolve(
                ...
                FVIZ_PROVENANCE_CELL,
                id,
                FVIZ_INVALID_ID,
                &record.original_cell_id,
                &cell_persistent);
            (void)fviz_provenance_resolve(
                fviz_poly_data_const_cell_data(data),
                FVIZ_PROVENANCE_FACE,
                id,
                FVIZ_INVALID_ID,
                &record.original_face_id,
                &face_persistent);
        }
    ...
    const FVizSelectionRecord* record)
{
    ...
    if (selection == NULL || record == NULL ||
        ...
        record->association < FVIZ_SELECTION_ACTOR || record->association > FVIZ_SELECTION_GLYPH_INSTANCE ||
...
FVizResult fviz_selection_refresh(FVizSelection* selection)
{
    ...
    for (i = 0u; i < fviz_array_count(selection->items); ++i)
    {
        ...
        if (record->association == FVIZ_SELECTION_ACTOR)
            ...
            record->state = glyph_mapper != NULL &&
                record->rendered_id < fviz_glyph_mapper_instance_count(glyph_mapper)
            ...
            record->state = data != NULL && fviz_provenance_find(
                fviz_poly_data_const_point_data(data), FVIZ_PROVENANCE_POINT,
                record->original_point_id, &record->rendered_id) == FVIZ_OK
                ? FVIZ_SELECTION_VALID : FVIZ_SELECTION_INVALID;
        else if (record->association == FVIZ_SELECTION_CELL)
            record->state = data != NULL && fviz_provenance_find(
                fviz_poly_data_const_cell_data(data), FVIZ_PROVENANCE_CELL,
...
FVizBool fviz_selection_contains(
    const FVizSelection* selection,
    const FVizActor* actor,
    FVizSelectionAssociation association,
    FVizSize id)
{
    if (association < FVIZ_SELECTION_ACTOR || association > FVIZ_SELECTION_GLYPH_INSTANCE)
        return FVIZ_FALSE;
    return fviz_selection_find_identity(selection, actor, association, id) != SIZE_MAX
        ? FVIZ_TRUE : FVIZ_FALSE;
}

FVizResult fviz_selection_remove(
    FVizSelection* selection,
    const FVizActor* actor,
    FVizSelectionAssociation association,
    FVizSize id)
{
    ...
    if (index + 1u < count)
        (void)memmove(&items[index], &items[index + 1u],
            (size_t)(count - index - 1u) * sizeof(*items));
    if (fviz_array_resize(selection->items, count - 1u) != FVIZ_OK)
        return fviz_last_error_code();
    fviz_object_modified((FVizObject*)selection);
    return FVIZ_OK;
}

FVizResult fviz_selection_copy(
    const FVizSelection* source,
...
FVizResult fviz_selection_apply(
    FVizSelection* selection,
    const FVizSelection* incoming,
    FVizSelectionModifier modifier)
{
    FVizSelection* snapshot = NULL;
    const FVizSelection* source = incoming;
    FVizSize i;
    if (selection == NULL || incoming == NULL || modifier < FVIZ_SELECTION_REPLACE ||
        modifier > FVIZ_SELECTION_TOGGLE) return FVIZ_ERROR_INVALID_ARGUMENT;
    ...
    for (i = 0u; i < fviz_selection_count(source); ++i)
    {
        ...
        contains = fviz_selection_contains(
            selection, record.actor, record.association, record.rendered_id);
        if (modifier == FVIZ_SELECTION_SUBTRACT ||
            (modifier == FVIZ_SELECTION_TOGGLE && contains != FVIZ_FALSE))
        {
            if (contains != FVIZ_FALSE)
                (void)fviz_selection_remove(
                    selection, record.actor, record.association, record.rendered_id);
        }
    ...
    const char* array_name)
{
    ...
    if (record->association == FVIZ_SELECTION_GLYPH_INSTANCE)
    {
        ...
        record->world_position = fviz_vec3(
            model.m[0] * instance.position.x + model.m[4] * instance.position.y + model.m[8] * instance.position.z + model.m[12],
            model.m[1] * instance.position.x + model.m[5] * instance.position.y + model.m[9] * instance.position.z + model.m[13],
            model.m[2] * instance.position.x + model.m[6] * instance.position.y + model.m[10] * instance.position.z + model.m[14]);
...
FVizResult fviz_selection_create_mask(
    const FVizSelection* selection,
    const FVizActor* actor,
    FVizSelectionAssociation association,
    ...
    FVizDataArray** out_mask)
{
    ...
    uint8_t* values;
    FVizSize index;
    if (selection == NULL || actor == NULL || out_mask == NULL)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    *out_mask = NULL;
    ...
    {
        const FVizSelectionRecord* record = &((const FVizSelectionItem*)
            fviz_array_const_at(selection->items, index))->record;
        if (record->actor == actor && record->association == association &&
            record->state == FVIZ_SELECTION_VALID && record->rendered_id < value_count)
...
FVizResult fviz_selection_convert_association(
    const FVizSelection* selection,
    FVizActor* actor,
    ...
    FVizSelection** out_selection)
{
    ...
    if (selection == NULL || actor == NULL || out_selection == NULL ||
        (target_association != FVIZ_SELECTION_POINT &&
         target_association != FVIZ_SELECTION_CELL))
        return FVIZ_ERROR_INVALID_ARGUMENT;
    *out_selection = NULL;
    data = fviz_actor_const_poly_data(actor);
    ...
    for (index = 0u; index < fviz_array_count(selection->items); ++index)
    {
        const FVizSelectionRecord* record = &((const FVizSelectionItem*)
            fviz_array_const_at(selection->items, index))->record;
        FVizSize triangle;
        if (record->actor != actor || record->state != FVIZ_SELECTION_VALID)
            continue;
        if (record->association == target_association)
        {
            if (fviz_selection_add(converted, actor, target_association,
                record->rendered_id) != FVIZ_OK) goto fail;
        }
        else if (record->association == FVIZ_SELECTION_POINT &&
                 target_association == FVIZ_SELECTION_CELL)
        {
            for (triangle = 0u; triangle < triangle_count; ++triangle)
            {
                ...
                if (ids[0] == record->rendered_id || ids[1] == record->rendered_id ||
                    ids[2] == record->rendered_id)
                    if (fviz_selection_add(converted, actor,
                        FVIZ_SELECTION_CELL, triangle) != FVIZ_OK) goto fail;
            }
        }
        else if (record->association == FVIZ_SELECTION_CELL &&
                 target_association == FVIZ_SELECTION_POINT)
        {
            ...
            for (corner = 0u; corner < 3u; ++corner)
                if (fviz_selection_add(converted, actor,
                    FVIZ_SELECTION_POINT, ids[corner]) != FVIZ_OK) goto fail;
        }
        else
        {
...
static FVizResult fviz_selection_gather_attributes(
    const FVizAttributeSet* source,
    FVizAttributeSet* destination,
    const FVizSize* tuple_ids,
    FVizSize tuple_count)
{
    ...
    for (array_index = 0u; array_index < fviz_attribute_set_count(source); ++array_index)
    {
        const FVizDataArray* source_array =
            fviz_attribute_set_const_array_at(source, array_index);
        ...
        if (fviz_data_array_create(fviz_data_array_type(source_array),
            fviz_data_array_components(source_array), &output) != FVIZ_OK) return fviz_last_error_code();
        for (tuple = 0u; tuple < tuple_count; ++tuple)
        {
            ...
            { fviz_release(output); return FVIZ_ERROR_INVALID_STATE; }
        }
...
FVizResult fviz_selection_extract_poly_data(
    const FVizSelection* selection,
    const FVizActor* actor,
    FVizSelectionAssociation association,
    FVizPolyData** out_poly_data)
{
    ...
    *out_poly_data = NULL;
    ...
    if (fviz_size_multiply(fviz_poly_data_point_count(source), sizeof(*point_map), &bytes) != FVIZ_OK)
        goto fail;
    point_map = (uint32_t*)fviz_alloc(bytes);
    if (fviz_size_multiply(fviz_poly_data_point_count(source), sizeof(*point_ids), &bytes) != FVIZ_OK)
        goto fail;
    point_ids = (FVizSize*)fviz_alloc(bytes);
    if (fviz_size_multiply(fviz_poly_data_triangle_count(source), sizeof(*cell_ids), &bytes) != FVIZ_OK)
        goto fail;
    ...
    if ((point_map == NULL && fviz_poly_data_point_count(source) != 0u) ||
        ...
        (cell_ids == NULL && fviz_poly_data_triangle_count(source) != 0u) ||
        fviz_poly_data_create(&output) != FVIZ_OK) goto fail;
    ...
    for (index = 0u; index < fviz_array_count(selection->items); ++index)
    {
        const FVizSelectionRecord* record = &((const FVizSelectionItem*)
            fviz_array_const_at(selection->items, index))->record;
        if (record->actor != actor || record->association != association ||
            record->state != FVIZ_SELECTION_VALID) continue;
        if (association == FVIZ_SELECTION_POINT)
        {
            FVizVec3 point;
            uint32_t new_id;
            ...
            if (fviz_poly_data_get_point(source, record->rendered_id, &point) != FVIZ_OK ||
                ...
                fviz_poly_data_add_vertex(output, new_id) != FVIZ_OK) goto fail;
        ...
        else
        {
            ...
            if (fviz_poly_data_add_triangle(
                output, new_ids[0], new_ids[1], new_ids[2]) != FVIZ_OK) goto fail;
            cell_ids[cell_count++] = record->rendered_id;
        }
    }
    if (fviz_selection_gather_attributes(fviz_poly_data_const_point_data(source),
            fviz_poly_data_point_data(output), point_ids, point_count) != FVIZ_OK ||
        (association == FVIZ_SELECTION_CELL &&
         fviz_selection_gather_attributes(fviz_poly_data_const_cell_data(source),
            fviz_poly_data_cell_data(output), cell_ids, cell_count) != FVIZ_OK)) goto fail;
    for (index = 0u; index < fviz_attribute_set_count(fviz_poly_data_const_field_data(source)); ++index)
    {
        const FVizDataArray* array = fviz_attribute_set_const_array_at(
            fviz_poly_data_const_field_data(source), index);
        ...
        if (array != NULL && fviz_data_array_deep_copy(array, &copy) == FVIZ_OK)
        {
            if (fviz_attribute_set_add(fviz_poly_data_field_data(output),
                fviz_attribute_set_name_at(fviz_poly_data_const_field_data(source), index), copy) != FVIZ_OK)
            { fviz_release(copy); goto fail; }
            fviz_release(copy);
        }
    }
    fviz_free(cell_ids); fviz_free(point_ids); fviz_free(point_map);
...
static FVizSize fviz_named_selection_collection_find(
    const FVizNamedSelectionCollection* collection, const char* name)
{
    FVizSize index;
    if (collection == NULL || name == NULL) return SIZE_MAX;
    for (index = 0u; index < fviz_array_count(collection->entries); ++index)
    {
        const FVizNamedSelectionEntry* entry = (const FVizNamedSelectionEntry*)
            fviz_array_const_at(collection->entries, index);
...
void fviz_named_selection_collection_clear(FVizNamedSelectionCollection* collection)
{
    ...
    for (index = 0u; index < fviz_array_count(collection->entries); ++index)
    {
        FVizNamedSelectionEntry* entry = (FVizNamedSelectionEntry*)
            fviz_array_at(collection->entries, index);
...
FVizResult fviz_named_selection_collection_create(
    FVizNamedSelectionCollection** out_collection)
{
    ...
    *out_collection = NULL;
    ...
    if (fviz_array_create(sizeof(FVizNamedSelectionEntry),
        &collection->entries) != FVIZ_OK)
    {
...
FVizResult fviz_named_selection_collection_set(
    FVizNamedSelectionCollection* collection,
    const char* name,
    FVizSelection* selection)
{
    ...
    if (collection == NULL || name == NULL || name[0] == '\0' || selection == NULL)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    index = fviz_named_selection_collection_find(collection, name);
    if (index != SIZE_MAX)
    {
        FVizNamedSelectionEntry* current = (FVizNamedSelectionEntry*)
            fviz_array_at(collection->entries, index);
...
FVizResult fviz_named_selection_collection_remove(
    FVizNamedSelectionCollection* collection, const char* name)
{
    FVizSize index;
...
FVizSize fviz_named_selection_collection_count(
    const FVizNamedSelectionCollection* collection)
{
    return collection != NULL ? fviz_array_count(collection->entries) : 0u;
}

const char* fviz_named_selection_collection_name(
    const FVizNamedSelectionCollection* collection, FVizSize index)
{
    const FVizNamedSelectionEntry* entry = collection != NULL
...
FVizSelection* fviz_named_selection_collection_get(
    FVizNamedSelectionCollection* collection, const char* name)
{
    const FVizSize index = fviz_named_selection_collection_find(collection, name);
    FVizNamedSelectionEntry* entry = index != SIZE_MAX
        ? (FVizNamedSelectionEntry*)fviz_array_at(collection->entries, index) : NULL;
...
static FVizBool fviz_selection_point_on_segment(
    double px, double py, double ax, double ay, double bx, double by)
{
    const double cross = (px - ax) * (by - ay) - (py - ay) * (bx - ax);
    const double scale = 1.0 + ((bx - ax) < 0.0 ? -(bx - ax) : (bx - ax)) +
        ((by - ay) < 0.0 ? -(by - ay) : (by - ay));
...
static FVizBool fviz_selection_point_in_polygon(
    const int* xy_points, FVizSize point_count, double x, double y)
{
    ...
    for (i = 0u; i < point_count; ++i)
    {
        ...
        if (fviz_selection_point_on_segment(x, y, xj, yj, xi, yi) != FVIZ_FALSE)
            return FVIZ_TRUE;
...
static double fviz_selection_orientation(
    double ax, double ay, double bx, double by, double cx, double cy)
{
    return (bx - ax) * (cy - ay) - (by - ay) * (cx - ax);
}

static FVizBool fviz_selection_segments_intersect(
...
static FVizBool fviz_selection_segment_intersects_polygon(
    const int* xy_points, FVizSize point_count, double ax, double ay, double bx, double by)
{
    FVizSize i;
    FVizSize j;
    ...
    for (i = 0u; i < point_count; ++i)
    {
        if (fviz_selection_segments_intersect(
                ax, ay, bx, by,
                (double)xy_points[j * 2u], (double)xy_points[j * 2u + 1u],
...
static FVizBool fviz_selection_point_in_triangle(
    double px, double py, const double sx[3], const double sy[3])
{
...
static FVizBool fviz_selection_triangle_intersects_polygon(
    const int* xy_points, FVizSize point_count, const double sx[3], const double sy[3])
{
    FVizSize i;
    FVizSize edge;
    for (i = 0u; i < 3u; ++i)
        if (fviz_selection_point_in_polygon(xy_points, point_count, sx[i], sy[i]) != FVIZ_FALSE)
            return FVIZ_TRUE;
    for (i = 0u; i < point_count; ++i)
        if (fviz_selection_point_in_triangle(
                (double)xy_points[i * 2u], (double)xy_points[i * 2u + 1u], sx, sy) != FVIZ_FALSE)
            return FVIZ_TRUE;
    for (edge = 0u; edge < 3u; ++edge)
        if (fviz_selection_segment_intersects_polygon(
                xy_points, point_count, sx[edge], sy[edge], sx[(edge + 1u) % 3u], sy[(edge + 1u) % 3u]) != FVIZ_FALSE)
            return FVIZ_TRUE;
    return FVIZ_FALSE;
}

static FVizBool fviz_selection_project_world(
    FVizRenderer* renderer, FVizVec3 world, int width, int height, double* out_x, double* out_y)
{
    FVizVec3 display;
    if (fviz_renderer_world_to_display(renderer, world, width, height, &display) != FVIZ_OK)
        return FVIZ_FALSE;
...
static FVizBool fviz_selection_actor_region_intersects(
    FVizRenderer* renderer, const FVizActor* actor, int width, int height,
    const int* xy_points, FVizSize point_count)
{
    ...
    for (i = 0u; i < 8u; ++i)
    {
        double x;
        double y;
        if (fviz_selection_project_world(renderer, corners[i], width, height, &x, &y) == FVIZ_FALSE)
            continue;
        if (fviz_selection_point_in_polygon(xy_points, point_count, x, y) != FVIZ_FALSE)
            return FVIZ_TRUE;
...
static FVizBounds fviz_selection_bounds_from_points(const FVizVec3* points, FVizSize count)
{
    ...
    FVizSize i;
    for (i = 0u; i < count; ++i) fviz_bounds_include_point(&bounds, points[i]);
    return bounds;
}

FVizResult fviz_selection_select_frustum(
    FVizRenderer* renderer,
    const FVizFrustum* frustum,
    FVizSelectionAssociation association,
    FVizSelection** out_selection)
{
    ...
    *out_selection = NULL;
    ...
    for (actor_index = 0u; scene != NULL && actor_index < fviz_scene_actor_count(scene); ++actor_index)
    {
        ...
        if (actor == NULL || fviz_actor_is_visible(actor) == FVIZ_FALSE ||
            fviz_actor_pickable(actor) == FVIZ_FALSE ||
        ...
        if (association == FVIZ_SELECTION_GLYPH_INSTANCE)
        {
            ...
            for (instance_index = 0u; instance_index < fviz_glyph_mapper_instance_count(glyphs); ++instance_index)
            {
                ...
                if (fviz_glyph_mapper_get_instance(glyphs, instance_index, &instance) == FVIZ_OK &&
                    fviz_frustum_contains_point(frustum,
                        fviz_selection_transform_point(model, instance.position)) != FVIZ_FALSE &&
        ...
        }
        {
            ...
            if (association == FVIZ_SELECTION_POINT)
            {
                ...
                for (point_index = 0u; point_index < fviz_poly_data_point_count(data); ++point_index)
                    if (fviz_frustum_contains_point(frustum,
                            fviz_selection_transform_point(model, points[point_index])) != FVIZ_FALSE &&
            ...
            else
            {
                ...
                for (triangle_index = 0u; triangle_index < fviz_poly_data_triangle_count(data); ++triangle_index)
                {
                    ...
                    FVizVec3 world[3] = {
                        fviz_selection_transform_point(model, points[tri[0]]),
                    ...
                    {
                        if (fviz_frustum_intersects_bounds(frustum,
                                fviz_selection_bounds_from_points(world, 3u)) != FVIZ_FALSE &&
                    ...
                    else if (association == FVIZ_SELECTION_EDGE)
                    {
                        ...
                        for (edge = 0u; edge < 3u; ++edge)
                        {
                            ...
                            if (fviz_frustum_intersects_bounds(frustum,
                                    fviz_selection_bounds_from_points(edge_points, 2u)) != FVIZ_FALSE &&
                                fviz_selection_add(selection, actor, association,
                                    triangle_index * 3u + edge) != FVIZ_OK)
                                goto fail;
                        }
                    }
                }
...
static FVizResult fviz_selection_region_visit_candidate(
    ...
    FVizSelectionRegionStatistics* statistics)
{
    ...
    interval = options->cancellation_check_interval != 0u
        ? options->cancellation_check_interval : 1024u;
    if ((statistics->candidates_tested % interval) == 0u &&
        options->cancel(options->cancel_user_data) != FVIZ_FALSE)
    {
...
static FVizResult fviz_selection_region_add(
    FVizSelection* selection,
    FVizActor* actor,
    FVizSelectionAssociation association,
    FVizSize id,
    ...
    FVizSelectionRegionStatistics* statistics)
{
    if (options->maximum_results != 0u &&
        fviz_selection_count(selection) >= options->maximum_results)
    {
...
static FVizResult fviz_selection_select_polygon_internal(
    FVizRenderer* renderer,
    int display_width,
    int display_height,
    const int* xy_points,
    ...
    FVizSelection** out_selection)
{
    ...
    if (renderer == NULL || display_width <= 0 || display_height <= 0 || xy_points == NULL ||
        point_count < 3u || out_selection == NULL || association < FVIZ_SELECTION_ACTOR ||
        association > FVIZ_SELECTION_GLYPH_INSTANCE)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    *out_selection = NULL;
    ...
    for (actor_index = 0u; scene != NULL && actor_index < fviz_scene_actor_count(scene); ++actor_index)
    {
        ...
        if (actor == NULL || fviz_actor_is_visible(actor) == FVIZ_FALSE ||
            ...
            fviz_renderer_actor_is_renderable(renderer, actor, aspect_ratio,
                (int)(((viewport[3] - viewport[1]) * (float)display_height) > 1.0f
                    ? ((viewport[3] - viewport[1]) * (float)display_height) : 1.0f)) == FVIZ_FALSE)
            continue;
        if (association == FVIZ_SELECTION_GLYPH_INSTANCE)
        {
            const FVizGlyphMapper* glyphs = fviz_actor_const_glyph_mapper(actor);
            ...
            for (instance_index = 0u; instance_index < fviz_glyph_mapper_instance_count(glyphs); ++instance_index)
            {
                ...
                if (fviz_glyph_mapper_get_instance(glyphs, instance_index, &instance) == FVIZ_OK &&
                    fviz_selection_project_world(renderer,
                        fviz_selection_transform_point(model, instance.position),
                        display_width, display_height, &sx, &sy) != FVIZ_FALSE &&
                    fviz_selection_point_in_polygon(xy_points, point_count, sx, sy) != FVIZ_FALSE &&
                    (candidate_result = fviz_selection_region_add(
                        selection, actor, association, instance_index, options, statistics)) != FVIZ_OK)
                {
        ...
        if (association == FVIZ_SELECTION_ACTOR)
        {
            candidate_result = fviz_selection_region_visit_candidate(options, statistics);
            if (candidate_result != FVIZ_OK) goto fail;
            if (fviz_selection_actor_region_intersects(
                    renderer, actor, display_width, display_height, xy_points, point_count) != FVIZ_FALSE &&
                (candidate_result = fviz_selection_region_add(
                    selection, actor, association, 0u, options, statistics)) != FVIZ_OK)
            {
                if (candidate_result == FVIZ_ERROR_OVERFLOW) goto complete;
                goto fail;
            }
            ...
            if (association == FVIZ_SELECTION_POINT)
            {
                ...
                for (point_index = 0u; point_index < fviz_poly_data_point_count(poly_data); ++point_index)
                {
                    ...
                    if (fviz_selection_project_world(renderer,
                        ...
                        fviz_selection_point_in_polygon(xy_points, point_count, sx, sy) != FVIZ_FALSE &&
                        (candidate_result = fviz_selection_region_add(
                            selection, actor, association, point_index, options, statistics)) != FVIZ_OK)
                    {
                        if (candidate_result == FVIZ_ERROR_OVERFLOW) goto complete;
                        goto fail;
                    }
            ...
            else
            {
                ...
                for (triangle_index = 0u; triangle_index < fviz_poly_data_triangle_count(poly_data); ++triangle_index)
                {
                    const uint32_t ids[3] = {
                        indices[triangle_index * 3u],
                        indices[triangle_index * 3u + 1u],
                    ...
                    {
                        if (fviz_selection_project_world(renderer,
                                fviz_selection_transform_point(model, points[ids[corner]]),
                                display_width, display_height, &sx[corner], &sy[corner]) == FVIZ_FALSE)
                        {
                    ...
                    if (association == FVIZ_SELECTION_CELL)
                    {
                        if (fviz_selection_triangle_intersects_polygon(
                                xy_points, point_count, sx, sy) != FVIZ_FALSE &&
                            (candidate_result = fviz_selection_region_add(
                                selection, actor, association, triangle_index, options, statistics)) != FVIZ_OK)
                        {
                            if (candidate_result == FVIZ_ERROR_OVERFLOW) goto complete;
                            goto fail;
                        }
                    ...
                    else if (association == FVIZ_SELECTION_EDGE)
                    {
                        ...
                        for (edge = 0u; edge < 3u; ++edge)
                        {
                            ...
                            if (candidate_result != FVIZ_OK) goto fail;
                            if (fviz_selection_segment_intersects_polygon(
                                    xy_points, point_count,
                                    sx[edge], sy[edge], sx[(edge + 1u) % 3u], sy[(edge + 1u) % 3u]) != FVIZ_FALSE &&
                                (candidate_result = fviz_selection_region_add(
                                    selection, actor, association,
                                    triangle_index * 3u + edge, options, statistics)) != FVIZ_OK)
                            {
                                if (candidate_result == FVIZ_ERROR_OVERFLOW) goto complete;
                                goto fail;
                            }
...
FVizResult fviz_selection_select_polygon_with_options(
    FVizRenderer* renderer,
    int display_width,
    int display_height,
    const int* xy_points,
    ...
    FVizSelection** out_selection)
{
    ...
    return fviz_selection_select_polygon_internal(
        renderer, display_width, display_height, xy_points, point_count,
        association, applied, reported, out_selection);
}

FVizResult fviz_selection_select_polygon(
    FVizRenderer* renderer,
    int display_width,
    int display_height,
    const int* xy_points,
    ...
    FVizSelection** out_selection)
{
    return fviz_selection_select_polygon_with_options(
        renderer, display_width, display_height, xy_points, point_count,
        association, NULL, NULL, out_selection);
}

FVizResult fviz_selection_select_rectangle(
    FVizRenderer* renderer,
    int display_width,
    int display_height,
    int start_x,
    ...
    FVizSelection** out_selection)
{
    ...
    return fviz_selection_select_polygon(
        renderer, display_width, display_height, polygon, 4u, association, out_selection);
}
