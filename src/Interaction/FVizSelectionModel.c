...
static const FVizObjectClass g_fviz_selection_model_class = {
    FVIZ_TYPE_SELECTION_MODEL,
    "FVizSelectionModel",
    &g_fviz_object_class,
    fviz_selection_model_destroy,
    NULL
};

static FVizBool fviz_selection_model_valid_association(FVizSelectionAssociation association)
{
    return association >= FVIZ_SELECTION_ACTOR && association <= FVIZ_SELECTION_GLYPH_INSTANCE
        ? FVIZ_TRUE : FVIZ_FALSE;
}

static FVizBool fviz_selection_model_valid_modifier(FVizSelectionModifier modifier)
{
    return modifier >= FVIZ_SELECTION_REPLACE && modifier <= FVIZ_SELECTION_TOGGLE
        ? FVIZ_TRUE : FVIZ_FALSE;
}
...
FVizResult fviz_selection_model_create(FVizSelectionModel** out_model)
{
    ...
    *out_model = NULL;
    model = (FVizSelectionModel*)fviz_internal_object_allocate(
        sizeof(*model), &g_fviz_selection_model_class, NULL);
...
void fviz_selection_model_set_association(
    FVizSelectionModel* model, FVizSelectionAssociation association)
{
...
void fviz_selection_model_set_modifier(
    FVizSelectionModel* model, FVizSelectionModifier modifier)
{
    if (model == NULL || fviz_selection_model_valid_modifier(modifier) == FVIZ_FALSE ||
...
FVizResult fviz_selection_model_apply(
    FVizSelectionModel* model,
    const FVizSelection* incoming,
    FVizSelectionModifier modifier)
{
    FVizResult result;
    if (model == NULL || incoming == NULL ||
...
FVizResult fviz_selection_model_select_at(
    FVizSelectionModel* model,
...
FVizResult fviz_selection_model_select_rectangle(
    FVizSelectionModel* model,
    FVizRenderWindow* window,
    int start_x,
    int start_y,
    ...
    FVizSelectionModifier modifier)
{
    ...
    if (model == NULL || window == NULL ||
        fviz_selection_model_valid_modifier(modifier) == FVIZ_FALSE)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    result = fviz_render_window_select_rectangle_association(
        window, start_x, start_y, end_x, end_y, model->association, &incoming);
    if (result != FVIZ_OK) return result;
    result = fviz_selection_model_apply(model, incoming, modifier);
    fviz_release(incoming);
    return result;
...
FVizResult fviz_selection_model_select_polygon(
    FVizSelectionModel* model,
    FVizRenderWindow* window,
    const int* xy_points,
    ...
    FVizSelectionModifier modifier)
{
    ...
    if (model == NULL || window == NULL || xy_points == NULL || point_count < 3u ||
        fviz_selection_model_valid_modifier(modifier) == FVIZ_FALSE)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    result = fviz_render_window_select_polygon(
        window, xy_points, point_count, model->association, &incoming);
    if (result != FVIZ_OK) return result;
    result = fviz_selection_model_apply(model, incoming, modifier);
    fviz_release(incoming);
    return result;
...
FVizResult fviz_selection_model_select_frustum(
    FVizSelectionModel* model,
    ...
    FVizSelectionModifier modifier)
{
    FVizSelection* incoming = NULL;
    FVizResult result;
    if (model == NULL || renderer == NULL || frustum == NULL ||
        modifier < FVIZ_SELECTION_REPLACE || modifier > FVIZ_SELECTION_TOGGLE)
        return FVIZ_ERROR_INVALID_ARGUMENT;
    result = fviz_selection_select_frustum(
        renderer, frustum, model->association, &incoming);
    if (result != FVIZ_OK) return result;
    result = fviz_selection_model_apply(model, incoming, modifier);
    fviz_release(incoming);
    return result;
...
void fviz_selection_model_set_hover_update_rate(
    FVizSelectionModel* model, float updates_per_second)
{
...
FVizResult fviz_selection_model_process_hover_event(
    FVizSelectionModel* model,
    FVizRenderWindow* window,
    const FVizInteractionEvent* event)
{
    ...
    if (model->have_hover_sample != FVIZ_FALSE &&
        model->last_hover_x == event->x && model->last_hover_y == event->y)
        return FVIZ_OK;
    minimum_interval = model->hover_update_rate > 0.0f
        ? 1.0 / (double)model->hover_update_rate : 0.0;
    if (model->have_hover_sample != FVIZ_FALSE && event->timestamp_seconds > 0.0 &&
        model->last_hover_timestamp > 0.0 &&
...
FVizResult fviz_selection_model_update_hover(
    FVizSelectionModel* model,
    FVizRenderWindow* window,
    int x,
    int y)
