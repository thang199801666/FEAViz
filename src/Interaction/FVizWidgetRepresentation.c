...
static FVizMTime fviz_widget_representation_entries_mtime(const FVizWidgetRepresentationEntry* entries, FVizSize count,
                                                          FVizMTime mtime)
{
    FVizSize i;
    for (i = 0u; i < count; ++i)
...
static FVizMTime fviz_widget_representation_mtime(const FVizObject* object)
{
    ...
    mtime = fviz_widget_representation_entries_mtime(representation->actors, representation->actor_count, mtime);
    mtime = fviz_widget_representation_entries_mtime(representation->text_actors_2d,
                                                     representation->text_actor_2d_count, mtime);
    mtime = fviz_widget_representation_entries_mtime(representation->billboard_text_actors_3d,
                                                     representation->billboard_text_actor_3d_count, mtime);
    return fviz_widget_representation_entries_mtime(representation->label_sets_3d, representation->label_set_3d_count,
                                                    mtime);
}

static FVizResult fviz_widget_representation_entries_reserve(FVizWidgetRepresentationEntry** entries,
                                                             FVizSize* capacity, FVizSize required)
{
    ...
    if (fviz_size_multiply(new_capacity, sizeof(**entries), &bytes) != FVIZ_OK) return fviz_last_error_code();
...
static FVizBool fviz_widget_representation_object_visible(FVizWidgetRepresentationKind kind, FVizObject* object)
{
    switch (kind)
    {
...
static void fviz_widget_representation_apply_visibility(FVizWidgetRepresentation* representation,
                                                        FVizWidgetRepresentationKind kind,
                                                        FVizWidgetRepresentationEntry* entry)
{
    const FVizBool visible =
        representation->visible != FVIZ_FALSE && entry->local_visible != FVIZ_FALSE ? FVIZ_TRUE : FVIZ_FALSE;
    switch (kind)
    {
        case FVIZ_WIDGET_REPRESENTATION_ACTOR:
...
static FVizResult fviz_widget_representation_add_to_renderer(FVizWidgetRepresentation* representation,
                                                             FVizWidgetRepresentationKind kind, FVizObject* object)
{
    switch (kind)
    {
        case FVIZ_WIDGET_REPRESENTATION_ACTOR:
        ...
        case FVIZ_WIDGET_REPRESENTATION_BILLBOARD_3D:
            return fviz_renderer_add_billboard_text_actor_3d(representation->renderer,
                                                             (FVizBillboardTextActor3D*)object);
        case FVIZ_WIDGET_REPRESENTATION_LABEL_SET_3D:
...
static void fviz_widget_representation_remove_from_renderer(FVizWidgetRepresentation* representation,
                                                            FVizWidgetRepresentationKind kind, FVizObject* object)
{
    switch (kind)
    {
        ...
        case FVIZ_WIDGET_REPRESENTATION_TEXT_2D:
            ...
            break;
        case FVIZ_WIDGET_REPRESENTATION_BILLBOARD_3D:
            (void)fviz_renderer_remove_billboard_text_actor_3d(representation->renderer,
                                                               (FVizBillboardTextActor3D*)object);
            break;
        case FVIZ_WIDGET_REPRESENTATION_LABEL_SET_3D:
...
static FVizResult fviz_widget_representation_add_entry(FVizWidgetRepresentation* representation,
                                                       FVizWidgetRepresentationKind kind,
                                                       FVizWidgetRepresentationEntry** entries, FVizSize* count,
                                                       FVizSize* capacity, FVizObject* object)
{
    FVizSize i;
    FVizWidgetRepresentationEntry* entry;
    ...
    if (kind == FVIZ_WIDGET_REPRESENTATION_ACTOR) fviz_actor_set_pickable((FVizActor*)object, FVIZ_FALSE);
...
static FVizResult fviz_widget_representation_remove_entry(FVizWidgetRepresentation* representation,
                                                          FVizWidgetRepresentationKind kind,
                                                          FVizWidgetRepresentationEntry* entries, FVizSize* count,
                                                          FVizObject* object)
{
    ...
    for (i = 0u; i < *count; ++i)
    {
        if (entries[i].object == object)
        {
            ...
            fviz_release(object);
            if (i + 1u < *count)
                (void)memmove(&entries[i], &entries[i + 1u], (size_t)(*count - i - 1u) * sizeof(entries[0]));
            --(*count);
            fviz_object_modified((FVizObject*)representation);
            return FVIZ_OK;
        }
...
static FVizResult fviz_widget_representation_set_entry_visible(FVizWidgetRepresentation* representation,
                                                               FVizWidgetRepresentationKind kind,
                                                               FVizWidgetRepresentationEntry* entries, FVizSize count,
                                                               FVizObject* object, FVizBool visible)
{
    FVizSize i;
...
static void fviz_widget_representation_clear_entries(FVizWidgetRepresentation* representation,
                                                     FVizWidgetRepresentationKind kind,
                                                     FVizWidgetRepresentationEntry* entries, FVizSize* count)
{
    FVizSize i;
...
static const FVizObjectClass g_fviz_widget_representation_class = {
    FVIZ_TYPE_WIDGET_REPRESENTATION, "FVizWidgetRepresentation", &g_fviz_object_class,
    fviz_widget_representation_destroy, fviz_widget_representation_mtime};

FVizResult fviz_widget_representation_create(FVizRenderer* renderer, FVizWidgetRepresentation** out_representation)
{
...
{
    return fviz_widget_representation_add_entry(
        representation, FVIZ_WIDGET_REPRESENTATION_ACTOR, representation != NULL ? &representation->actors : NULL,
...
{
    return fviz_widget_representation_remove_entry(
        representation, FVIZ_WIDGET_REPRESENTATION_ACTOR, representation != NULL ? representation->actors : NULL,
...
void fviz_widget_representation_remove_all_actors(FVizWidgetRepresentation* representation)
{
    ...
    fviz_widget_representation_clear_entries(representation, FVIZ_WIDGET_REPRESENTATION_ACTOR, representation->actors,
                                             &representation->actor_count);
}
...
FVizActor* fviz_widget_representation_actor_at(FVizWidgetRepresentation* representation, FVizSize index)
{
    return representation != NULL && index < representation->actor_count
               ? (FVizActor*)representation->actors[index].object
               : NULL;
}

FVizResult fviz_widget_representation_set_actor_visible(FVizWidgetRepresentation* representation, FVizActor* actor,
                                                        FVizBool visible)
{
    return fviz_widget_representation_set_entry_visible(
        representation, FVIZ_WIDGET_REPRESENTATION_ACTOR, representation != NULL ? representation->actors : NULL,
        representation != NULL ? representation->actor_count : 0u, (FVizObject*)actor, visible);
}

FVizResult fviz_widget_representation_add_text_actor_2d(FVizWidgetRepresentation* representation,
                                                        FVizTextActor2D* actor)
{
    return fviz_widget_representation_add_entry(representation, FVIZ_WIDGET_REPRESENTATION_TEXT_2D,
                                                ...
                                                representation != NULL ? &representation->text_actor_2d_capacity : NULL,
                                                (FVizObject*)actor);
}

FVizResult fviz_widget_representation_remove_text_actor_2d(FVizWidgetRepresentation* representation,
                                                           FVizTextActor2D* actor)
{
...
FVizTextActor2D* fviz_widget_representation_text_actor_2d_at(FVizWidgetRepresentation* representation, FVizSize index)
{
    return representation != NULL && index < representation->text_actor_2d_count
               ? (FVizTextActor2D*)representation->text_actors_2d[index].object
               : NULL;
}

FVizResult fviz_widget_representation_set_text_actor_2d_visible(FVizWidgetRepresentation* representation,
                                                                FVizTextActor2D* actor, FVizBool visible)
{
    return fviz_widget_representation_set_entry_visible(
        representation, FVIZ_WIDGET_REPRESENTATION_TEXT_2D,
        representation != NULL ? representation->text_actors_2d : NULL,
        representation != NULL ? representation->text_actor_2d_count : 0u, (FVizObject*)actor, visible);
}

FVizResult fviz_widget_representation_add_billboard_text_actor_3d(FVizWidgetRepresentation* representation,
                                                                  FVizBillboardTextActor3D* actor)
{
    return fviz_widget_representation_add_entry(
        ...
        representation != NULL ? &representation->billboard_text_actor_3d_capacity : NULL, (FVizObject*)actor);
}

FVizResult fviz_widget_representation_remove_billboard_text_actor_3d(FVizWidgetRepresentation* representation,
                                                                     FVizBillboardTextActor3D* actor)
{
    return fviz_widget_representation_remove_entry(
        representation, FVIZ_WIDGET_REPRESENTATION_BILLBOARD_3D,
...
FVizSize fviz_widget_representation_billboard_text_actor_3d_count(const FVizWidgetRepresentation* representation)
{
    return representation != NULL ? representation->billboard_text_actor_3d_count : 0u;
}

FVizBillboardTextActor3D*
fviz_widget_representation_billboard_text_actor_3d_at(FVizWidgetRepresentation* representation, FVizSize index)
{
    return representation != NULL && index < representation->billboard_text_actor_3d_count
               ? (FVizBillboardTextActor3D*)representation->billboard_text_actors_3d[index].object
               : NULL;
}

FVizResult fviz_widget_representation_set_billboard_text_actor_3d_visible(FVizWidgetRepresentation* representation,
                                                                          FVizBillboardTextActor3D* actor,
                                                                          FVizBool visible)
{
    return fviz_widget_representation_set_entry_visible(
        representation, FVIZ_WIDGET_REPRESENTATION_BILLBOARD_3D,
        representation != NULL ? representation->billboard_text_actors_3d : NULL,
        representation != NULL ? representation->billboard_text_actor_3d_count : 0u, (FVizObject*)actor, visible);
}

FVizResult fviz_widget_representation_add_label_set_3d(FVizWidgetRepresentation* representation,
                                                       FVizLabelSet3D* label_set)
{
    return fviz_widget_representation_add_entry(representation, FVIZ_WIDGET_REPRESENTATION_LABEL_SET_3D,
                                                ...
                                                representation != NULL ? &representation->label_set_3d_capacity : NULL,
                                                (FVizObject*)label_set);
}

FVizResult fviz_widget_representation_remove_label_set_3d(FVizWidgetRepresentation* representation,
                                                          FVizLabelSet3D* label_set)
{
...
FVizLabelSet3D* fviz_widget_representation_label_set_3d_at(FVizWidgetRepresentation* representation, FVizSize index)
{
    return representation != NULL && index < representation->label_set_3d_count
               ? (FVizLabelSet3D*)representation->label_sets_3d[index].object
               : NULL;
}

FVizResult fviz_widget_representation_set_label_set_3d_visible(FVizWidgetRepresentation* representation,
                                                               FVizLabelSet3D* label_set, FVizBool visible)
{
    return fviz_widget_representation_set_entry_visible(
        representation, FVIZ_WIDGET_REPRESENTATION_LABEL_SET_3D,
        representation != NULL ? representation->label_sets_3d : NULL,
...
void fviz_widget_representation_remove_all_annotations(FVizWidgetRepresentation* representation)
{
    ...
    fviz_widget_representation_clear_entries(representation, FVIZ_WIDGET_REPRESENTATION_BILLBOARD_3D,
                                             ...
                                             &representation->billboard_text_actor_3d_count);
...
void fviz_widget_representation_set_visible(FVizWidgetRepresentation* representation, FVizBool visible)
{
    FVizSize i;
    if (representation == NULL) return;
    visible = visible != FVIZ_FALSE ? FVIZ_TRUE : FVIZ_FALSE;
    ...
    for (i = 0u; i < representation->actor_count; ++i)
        fviz_widget_representation_apply_visibility(representation, FVIZ_WIDGET_REPRESENTATION_ACTOR,
                                                    &representation->actors[i]);
    for (i = 0u; i < representation->text_actor_2d_count; ++i)
        fviz_widget_representation_apply_visibility(representation, FVIZ_WIDGET_REPRESENTATION_TEXT_2D,
                                                    &representation->text_actors_2d[i]);
    for (i = 0u; i < representation->billboard_text_actor_3d_count; ++i)
        fviz_widget_representation_apply_visibility(representation, FVIZ_WIDGET_REPRESENTATION_BILLBOARD_3D,
                                                    &representation->billboard_text_actors_3d[i]);
    for (i = 0u; i < representation->label_set_3d_count; ++i)
        fviz_widget_representation_apply_visibility(representation, FVIZ_WIDGET_REPRESENTATION_LABEL_SET_3D,
                                                    &representation->label_sets_3d[i]);
    fviz_object_modified((FVizObject*)representation);
}
