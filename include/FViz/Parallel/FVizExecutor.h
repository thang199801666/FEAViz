...
FVIZ_PARALLEL_API void fviz_executor_options_initialize(FVizExecutorOptions* options);
FVIZ_PARALLEL_API FVizResult fviz_executor_create(const FVizExecutorOptions* options, FVizExecutor** out_executor);
...
FVIZ_PARALLEL_API void fviz_executor_destroy(FVizExecutor* executor);
FVIZ_PARALLEL_API FVizResult fviz_executor_submit(FVizExecutor* executor, int priority, FVizTaskFn function, void* user_data,
...
FVIZ_PARALLEL_API FVizResult fviz_executor_submit_context(FVizExecutor* executor, int priority, FVizTaskContextFn function,
...
FVIZ_PARALLEL_API FVizResult fviz_future_then(FVizFuture* antecedent, FVizExecutor* executor, int priority,
                                     FVizContinuationFn function, void* user_data,
                                     FVizTaskDataDestroyFn user_data_destroy, FVizTaskValueDestroyFn value_destroy,
                                     FVizFuture** out_future);
FVIZ_PARALLEL_API void fviz_executor_get_statistics(const FVizExecutor* executor, FVizExecutorStatistics* out_statistics);

FVIZ_PARALLEL_API void fviz_future_cancel(FVizFuture* future);
FVIZ_PARALLEL_API FVizBool fviz_future_ready(const FVizFuture* future);
FVIZ_PARALLEL_API FVizCancellationToken* fviz_task_context_cancellation(FVizTaskContext* context);
FVIZ_PARALLEL_API FVizResult fviz_task_context_report_progress(FVizTaskContext* context, double progress);
FVIZ_PARALLEL_API double fviz_future_progress(const FVizFuture* future);
FVIZ_PARALLEL_API void fviz_future_set_progress_callback(FVizFuture* future, FVizFutureProgressFn callback, void* user_data);
FVIZ_PARALLEL_API FVizResult fviz_future_wait(FVizFuture* future);
/* Transfers the task value exactly once after successful completion. */
FVIZ_PARALLEL_API FVizResult fviz_future_take_value(FVizFuture* future, void** out_value);
FVIZ_PARALLEL_API void fviz_future_destroy(FVizFuture* future);
